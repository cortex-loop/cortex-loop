    ...
    def __post_init__(self) -> None:
        ...
        for entry in self.brake_tonic_history:
            if isinstance(entry, bool) or not isinstance(entry, (int, float)):
                ...
                raise TypeError(
                    "ReferenceRuntimeSessionArtifact.brake_tonic_history must contain only "
                    f"numeric values in [0.0, 1.0], got {actual_type}."
                )
            if not 0.0 <= float(entry) <= 1.0:
                raise ValueError(
                    "ReferenceRuntimeSessionArtifact.brake_tonic_history entries must be "
                    "between 0.0 and 1.0."
                )
        if (
...
def parse_reference_runtime_session_artifact(
    ...
    artifact = ReferenceRuntimeSessionArtifact(
        ...
        brake_tonic_history=_brake_tonic_history_tuple(
            ...
            "ReferenceRuntimeSessionArtifact.control_residue.brake_tonic_history",
        ),
    )
