...
class InterventionBudget:
    ...
    def __post_init__(self) -> None:
        ...
        if isinstance(self.remaining_repairs, bool) or not isinstance(self.remaining_repairs, int):
...
class PreservationState:
    ...
    def from_payload(cls, payload: Mapping[str, Any]) -> PreservationState:
        ...
        return cls(
            ...
            intervention_budget=InterventionBudget.from_payload(payload["intervention_budget"]),
        )
...
def derive_preservation_state(
    ...
    *,
    ...
    if outcome.status == "passed" or outcome.failure_class in _STOP_FAILURE_CLASSES | _CHECK_FAILURE_CLASSES:
...
def choose_preservation_move(state: PreservationState) -> str:
    ...
    for move in ("continue", "check", "repair", "stop"):
        if move in state.intervention_budget.allowed_moves:
            return move
    raise ValueError("PreservationState.intervention_budget.allowed_moves must contain a legal move.")
