...
.PHONY: product-test product-openai-cli product-openai-service experimental-test repo-hygiene $(LAB_ALIAS_TARGETS)

product-test:
	$(PYTEST) tests/product -q

product-openai-cli:
	$(PYTHON) -m cortex.runtime.openai_cli $(ARGS)

product-openai-service:
	$(PYTHON) -m cortex.runtime.openai_service $(ARGS)
