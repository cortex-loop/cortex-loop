...
- header: `## Cortex Repo Hygiene Grid`
...
`Std:*`, `Mirror:*`, and (when active) `Dogfood:*` rows with the
actual value. Do not paste a verbatim skeleton with
...
4. Any `Std:*`, `Mirror:*`, or active `Dogfood:*` row still contains
