...
**Single-grid rule.** Every chat ends with the Cortex Repo Hygiene
Grid as the single closure artifact. All closure / handoff material
lives inside the `## Cortex Repo Hygiene Grid` markdown block. Normal
response prose may precede the grid; nothing closure-shaped may
follow it or appear before it (no separate metadata block, no
separate Final Handoff Mirror, no separate verification summary).
...
It renders as one consolidated markdown block with these sections:
...
- `### Work Reflection` (10 prompts, only when work was performed)
- `### Standard Metadata` (table, required — closure metadata)
- `### Final Handoff Mirror` (sections, required — closure mirror)
- `### Dogfood Signal` (only when Codex App dogfood mode is active)
...
then edits the skeleton in place by replacing each
`[≥48 chars + cite a repo surface — …]` Goals Analysis prompt with
substantive prose, and replacing each `<fill: …>` placeholder in
Standard Metadata, Final Handoff Mirror, and (when active) Dogfood
Signal with the actual value. Do not paste a verbatim skeleton with
brackets remaining; do not write a separate closure section outside
the grid.
...
1. Required signature markers missing
...
2. Closure-shaped substrings (`Ending branch`, `Verification summary`,
   ...
   `Final Handoff Mirror`) appear in prose **before** the grid header.
3. Any of the five Goals Analysis fields still contains the literal
   `≥48 chars + cite a repo surface —` template substring (per-field
   check; one unfilled field is enough to block).
4. Any Standard Metadata or Final Handoff Mirror cell still contains
   the `<fill` placeholder substring.
...
finalize, or publish on `FAIL`. The grid's Goals Analysis section
requires substantive answers (each field ≥48 chars) with at least one
repo-surface citation (`docs/CORTEX.md`,
`internal/truth/cortex_status.json`, a `cortex/**` path, a V2 packet
section, a test file). Handwave answers (e.g. "fine", "looks good",
"no issues") are rejected by `reflection-check`'s substantive-content
rule on close-session.
