...
| `feedback_window_realization` | `decision_visible` | host output / realization feedback → feedback window → next runtime step policy and route selection | future route/brake/closure decisions, not direct text unless a later host-control request translates it | prevents stream-only churn from counting as progress and raises pressure after repeated rejection/override/probe failure | Feedback windows are structural; live claim that they improve output needs pinned before/after model runs. |
...
| `feedback_window_realization` | product Cortex when feedback changes next route/brake decision; monitor-only if retained only as a public summary | `bridge` | For every new feedback signal, require a route/brake/closure consequence or classify it as lab telemetry. | General learning from feedback is post-training; event-local realization feedback that alters the next runtime decision is Cortex. |
