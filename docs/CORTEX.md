...
| `feedback_window_realization` | `conditional_model_visible_openai` | host output / last `ReferenceRealizationFeedback` → `runtime_context_from_last_feedback(...)` → OpenAI host-control `instructions` for ordinary calls or verified-work `input_text` before workspace context | bounded `CORTEX_RUNTIME_CONTEXT_V1` text derived only from the immediately prior feedback entry; clean or absent feedback emits no block | shapes the next OpenAI call away from premature closure after stream-only, failed-probe, warning, friction, override, or braked realization without accumulating memory across turns | This is structural model-I/O shaping on OpenAI only; live output-quality lift and Claude/Gemini ports remain unearned until paired runs prove them. |
| `host_runtime_sessions` | `state_to_decision_visible` | host session artifact → runtime step → route/closure/policy payload → host control request or CLI/service response | carried state that influences later route and closure behavior; raw session JSON is not model input | maintains continuity across events and hosts without pretending all hosts expose the same transport affordances | Session state can be perfectly coherent internally while still failing to affect a model if no host-control path consumes it. |
| `host_control_transports` | `direct_model_visible` | `OpenAIHostControlRequest.input_text/instructions`, `ClaudeHostControlRequest.input_text/system`, `GeminiHostControlRequest.input_text/instructions` → respective live/fixture transport bodies | provider request text/system/instructions; metadata and audit intensity are boundary/control fields, not assumed model-visible text | this is the main bridge where Cortex can change what the model receives or what output is accepted | Any internal logic not consumed here, or not converted into route/block behavior before here, remains monitoring/scaffolding rather than product Cortex. |
...
| `feedback_window_realization` | product Cortex on the OpenAI host-control lane when last-step feedback is translated into bounded model-visible runtime context; still monitor-only if retained only as a public summary | `bridge_landed_openai_structural` | Keep the bridge last-feedback-only and run paired live baseline-vs-shaped evaluations before claiming output lift or porting the path to Claude/Gemini. | General learning from feedback is post-training; event-local realization feedback that alters the next runtime decision is Cortex. |
