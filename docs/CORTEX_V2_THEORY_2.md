...
- `docs/CORTEX_V2_ACTIVE_WORKSTREAM.md` now records the bounded K train as landed, records `N2` as blocked pending a capable machine, records `M2`, `J1`, `J2`, and `J3` as landed on `main`, and records the current J4 implementation result on the review line: exact runtime-visible missing-capability / missing-context pressure now admits and selects `seek-context` on the reference runtime lane, generic host friction still does not open that route, and `J4C` remains closed pending canonical-line acceptance.
