# scenario_host_reference_01__baseline_non_mediated__run_003
...
This committed run packet records one reference-host baseline-only host-realization packet within the committed reference host-realization paired-run series for mediation evidence review.
...
- run_id: `reference_host_realization_baseline_run_003`
...
- starting_request_or_event: `ApprovalResult` with `commitment_id=commit-packet-3` and `session_id=packet-session-3`
...
- event_trace_refs: `reference-lane:commit-packet-3`
- contradiction_refs: `artifact-check:supporting artifact trace remained partial`
- degradation_refs: `host-surface-degraded-003`
- aux_burden_refs_if_present: none
- evaluation_packet_refs_if_present: `docs/experimental/CORTEX_V2_REFERENCE_LANE_PACKET_EXAMPLE_2.md`

## Lift-Axis Observations

### Reduced Thrashing

- observation: This baseline packet is part of the committed reference host-realization paired-run series, but it is not a branch-control comparison.
...
- failure_tags: `none`
- notes: This packet is the baseline side of `pair_reference_host_003`. A single packet does not justify mediation; package-level evidence notes govern verdicts.

## Reviewer Note

- reviewer_note: This is baseline-only committed evidence within the committed reference host-realization paired-run series. It is not comparative mediation evidence by itself, does not justify mediation, and package-level evidence notes govern any verdict.
