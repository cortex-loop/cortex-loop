...
| AUX-only SQLite episode store over bounded support-memory episodes with fingerprint dedupe and host/time filtering | `SqliteSupportMemoryStore` | `cortex/aux/persistence.py` | `test_aux_persistence.py::test_sqlite_support_memory_store_round_trips_dedupes_and_filters` + `test_aux_persistence.py::test_sqlite_support_memory_store_memory_path_persists_within_store_instance` | landed |
| host-scoped offline distillation over bounded stored episodes back into explicit offline publication | `distill_offline_support_publication()` | `cortex/aux/distillation.py` | `test_aux_distillation.py::test_distillation_requires_two_supporting_episodes_for_positive_priors` + `test_aux_distillation.py::test_distillation_suppresses_positive_priors_for_burden_heavy_window` + `test_aux_distillation.py::test_distillation_empty_window_keeps_baseline_inactive` | landed |
