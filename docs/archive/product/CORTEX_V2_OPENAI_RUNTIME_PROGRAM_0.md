...
- `python3 -m cortex.hosts.openai.cli`
...
- `OpenAIRuntimeSession`, `OpenAIRuntimeSessionArtifact`, `run_openai_runtime_step()`, and `python3 -m cortex.hosts.openai.cli` are now landed `O1` surfaces,
...
- `python3 -m cortex.hosts.openai.cli` now emits the compact `decision + journal` projection while preserving `raw_host_event_name`,
