...
- `python3 -m cortex.hosts.openai.ingress_cli`
...
- `python3 -m cortex.hosts.openai.ingress_cli` now drives the accepted `O1` runtime shell from raw transcript records,
...
- `python3 -m cortex.hosts.openai.ingress_cli` now feeds the compressed OpenAI-only `O1` shell without changing its transcript parser boundary,
