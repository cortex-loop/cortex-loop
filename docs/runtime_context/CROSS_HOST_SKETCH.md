...
runtime-context constraint sentence should remain portable across hosts if
the OpenAI bridge later earns enough evidence to port.

## Sketch

The runtime context constraint ports cleanly as provider-neutral lifecycle
context, but the placement is host-specific. OpenAI ordinary calls place
the sentence in `OpenAIHostControlRequest.instructions`; OpenAI verified
work places it in model-visible `input_text` because verified-work
instructions are fixed. A Claude port should place the same sentence in
`ClaudeHostControlRequest.system` while leaving `input_text` as the user
task. A Gemini port should place the same sentence in
...
- Host ports must preserve the same last-feedback-only rule, clean-window
  kill switch, single-sentence shape, and no-accumulation rule.
