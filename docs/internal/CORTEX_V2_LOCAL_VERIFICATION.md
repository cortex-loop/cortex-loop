...
For routine repo-local verification, use the split post-E22 entry points below:
- `make product-test`
- `make experimental-test`
- `make -C internal test`
- `make -C lab verify`
- `make -C lab test-smoke`

## Maintainer workflow

Maintainer branch/session hygiene is governed by `docs/internal/REPO_WORKFLOW.md`.
...
python internal/workflow/repo_workflow.py close-session --message "docs: end-state summary"
...
python3 -m pytest tests/experimental -q
python3 -m pytest tests/internal -q
python3 -m pytest tests/unit -q
python3 -m pytest tests/integration -q
python3 -m pytest tests/lab -q
```
...
python3 -m pytest tests/unit/test_correspondence_contract.py \
  ...
  tests/lab/test_lab_surface.py -q
```
...
```sh
make -C lab revalidate-reference-runtime
```
...
```sh
make -C lab revalidate-reference-runtime-continuity
```
...
```sh
make -C lab revalidate-openai-runtime
```
...
```sh
make -C lab revalidate-openai-ingress
```
...
```sh
make -C lab revalidate-openai-service
```
...
```sh
make -C lab revalidate-executive-loop
```
...
```sh
make -C lab revalidate-gemini-runtime
```
...
```sh
make -C lab revalidate-gemini-ingress
```
...
```sh
make -C lab revalidate-gemini-service
```
...
```sh
make -C lab revalidate-gemini-host-control
```
...
```sh
make -C lab revalidate-claude-runtime
```
...
```sh
make -C lab revalidate-claude-ingress
```
...
```sh
make -C lab revalidate-claude-service
```
...
```sh
make -C lab revalidate-claude-host-control
```
...
```sh
python3 lab/live_preflight.py --skip-updates
```
...
```sh
python3 lab/live_preflight.py --skip-updates
python3 lab/live_cortex_host_control.py --lane automation --provider openai --suite current
python3 lab/live_cortex_host_control.py --lane automation --provider openai --suite canonical_anchor
python3 lab/live_compare.py
...
python3 lab/live_cortex_host_control.py --lane automation --provider openai --suite current
python3 lab/live_cortex_host_control.py --lane automation --provider openai --suite canonical_anchor
python3 lab/live_cortex_host_control.py --lane automation --provider claude --suite canonical_anchor
```
...
```sh
python3 lab/live_provider_baselines.py --lane operator
make live-provider-baselines
python3 lab/live_provider_baselines.py --lane automation
...
```sh
python3 lab/live_host_native_product_paths.py
...
```sh
python3 lab/live_openai_app_server_operator.py
...
```sh
python3 lab/live_operator_payoff_audit.py
...
```sh
python3 lab/live_operator_directionality.py
...
```sh
python3 lab/live_operator_directionality_audit.py
...
```sh
make -C lab revalidate-reference-packet
```
...
It is useful context for the proof-packet prerequisite gate recorded in `docs/internal/CORTEX_V2_IMPLEMENTATION_STATUS_NOTE.md`, but it does not update gate truth and it does not overwrite the committed example doc.
...
```sh
make -C lab emit-reference-packet-candidate
```
...
```sh
make -C lab revalidate-gemini-packet
```
...
```sh
make -C lab emit-gemini-packet-candidate
```
...
```sh
make -C lab revalidate-openai-packet
```
...
```sh
make -C lab emit-openai-packet-candidate
```
...
```sh
make -C lab revalidate-latency-evidence
```
...
It is useful context for the landed latency evidence gate recorded in `docs/internal/CORTEX_V2_IMPLEMENTATION_STATUS_NOTE.md`, but it does not update gate truth and it does not overwrite the committed latency doc.
...
```sh
make -C lab emit-latency-evidence-candidate
```
...
python3 lab/mediation_evidence_package.py --check
```

Repo-local entry point:

```sh
make -C lab revalidate-mediation-evidence-package
```
...
```sh
make -C lab revalidate-mediation-branch-discipline
```
...
```sh
make -C lab revalidate-mediation-non-thrash-burden
```
...
```sh
make -C lab revalidate-mediation-claude-host-realization
```
...
```sh
make -C lab revalidate-mediation-claude-uncertainty
```
...
```sh
make -C lab revalidate-mediation-reference-host-realization-basis
```
...
```sh
make -C lab revalidate-reference-mediated-host-realization
```
...
```sh
make -C lab emit-reference-mediated-host-realization-candidate
```
...
```sh
make -C lab revalidate-mediation-gemini-host-realization-basis
```
...
```sh
make -C lab revalidate-gemini-mediated-host-realization
```
...
```sh
make -C lab emit-gemini-mediated-host-realization-candidate
```
...
```sh
make -C lab revalidate-openai-mediated-host-realization
```
...
```sh
make -C lab emit-openai-mediated-host-realization-candidate
```
...
```sh
make -C lab revalidate-mediation-openai-host-realization-basis
```
...
```sh
make -C lab revalidate-mediation-run-packets
```
...
```sh
make -C lab revalidate-reference-mediation-baselines
```
...
```sh
make -C lab revalidate-gemini-mediation-baselines
```
...
```sh
make -C lab emit-gemini-mediation-baselines-candidate
```
...
```sh
make -C lab revalidate-openai-mediation-baselines
```
...
```sh
make -C lab emit-openai-mediation-baselines-candidate
```
...
```sh
make -C lab revalidate-openai-mediated-thrash
```
...
```sh
make -C lab emit-openai-mediated-thrash-candidate
```
...
```sh
make -C lab revalidate-openai-mediated-uncertainty
```
...
```sh
make -C lab emit-openai-mediated-uncertainty-candidate
```
...
```sh
make -C lab revalidate-gemini-mediated-uncertainty
```
...
```sh
make -C lab emit-gemini-mediated-uncertainty-candidate
```
...
```sh
make -C lab revalidate-gemini-mediated-thrash
```
...
```sh
make -C lab emit-gemini-mediated-thrash-candidate
```
...
```sh
make -C lab emit-reference-mediation-baselines-candidate
```
...
```sh
make -C lab revalidate-reference-mediated-thrash
```
...
```sh
make -C lab emit-reference-mediated-thrash-candidate
```
...
```sh
make -C lab revalidate-reference-mediated-uncertainty
```
...
```sh
make -C lab emit-reference-mediated-uncertainty-candidate
```
...
```sh
make -C lab revalidate-mediation-gemini-uncertainty-basis
```
...
```sh
make -C lab revalidate-mediation-gemini-thrash-basis
```
...
```sh
make -C lab revalidate-mediation-openai-uncertainty-basis
```
...
```sh
make -C lab revalidate-mediation-openai-thrash-basis
```
...
```sh
make -C lab revalidate-mediation-reference-uncertainty-basis
```
...
```sh
make -C lab revalidate-mediation-reference-thrash-basis
```
...
```sh
make -C lab revalidate-mediation-evidence
```
...
```sh
make -C lab revalidate-mediation-justification
```
...
make experimental-test
make -C internal test
make -C lab seam-preflight
make -C lab test-unit
make -C lab test-integration
make -C lab test-smoke
make -C lab verify
```
...
The first committed baseline in `docs/lab/CORTEX_V2_COVERAGE_BASELINE_NOTE_0.md` was captured in an environment where that prerequisite was present.
