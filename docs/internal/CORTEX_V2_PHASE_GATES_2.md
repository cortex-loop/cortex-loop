...
| `O4R` OpenAI verified-work restoration lane | `docs/CORTEX_V2_OPENAI_VERIFIED_WORK_PROGRAM_0.md`; `make revalidate-openai-host-control`; `make -C lab revalidate-openai-operator-cli`; `make -C lab revalidate-openai-operator-repair-pressure`; repeated `python3 lab/cortex_conformance.py --mode active --brain openai`; repeated `python3 lab/cortex_conformance.py --mode repair-pressure --brain openai`; repeated `make -C lab live-openai-operator-repair-pressure`; `.cortex/train_loops/verified-work-breadth-openai/summary.json`; `.cortex/train_loops/verified-work-repair-yield-openai/summary.json` | closed | landed | on the current review line, the shared verified-work law now includes a minimal preservation-state machine over task anchor, trusted structure, falsified structure, lawful repair surface, and intervention budget; runtime-native verification now persists optional preservation state inside the compact OpenAI journal on verified-work sessions only; the OpenAI verified-work path now derives the next move from that state, records a deterministic verified-work task anchor when no active goal exists, narrows the repair contract to the lawful repair surface, and verifies the second attempt on top of the preserved first-attempt file map; the thin `O4` path remains unchanged, the deterministic `full_files` verifier/profile registry and one bounded repair turn remain intact, the accepted product/runtime proof remains the historical OpenAI `service_api` line, and E24 records OpenAI `operator_cli` as the active proving-default lane for new conformance and train-loop iteration without repurposing the historical `CT2` anchor; the repair-pressure proving surface now exercises the preservation-aware repair branch cleanly on bookmarks and feature-flags and repeatedly through the canonical repo-local entrypoint, but the direct normalize-port proof set still contains one unreproduced operator-resume payload-sanitization crash in the shared OpenAI operator proof plumbing, so the current E23 operator-cli keep/cut read remains partial / env-sensitive rather than promotable |
