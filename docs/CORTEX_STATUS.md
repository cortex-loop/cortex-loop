...
- Current tracked train: `cortex-retained-active-policy-spine-live-gate1`
- Active quality/risk focus: The no-live retained-spine Live Gate 1 seam produced `pass_cortex_retained_active_policy_spine_live_gate1` and wrote `.cortex/live_validation/cortex_retained_active_policy_spine_live_gate1/retained_spine_contract.json`, `evaluator_design.json`, `live_plan.json`, `episode_table.jsonl`, `summary.json`, `leaderboard.json`, and `failure_analysis.json`. It schedules `60` dry-run rows (`4` arms x `5` v2 task families x `3` repeats) for the retained active-policy spine `userpromptsubmit_stop_taskstandard_spine` against `no_cortex_baseline`, `simple_hook_baseline`, `cortex_silent_perception`, and `cortex_active_policy`. The active arm uses only `userpromptsubmit_stop_taskstandard_spine`; PostToolUse task-standard context remains `role_demoted_non_current_support_history` and is not reactivated as current strategy, product progress, or earned active policy. The seam registers the future retained-spine live command/env pair `CORTEX_CODEX_APP_CLI_RETAINED_SPINE_LIVE_APPROVED=approved python3 lab/cortex_effectiveness_evaluator.py --retained-spine-live-matrix`; without approval the command returns `not_run_approval_required`, and the live runner remains not implemented in this no-live seam. The plan consumes the v2 registry, preserves `run_20260508T221352Z` as `failure_silent_perception_contamination`, preserves `run_20260509T112542Z` as `failure_no_value`, keeps simple-hook parity and silent-perception success as value blockers, and does not rescore v1/v2 artifacts. This seam did not run live, delete product code, change product host behavior, model-visible Cortex text, evaluator scoring, fixtures, hidden-verifier boundaries, root hooks, SRE law, active policy, or candidate policy. It does not earn Cortex value, behavior lift, exactness value lift, broad Cortex lift, Codex App parity, shipping promotion, product progress, or AlphaEvolve candidate-evolution permission. The next train is `cortex-retained-active-policy-spine-live-run`.
- Next product train after the current focus: `cortex-retained-active-policy-spine-live-run`
...
- Train: `cortex-retained-active-policy-spine-live-run`
- Surface: approval-gated retained-spine evaluator live matrix
- Executive benefit: Run the retained UserPromptSubmit + Stop + TaskStandardSpine spine through the registered four-arm v2 evaluator live matrix to test whether the smaller active spine beats no-Cortex, simple-hook, and silent controls before any program database, candidate generation, or AlphaEvolve automation loop.
...
- Primary metric: With explicit current-turn approval, execute `CORTEX_CODEX_APP_CLI_RETAINED_SPINE_LIVE_APPROVED=approved python3 lab/cortex_effectiveness_evaluator.py --retained-spine-live-matrix`; record whether `userpromptsubmit_stop_taskstandard_spine` earns scoped retained-spine value over `no_cortex_baseline`, `simple_hook_baseline`, and `cortex_silent_perception` with no dominance failure. Simple-hook parity and silent-perception success remain no-value blockers.
- Guardrail: No product code deletion, no product behavior change, no product host behavior change, no model-visible Cortex text change, no evaluator scoring or fixtures change, no hidden-verifier boundary change, no root hook change, no SRE law change, no active policy mutation, no PostToolUse reactivation, and no AlphaEvolve candidate policy. Live Codex is allowed only through the exact registered retained-spine evaluator command/env pair with explicit approval in the current turn. Do not claim broad Cortex lift, Codex App parity, shipping promotion, product progress, or candidate-evolution permission. Do not change product behavior, model-visible Cortex text, evaluator scoring or fixtures, hidden-verifier boundaries, root hooks, SRE law, active policy, or candidate policy during this live seam.
- Kill rule: Fail or stop if the seam runs any unregistered live command, changes scoring or fixtures to favor Cortex, reactivates PostToolUse as earned active policy, treats simple-hook parity or silent success as Cortex value, mutates product policy after live starts, loads a runtime snapshot, mutates root config, omits any arm/family/repeat, or queues candidate evolution from anything short of scoped retained-spine value.
