...
- Current tracked train: `codex-app-cli-posttooluse-task-standard-actuator-architecture-decision`
- Active quality/risk focus: This no-runtime-change decision seam classifies the `failure_context_ignored` live result on `task_standard_exactness` as PostToolUse timing/selection failure: host delivery passed, task-standard capture passed in the mismatch case, clean/control overcontrol passed, hidden/root/runtime boundaries passed, and immediate next-action effect failed because context fired after a failed missing-artifact precondition; the next model tool did not perform the named direct check. Later exactness checks came through the existing Stop continuation loop, and Stop continuation remains the only live-proven corrective actuator for this exactness shape. Queue phase-aware no-live PostToolUse calibration Gate 0 before any further live run; this is not a three-arm behavior comparison, earns no broad behavior lift, and no signed text, PostToolUse text, Stop text, SRE law, scored matcher, threshold, fixture, scoring, root-hook, hidden-verifier, Sinkhorn/transport, PreToolUse, PermissionRequest, live Codex run, or three-arm comparison changes are part of this seam.
- Next product train after the current focus: `codex-app-cli-posttooluse-task-standard-phase-aware-calibration-gate0`
...
- Train: `codex-app-cli-posttooluse-task-standard-phase-aware-calibration-gate0`
- Surface: product host actuator + lab proof
- Executive benefit: Calibrate the PostToolUse actuator so context is reserved until a product-visible artifact or candidate output exists and a required task-standard item remains unresolved.
...
- Primary metric: Prove without live spend that PostToolUse stays silent after failed pre-artifact checks, emits only after candidate output exists and a required `WORK_STANDARD` / `CLOSURE_EVIDENCE` item remains unresolved, and that the harness decision window treats artifact creation as a prerequisite while still failing closure before the named direct check.
- Guardrail: Do not tune signed UserPromptSubmit text, PostToolUse text, Stop text, SRE law, scored matcher, thresholds, fixtures, scoring, root hooks, hidden-verifier boundaries, Sinkhorn/transport, PreToolUse denial, or PermissionRequest policy. No live Codex run or three-arm behavior comparison.
- Kill rule: If phase-aware structural proof cannot prevent pre-artifact context spend while preserving clean-control silence and mismatch context, stop PostToolUse live probing and return to lifecycle-actuator design.
