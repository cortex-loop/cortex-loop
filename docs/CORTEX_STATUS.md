...
- Current tracked train: `cortex-effectiveness-v2-live-matrix-gate1`
- Active quality/risk focus: The no-live Cortex effectiveness v2 live-matrix Gate 1 wired the v2 case registry into `.cortex/live_validation/cortex_effectiveness_v2_live_matrix_gate1/evaluator_design.json`, `v2_case_registry.json`, `live_plan.json`, `episode_table.jsonl`, `summary.json`, `leaderboard.json`, and `failure_analysis.json`. Verdict: `pass_cortex_effectiveness_v2_live_matrix_gate1`. It consumed the v2 registry, validated all five immutable v2 cases, and scheduled `60 dry-run rows`: four arms (`no_cortex_baseline`, `simple_hook_baseline`, `cortex_silent_perception`, `cortex_active_policy`) across five v2 families and three repeats: `exactness_evidence_recovery_v2`, `truthful_closure_v2`, `blocker_surfacing_v2`, `continuity_after_interruption_v2`, and `clean_verified_work_control_v2`. Row IDs include v2 `case_id`, repeat, and arm; workspace seeds are matched across arms for the same case/repeat while remaining row-isolatable later. The plan preserves `run_20260508T221352Z` as `failure_silent_perception_contamination`, does not replace `LIVE_MATRIX_CASES`, does not alter the v1 planner, and does not retroactively rescore v1 artifacts. It registers the future approval shape `CORTEX_CODEX_APP_CLI_EVALUATOR_LIVE_APPROVED=approved python3 lab/cortex_effectiveness_evaluator.py --v2-live-matrix` with `not_run_approval_required` when approval is absent, but it does not implement or run the v2 live runner in this seam. This earns v2 live-matrix interface readiness only; it does not earn Cortex value, behavior lift, exactness value lift, broad Cortex lift, Codex App parity, shipping promotion, product progress, or AlphaEvolve candidate-evolution permission. No live Codex run occurred and no product host behavior, model-visible Cortex text, evaluator scoring, hidden verifier boundary, root hook, SRE law, fixture, or candidate policy changed. The next train is `cortex-effectiveness-v2-live-matrix-run`.
- Next product train after the current focus: `cortex-effectiveness-v2-live-matrix-run`
...
- Train: `cortex-effectiveness-v2-live-matrix-run`
- Surface: approval-gated live evaluator v2 matrix run
- Executive benefit: Run the v2 four-arm evaluator matrix so active Cortex can be compared against no-Cortex, the small simple-hook challenger, and silent Cortex on sharper v2 task families before any value claim or candidate evolution.
...
- Primary metric: With exact current-turn approval, run `CORTEX_CODEX_APP_CLI_EVALUATOR_LIVE_APPROVED=approved python3 lab/cortex_effectiveness_evaluator.py --v2-live-matrix` and produce 60 live rows plus `summary.json`, `episode_table.jsonl`, `leaderboard.json`, and `failure_analysis.json`; active Cortex earns scoped value only if it beats both no-Cortex and the simple hook on at least one v2 family with no dominance failure, no silent contamination, no simple-hook parity, no root/runtime mutation, and no hidden-verifier leakage.
- Guardrail: Live is allowed only through the exact registered command/env pair and must not use `--require-pass`; negative verdicts are evidence. Do not change product host behavior, model-visible Cortex text, signed UserPromptSubmit text, PostToolUse text, Stop text, evaluator scoring, fixtures, hidden-verifier boundaries, root hooks, SRE law, matcher threshold, Sinkhorn/transport, candidate policy, PreToolUse denial, PermissionRequest policy, output-law, typed intervention, host runtime extraction, or AlphaEvolve candidate mutation in the live-run interpretation seam. Any positive value result stops for user review before merge.
- Kill rule: Fail or stop for architecture decision if the live run omits a required arm or v2 family, mutates root config or runtime snapshot boundaries, leaks hidden verifier data, credits simple-hook parity or silent perception success as Cortex value, weakens dominance gates, changes scoring/fixtures/product policy during interpretation, produces ambiguous/missing artifacts, or auto-merges a positive value claim.
