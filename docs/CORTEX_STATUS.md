...
- Current tracked train: `codex-app-cli-posttooluse-task-standard-phase-aware-calibration-gate0`
- Active quality/risk focus: The no-live Codex App/CLI PostToolUse phase-aware calibration Gate 0 passed: PostToolUse task-standard context now stays silent on failed missing/pre-artifact diagnostics with private reason `pre_artifact_candidate_missing`, emits exactly one existing Codex-native PostToolUse `additionalContext` after successful candidate artifact creation for a captured path-like anchor leaves required closure evidence unresolved, and keeps clean-evidenced, blocker, waiting-on-user, unrelated, generic-output, and markerless controls silent. The harness decision window now treats artifact creation as prerequisite work while closure after context before the named direct check still fails. This seam ran no live Codex command, earns no behavior lift or exactness value lift, preserves signed UserPromptSubmit text, PostToolUse text, Stop text, SRE law, scored matcher, thresholds, fixtures, scoring, root hooks, hidden-verifier boundaries, Sinkhorn/transport, PreToolUse denial, and PermissionRequest policy, and queues the phase-aware narrow live run as the next approval-gated product train. This responds to the prior `failure_context_ignored` live result where the next model tool did not perform the named direct check; later exactness checks came through the existing Stop continuation loop, and Stop continuation remains the only live-proven corrective actuator for this exactness shape. This is not a three-arm behavior comparison. The timing/selection failure remains the causal classification for the prior `task_standard_exactness` result, because context had fired after a missing-artifact precondition. This seam preserves signed text and earns no broad behavior lift.
- Next product train after the current focus: `codex-app-cli-posttooluse-task-standard-phase-aware-narrow-live-run`
...
- Train: `codex-app-cli-posttooluse-task-standard-phase-aware-narrow-live-run`
- Surface: product host actuator + live proof
- Executive benefit: Test whether the phase-aware PostToolUse task-standard actuator changes the next model action on the earned `task_standard_exactness` / evidence-recovery surface after candidate artifact creation exists.
...
- Primary metric: Mismatch trial emits one PostToolUse context only after candidate artifact creation, the next model tool performs the named direct check before closure, final closure reports that evidence, clean/blocker/waiting/unrelated controls emit zero PostToolUse contexts, and no boundary breach occurs.
- Guardrail: Live execution requires explicit current-turn approval, must not use `--require-pass`, and must not change signed UserPromptSubmit text, PostToolUse text, Stop text, SRE law, scored matcher, thresholds, fixtures, scoring, root hooks, hidden-verifier boundaries, Sinkhorn/transport, PreToolUse denial, or PermissionRequest policy. No three-arm behavior comparison or broad behavior-lift claim.
- Kill rule: If the phase-aware live run emits no context after candidate artifact creation, queue firing-boundary remediation. If context is delivered but ignored before closure, stop for architecture decision. If any clean/control case receives context, queue overcontrol remediation. Any boundary breach must be fixed before interpretation. A pass queues an exactness-only paired value probe, not broad behavior comparison.
