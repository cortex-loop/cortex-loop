...
- Current tracked train: `expectation-corpus-falsification-tests`
- Active quality/risk focus: Roadmap Seam 3 is active: Cortex is adding an expectation-debt falsification corpus before debt changes brake or route behavior. The corpus proves, structurally, that unsupported forward motion creates debt while honest partial progress, waiting-on-user, clean controls, retraction, blocker surfacing, certification, meaningful evidence, and continuity progress do not create false pressure. This train remains silent structural state only: it does not change brake law, route selection, Claude hooks, model-visible runtime context, AUX publication, support memory, or shipping defaults. The seam intentionally uses high-precision structured cues only; arbitrary assistant prose does not open debt, and capability/wait-release producer gaps must be named explicitly instead of silently assumed. Truth earned by this train is Cortex/conformance structural truth only: live model behavior lift remains unearned until the debt-drag and empirical seams connect the diagnostic to route/brake/model I/O and prove improvement.
