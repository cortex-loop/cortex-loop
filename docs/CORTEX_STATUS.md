...
- Active quality/risk focus: The no-live retained-spine clean-control replication Gate 1 registered the narrow future replication check for `clean_verified_work_control_v2` after the stability Gate 0 classified `run_20260509T192719Z` repeat 1 as `no_cortex_closure_readout_instability`. Gate 1 command `python3 lab/cortex_effectiveness_evaluator.py --retained-spine-clean-control-replication-gate1 --require-pass` passed and wrote `clean_control_replication_plan.json`, `episode_table.jsonl`, `gate1_report.json`, `summary.json`, and `registered_live_command.json` under `.cortex/live_validation/cortex_retained_spine_clean_control_replication_gate1/`. The plan has exactly 20 dry-run rows: four arms (`no_cortex_baseline`, `simple_hook_baseline`, `cortex_silent_perception`, `cortex_active_policy`) across `clean_verified_work_control_v2` for five repeats, with deterministic row IDs and matched workspace seeds per repeat. Active rows use only `userpromptsubmit_stop_taskstandard_spine`; PostToolUse task-standard context remains disabled and role-demoted; simple-hook support I/O stays lab-only metadata with non-active mission rows using `model_io_path=none_lab_proof_only` and `product_spine=[]`. The future live command is registered as `CORTEX_CODEX_APP_CLI_RETAINED_SPINE_CLEAN_CONTROL_REPLICATION_APPROVED=approved python3 lab/cortex_effectiveness_evaluator.py --retained-spine-clean-control-replication-live`; without approval it returns `not_run_approval_required`, and with approval in this seam it returns `not_run_registered_future_live_only`. The next train is `cortex-retained-spine-clean-control-replication-live-run`. This seam earns replication readiness only, not Cortex value, retained-spine value, retained-spine no-value parity, behavior lift, exactness value lift, broad Cortex lift, Codex App parity, shipping promotion, product progress, live result interpretation, full retained-spine matrix rerun permission, PostToolUse reactivation, or AlphaEvolve candidate-evolution permission.
