...
Current train: `codex-app-cli-inner-loop-speech`. Next train: `codex-app-cli-hook-native-product-probe`. Shipping default: `openai.codex_app_cli`. Keep Cortex truth, brain-wiring truth, conformance truth, shipping truth, and live behavior-lift claims separate; structural proof alone does not earn model-output lift.
...
- Current tracked train: `codex-app-cli-inner-loop-speech`
- Active quality/risk focus: The current focus implements Cortex's model-visible Codex App/CLI speech as identity-continuous threshold thoughts, keeps attached-context text as fallback only, retires catch-all obligation speech, and adds a pure product lifecycle directive path without touching repo workflow guardrails.
- Next product train after the current focus: `codex-app-cli-hook-native-product-probe`
...
- Train: `codex-app-cli-hook-native-product-probe`
- Surface: product
- Executive benefit: Wire the pure Codex App/CLI lifecycle directive path into an isolated hook-native product probe so Cortex can gate or speak at lifecycle closure instead of relying on wrapper/resume after a completed run.
- Why now: The inner-loop speech seam gives the product adaptor lawful identity-continuous text and a pure directive builder; the next highest-leverage move is to prove the hook actuator can carry it without confusing repo workflow guardrails or wrapper/resume evidence.
- Primary metric: A live or structural product probe shows Stop/pre-closure Codex App/CLI lifecycle handling consumes selected grounded interventions, filters non-assistant events, blocks with identity-continuous text only on prior-act anchors, and keeps repo Mission Reflection hooks out of product Cortex.
...
- Kill rule: Stop if Codex App proof is generalized to Codex CLI without evidence, if repo Mission Reflection hooks are treated as product Cortex, if identity-continuous text fires without a prior-act anchor, or if the seam cannot show a host lifecycle path to model input/output.
