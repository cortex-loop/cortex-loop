...
- Current tracked train: `codex-app-cli-product-hook-coordinator`
- Active quality/risk focus: The current focus builds the OpenAI Codex App/CLI product hook coordinator only: normalized lifecycle payloads, private per-session state, Stop-first grounded intervention coordination, host block JSON mapping, and structural guardrails proving project hook configuration and repo Mission Reflection hooks remain untouched.
- Next product train after the current focus: `codex-app-cli-hook-native-stop-activation-probe`
...
- Train: `codex-app-cli-hook-native-stop-activation-probe`
- Surface: product
- Executive benefit: Activate the isolated Codex App/CLI product hook coordinator for a Stop-only probe so Cortex can prove identity-continuous closure inhibition through a real host lifecycle actuator.
- Why now: The coordinator seam gives the product target a private state store and host-response mapper without touching hook config; the next highest-leverage move is the narrow activation/probe that proves whether that coordinator reaches real model I/O.
- Primary metric: A Stop-only activation probe shows real Codex App/CLI hook payloads reach the product coordinator, title/null-transcript and stop_hook_active events stay silent, selected grounded interventions return block JSON with identity-continuous text, and model continuation evidence is captured without using repo guardrails.
- Guardrail: Do not generalize Codex App hook proof to Codex CLI, do not activate PreToolUse/PostToolUse speech yet, do not import repo Mission Reflection hooks, and do not use hidden verifier facts, lab prompt scaffolds, or task identity as product perception.
- Kill rule: Stop if activation requires reusing the repo Mission Reflection hook, if project config cannot isolate product hook behavior, if identity-continuous text fires without transcript-backed prior act, or if the probe cannot reconstruct the model I/O path from hook payload to block reason.
