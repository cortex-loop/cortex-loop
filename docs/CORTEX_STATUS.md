...
- Current tracked train: `cortex-retained-active-policy-contraction-or-rebuild-decision`
- Active quality/risk focus: The retained active-policy contraction/rebuild decision recorded verdict `decision_contract_retained_spine_to_stop_only` in `docs/recon/cortex_retained_active_policy_contraction_or_rebuild_decision.md`. Evidence basis: v2 evaluator run `run_20260509T112542Z` produced `failure_no_value` with active Cortex `family_wins=0` and no boundary failures; retained-spine run `run_20260509T192719Z` preserved raw `fail` / `mission_contract_error` and corrected replay `failure_silent_perception_contamination` / `silent_perception_beat_no_cortex`; clean-control replication `run_20260510T122608Z` produced `pass_clean_control_stable` with no-Cortex, simple-hook, and silent Cortex mean score `3.0` while active Cortex scored mean `2.6` because repeat 4 missed truthful closure/evidence; and PostToolUse paired artifact `task_standard_posttooluse_paired_value_live_20260508T120907Z` remains `failure_no_value` with PostToolUse-specific proof surfaces role-demoted. The decision contracts UserPromptSubmit task-standard formation out of the active value spine, retains Stop closure / continuation as the smallest active product model-I/O surface with independent safety evidence to isolate next, keeps TaskStandardSpine and SRE tool-evidence classifier as support law only, keeps the v2 evaluator/simple-hook challenger as proof infrastructure, and defers the external audit host runtime duplication risk because current evidence says the immediate blocker is active-policy value over the simple hook, not host-shell drift. No Cortex value, retained-spine value, behavior lift, product progress, shipping promotion, PostToolUse reactivation, or AlphaEvolve candidate-evolution permission is earned. The next train is `cortex-stop-only-retained-spine-gate0`.
- Next product train after the current focus: `cortex-stop-only-retained-spine-gate0`
...
- Train: `cortex-stop-only-retained-spine-gate0`
- Surface: no-live Stop-only retained active-policy proof gate
- Executive benefit: Define the Stop-only retained active spine before any more live value work: isolate the smallest active product model-I/O surface that still has independent Stop closure/continuation evidence after the composed `userpromptsubmit_stop_taskstandard_spine` produced v2 `failure_no_value` and then underperformed clean-control replication.
...
- Primary metric: Stop-only Gate 0 records the exact product model-I/O path, supporting state law, forbidden claims, retained/deferred surfaces, and future proof criteria for a minimal Stop closure/continuation spine without claiming value.
- Guardrail: No live Codex run; no exact registered command/env pair applies in this Gate 0; no product deletion; no product code deletion. No product behavior change. Do not change product host behavior, model-visible Cortex text, evaluator scoring or fixtures, hidden-verifier boundary, root hook, SRE law, UserPromptSubmit active value policy, PostToolUse policy, PostToolUse reactivation, or AlphaEvolve candidate evolution. The Gate 0 may define a Stop-only contract and proof criteria only; it must not claim Cortex value or product progress.
- Kill rule: Fail or stop if the seam treats Stop-only as value without proof, reintroduces UserPromptSubmit as active value machinery, reactivates PostToolUse, deletes product law without removal proof, changes scoring or fixtures, runs live, queues candidate evolution, or omits the retained-spine `failure_no_value`, clean-control `pass_clean_control_stable`, and contraction decision evidence.
