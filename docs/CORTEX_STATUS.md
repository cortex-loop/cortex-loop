...
- Current tracked train: `codex-app-cli-product-perception-live-probe`
- Active quality/risk focus: The current focus ran the no-snapshot Codex App/CLI product-perception live probe. The isolated Codex CLI subject loaded the product Stop hook client, omitted --runtime-snapshot, preserved root repo guardrails, and wrote live diagnostics, but the real project-hook payload stream exposed Stop-only rows: 3 Stop rows, 0 UserPromptSubmit/tool/failure rows, 0 block rows, and silence reasons missing_product_perception_state then stop_hook_active. This earns a scoped negative on live payload sufficiency, not behavior lift.
- Next product train after the current focus: `codex-app-cli-product-event-capture-remediation`
...
- Train: `codex-app-cli-product-event-capture-remediation`
- Surface: product
- Executive benefit: Repair the Codex App/CLI hook-native product perception path so task-set and tool events can populate Cortex state before Stop without prewritten runtime snapshots.
- Why now: The no-snapshot live probe proved project hooks load but exposed only Stop payloads to the product hook client, while the Codex JSON stream contained command events that did not reach coordinator state.
- Primary metric: A rerun records product-observable task and tool events before Stop, derives or suppresses intervention from that persisted state without --runtime-snapshot, and keeps exact product-rendered text gated to due expectations only.
...
- Kill rule: Stop if Codex App/CLI cannot expose or adapt product-observable prompt/tool events without replacing Cortex perception with lab-side classification or prewritten snapshot state.
