...
- Current tracked train: `cortex-effectiveness-v2-case-registry-gate0`
- Active quality/risk focus: The no-live Cortex effectiveness v2 case-registry Gate 0 turned the measurement-stack diagnosis from `run_20260508T221352Z` into `.cortex/live_validation/cortex_effectiveness_v2_case_registry_gate0/v2_case_registry.json`, `gate0_report.json`, and `summary.json`. Verdict: `pass_cortex_effectiveness_v2_case_registry_gate0`. It preserves the v1 matrix artifact `run_20260508T221352Z` as `failure_silent_perception_contamination` and explicitly says no v1 live case was retroactively rescored. The registry defines five immutable `*_v2` case specs: `exactness_evidence_recovery_v2`, `truthful_closure_v2`, `blocker_surfacing_v2`, `continuity_after_interruption_v2`, and `clean_verified_work_control_v2`. Each case carries measurement rationale, baseline expectation, simple-hook challenge, silent-contamination guard, active-policy signal, dominance gates, acceptance criteria, forbidden shortcuts, v1 failure link, and the mission contract fields `executive_function`, `loop_stage`, `control_mode`, `truth_scope`, `model_io_path`, `product_spine`, and `contraction_implication`. Exactness, truthful closure, and blocker cases are designed not to collapse into simple-baseline parity; continuity includes a silent-contamination guard; clean verified-work remains a zero-intervention control. This earns v2 case-registry readiness only; it does not earn behavior lift, exactness value lift, broad Cortex lift, Codex App parity, shipping promotion, product progress, or AlphaEvolve candidate-evolution permission. No live Codex run occurred and no product host behavior, model-visible Cortex text, evaluator scoring, hidden verifier boundary, root hook, SRE law, fixture, or candidate policy changed. The next train is `cortex-effectiveness-v2-live-matrix-gate1`.
- Next product train after the current focus: `cortex-effectiveness-v2-live-matrix-gate1`
...
- Train: `cortex-effectiveness-v2-live-matrix-gate1`
- Surface: no-live lab/proof evaluator v2 live-matrix interface
- Executive benefit: Wire the v2 evaluator case registry into a dry-run future four-arm matrix plan so the next approved live run can test active Cortex against no-Cortex, simple-hook, and silent controls on sharper cases.
...
- Primary metric: Produce a no-live v2 live-matrix dry-run plan that consumes the v2 registry, schedules all four arms across all five v2 families with deterministic row IDs and workspace isolation, writes `live_plan.json`, `episode_table.jsonl`, `summary.json`, `leaderboard.json`, and `failure_analysis.json`, refuses without the exact approval env, and preserves simple-hook parity, silent success, and dominance gates as no-value.
- Guardrail: No live Codex run in this seam. Do not change product host behavior, product model-visible text, signed UserPromptSubmit text, PostToolUse text, Stop text, candidate policy, evaluator scoring, hidden-verifier boundaries, root hooks, SRE law, matcher threshold, Sinkhorn/transport, PreToolUse denial, PermissionRequest policy, output-law centralization, typed-intervention, host-runtime extraction, or AlphaEvolve candidate mutation. Do not replace or rescore v1 artifacts; v2 cases may only be wired into a dry-run/future-live plan.
- Kill rule: Fail if Gate 1 runs live, changes scoring, omits a required arm or v2 family, weakens simple-hook/silent no-value rules or dominance gates, lacks approval refusal, mutates product behavior, rewrites v1 evidence, or queues candidate evolution/value claims before an approved v2 matrix has real comparative evidence.
