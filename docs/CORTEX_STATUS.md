...
Current train: `codex-app-cli-stop-continuation-resolution-loop`. Next train: `codex-app-cli-hook-native-behavior-comparison`. Shipping default: `openai.codex_app_cli`. Keep Cortex truth, brain-wiring truth, conformance truth, shipping truth, and live behavior-lift claims separate; structural proof alone does not earn model-output lift.
...
- Current tracked train: `codex-app-cli-stop-continuation-resolution-loop`
- Active quality/risk focus: The current focus closed the Codex App/CLI Stop continuation-resolution loop on the live Codex CLI hook-native path. A scoped live run registered UserPromptSubmit, PreToolUse, PostToolUse, and Stop in an isolated subject git root with product hook config only and no --runtime-snapshot. The run passed `pass_resolved`: 1 UserPromptSubmit row, 2 PreToolUse rows, 2 PostToolUse rows, 2 Stop rows, 1 exact product-rendered block row, 2 continuation tool rows, no non-Stop stdout, no runtime snapshot, unchanged root guardrails, and final silence reason `pressure_below_visible_threshold` with the active verification expectation moved to resolved from a product-visible PostToolUse evidence ref. This earns live continuation state-accountability evidence only, not behavior lift, Codex App parity, root product hook activation, or PreToolUse motor-inhibition policy.
- Next product train after the current focus: `codex-app-cli-hook-native-behavior-comparison`
...
- Train: `codex-app-cli-hook-native-behavior-comparison`
- Surface: product
- Executive benefit: Measure whether the now-complete Codex CLI hook-native Stop loop improves real model behavior over silent-only control on closure-pressure tasks without slowing or overblocking clean work.
- Why now: The Stop continuation-resolution loop proved prompt/tool/Stop payloads can open, block, repair, and resolve a verification expectation from product-visible state; the remaining high-risk question is live behavior lift against a silent comparator.
- Primary metric: A paired live matrix shows hook-native Cortex improves premature-closure avoidance, evidence recovery, or goal continuity over silent-only control while clean verified, waiting/blocker, and honest partial-progress controls remain silent and unblocked.
...
- Kill rule: If hook-native Stop inhibition does not improve behavior over silent-only control or materially overblocks clean controls, stop and queue law/perception remediation rather than tuning text or broadening fixtures.
