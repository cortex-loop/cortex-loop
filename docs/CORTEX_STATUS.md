...
- Current tracked train: `codex-app-cli-cortex-value-ablation-audit`
- Active quality/risk focus: The current focus ran the Codex App/CLI value-ablation audit over existing live artifacts. Threshold replay showed all final Astro Stop rows had pressure 0.0, no active expectations, and one resolved verification expectation, so threshold tuning is not causal. Paydown ablation would catch all 3 hidden-failing full-Cortex Astro trials but would overblock 2 hidden-passing full-Cortex traces. Claim/evidence alignment found visible task-claim gaps without reading hidden verifier facts. This earns a decision to queue requirement-level perception, not behavior lift.
- Next product train after the current focus: `codex-app-cli-requirement-level-perception`
...
- Train: `codex-app-cli-requirement-level-perception`
- Surface: product
- Executive benefit: Add product-visible requirement-level claim/evidence perception so Cortex can distinguish generic verification-shaped activity from checks that actually address the model's closure claims before Stop.
...
- Primary metric: Gate 0 replay opens or preserves verification debt for visibly unmatched closure claims while suppressing clean/passing traces; live follow-up must beat raw and silent with actual block rows before any behavior-lift claim.
- Guardrail: Do not use hidden verifier facts, task identity, Astro-specific rules, fixture prompts, or generic paydown tightening that overblocks passing traces; hidden verifier facts remain scoring-only.
- Kill rule: If visible task obligations, closure claims, and observed checks cannot distinguish relevant verification from generic activity without overblocking clean/passing traces, stop and narrow the output-quality claim or move the question to a different actuator.
