...
Current train: `codex-app-cli-task-standard-behavior-comparison-live-run`. Next train: `codex-app-cli-task-standard-evidence-gating-remediation`. Shipping default: `openai.codex_app_cli`. Keep Cortex truth, brain-wiring truth, conformance truth, shipping truth, and live behavior-lift claims separate; structural proof alone does not earn model-output lift.
...
- Current tracked train: `codex-app-cli-task-standard-behavior-comparison-live-run`
- Active quality/risk focus: This product/lab seam ran the approved task-standard three-arm live comparison. The verdict is failure_overblock: active Cortex used captured TaskStandardSpine state and emitted real Stop block/continuation rows, but it also blocked two clean controls (`clean_verified_work` and `simple_success_file`) where raw and silent controls stayed unblocked. Behavior lift remains unearned, hidden scoring stayed scoring-only, root config stayed unchanged, no runtime snapshot loaded, and the next product train is task-standard evidence-gating remediation rather than text tuning, fixture repair, or another comparison.
- Next product train after the current focus: `codex-app-cli-task-standard-evidence-gating-remediation`
...
- Train: `codex-app-cli-task-standard-evidence-gating-remediation`
- Surface: product
- Executive benefit: Repair the task-standard evidence/gating relation so clean, adequately handled work stays silent while captured standards still block real closure gaps.
- Why now: The approved task-standard three-arm live comparison produced failure_overblock: active Cortex used captured TaskStandardSpine state and emitted real Stop block/continuation rows, but it also blocked clean controls where raw and silent arms stayed unblocked. Behavior lift remains forbidden until this gating calibration is fixed.
- Primary metric: Replay the live overblock rows and paired clean/mismatch trajectories. A valid remediation must keep real premature closure gaps blockable, keep honest blocker/waiting/narrowed-claim cases silent, and make the clean_verified_work and simple_success_file controls stay silent without using task identity, hidden verifier facts, fixture repair, or model-visible text edits.
- Guardrail: Do not edit the signed task-standard text, Stop text, SRE law, selector thresholds, hidden-verifier boundaries, root hook configuration, fixtures, or scoring. This train may only change task-agnostic standard/evidence alignment or gating consumption when live artifact replay proves a concrete overblock bug.
- Kill rule: If the overblock cannot be fixed without task-specific triggers, hidden scoring knowledge, broad text tuning, or weakening the standard/evidence law so real gaps underblock, stop for an architecture decision rather than running another behavior comparison.
