...
- Guardrail: no single-file, single-host, diagnostics-only, fixture-only, or short-run static proof may count as V2 communication closure; no paid service-lane command may run without explicit spend approval; closeout with `agent_loop_guard` may not set `allow_blocked` or opt out of full communication closure; conformance truth must keep Claude, Gemini, and reference in scope even while Claude/Codex CLI evidence is the immediate live-test focus; lab/live evidence stays watchlist unless a separate product seam earns it
...
- Work the active V2 full communication closure seam as a strict proof loop: inventory every Core/SRE/AUX packet row that must reach model-facing guidance, bind it to the shared executive guidance contract, prove Claude CLI and Codex CLI fixture rendering, require strict research review against partial/proxy communication, lock the S-tier live audit protocol with spend/no-spend authorization, expected 180-240 minute audit scope, per-row live artifact matrix, next-turn effect checks, and short-run anomaly handling, collect bounded live/watchlist evidence for Claude and Codex CLI surfaces, and keep closure claims forbidden until all loop gates pass with evidence.
