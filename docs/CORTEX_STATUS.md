...
- The brake tonic quiescence exit reconciliation seam is closed via SRE_2 §7.5 path B: doctrine is narrowed to threshold-hysteresis-only on the brake exit gate, and the rest-side EMA `tonic_quiescence` is retired from the BrakeTonic carrier, the SRE allocation diagnostics, and the host runtime serialization on all four host lanes (OpenAI, Claude, Gemini, reference); the locked `rho = 0.60` EMA regression is preserved, the quiet-productive-flow and sustained-calm decay unit tests stay green, no new memory carrier was introduced, and no host-specific exit policy fork was added.
