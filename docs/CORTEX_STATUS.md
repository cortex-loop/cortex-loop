...
- Current tracked train: `cortex-retained-spine-live-matrix-materialization-remediation`
- Active quality/risk focus: The no-live retained-spine materialization remediation repaired the evaluator mission metadata bug from `run_20260509T192719Z`: `simple_hook_baseline` rows now use `model_io_path=none_lab_proof_only` and `product_spine=[]`, while `lab_simple_hook_prompt_context` is preserved only as `support_model_io_path` lab support metadata. The active `cortex_active_policy` rows remain unchanged with `model_io_path=codex_hooks_UserPromptSubmit_Stop_hookSpecificOutput_or_block_stdout`, the retained `userpromptsubmit_stop_taskstandard_spine` product spine, and PostToolUse task-standard context disabled and role-demoted. Gate 0 command `python3 lab/cortex_effectiveness_evaluator.py --retained-spine-materialization-remediation-gate0 --require-pass` passed and wrote `materialization_repair_report.json`, `corrected_replay_report.json`, `gate0_report.json`, and `summary.json` under `.cortex/live_validation/cortex_retained_spine_live_matrix_materialization_remediation_gate0/`. The raw live artifact remains preserved as registered verdict `fail` with `mission_contract_error`; raw mission errors before repair were 15 and corrected mission errors after repair are 0. Corrected replay preserved 60 rows, all four arms, all five v2 task families, all three repeats, root config unchanged, no runtime snapshot, no hidden-verifier leakage, and no PostToolUse reactivation. The corrected replay verdict is `failure_silent_perception_contamination` with failure reason `silent_perception_beat_no_cortex`, so this seam earns materialization repair only, not retained-spine value, Cortex value, behavior lift, exactness value lift, broad Cortex lift, Codex App parity, shipping promotion, product progress, live-rerun permission, or AlphaEvolve candidate-evolution permission. The next train is `cortex-retained-spine-measurement-stack-remediation`.
- Next product train after the current focus: `cortex-retained-spine-measurement-stack-remediation`
...
- Train: `cortex-retained-spine-measurement-stack-remediation`
- Surface: no-live retained-spine measurement-stack remediation
- Executive benefit: Diagnose the corrected retained-spine replay from `run_20260509T192719Z`, where materialization repair removed mission-contract errors but exposed `failure_silent_perception_contamination`; determine why silent Cortex beat no-Cortex before any retained-spine live rerun, policy tuning, program database, candidate generation, or AlphaEvolve automation loop.
...
- Primary metric: No-live diagnosis classifies the corrected retained-spine replay by family and episode, preserves the raw `run_20260509T192719Z` failure, explains the silent-perception contamination without scoring/fixture/product-policy changes, and selects either measurement remediation, retained-spine rerun readiness, or contraction/rebuild pause.
- Guardrail: No live Codex run. No product behavior change. No product code deletion. Do not change product host behavior, model-visible Cortex text, evaluator scoring or fixtures to favor Cortex, hidden-verifier boundaries, root hooks, SRE law, active policy, PostToolUse policy or PostToolUse reactivation, or AlphaEvolve candidate policy. Do not reinterpret silent contamination as retained-spine value or no-value parity.
- Kill rule: Fail or stop if the seam reruns live, changes scoring or fixtures to favor Cortex, mutates product policy, reactivates PostToolUse as earned active policy, treats `failure_silent_perception_contamination` as Cortex value or retained-spine no-value parity, omits preservation of the raw `run_20260509T192719Z` failure, loads a runtime snapshot, mutates root config, or queues candidate evolution.
