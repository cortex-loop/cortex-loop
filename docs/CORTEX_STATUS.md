...
- Current tracked train: `cortex-executive-effectiveness-evaluator-gate0`
- Active quality/risk focus: The Cortex executive effectiveness evaluator Gate 0 passed as a no-live design gate. It defines the hard objective: Cortex earns value only when an active lifecycle policy improves next model behavior over both `no_cortex_baseline` and `simple_hook_baseline` without overcontrol or boundary failure. The design registers four arms (`no_cortex_baseline`, `simple_hook_baseline`, `cortex_silent_perception`, `cortex_active_policy`), five task families (exactness/evidence recovery, truthful closure, blocker surfacing, continuity after interruption, clean verified-work controls), active candidates (Stop-only, UserPromptSubmit+Stop, PostToolUse+Stop, lifecycle-composed policy, and PreToolUse later after host contract verification), dominance gates for overcontrol, repeated intervention loop, trace ambiguity, hidden-verifier leakage, root config mutation, runtime snapshot, simple-hook parity, and silent perception contamination, and contraction obligations for stale PostToolUse proof surfaces. The design is AlphaEvolve-inspired only at the methodology level: hard evaluator first, then evaluator build, then bounded candidate iteration later. No live Codex run occurred. No behavior lift, exactness value lift, broad Cortex lift, Codex App parity, shipping promotion, product host behavior change, model-visible text change, SRE law change, matcher threshold change, fixture scoring change, or AlphaEvolve mutation loop is earned or authorized.
