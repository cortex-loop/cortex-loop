...
- Current tracked train: `cortex-executive-effectiveness-evaluator-live-gate1`
- Active quality/risk focus: The Cortex executive effectiveness evaluator Live Gate 1 passed as a no-live lab/proof interface seam. `lab/cortex_effectiveness_evaluator.py --live-gate1 --require-pass` now writes `evaluator_design.json`, `live_plan.json`, `episode_table.jsonl`, `summary.json`, `leaderboard.json`, and `failure_analysis.json` under `.cortex/live_validation/cortex_effectiveness_evaluator_live_gate1/`. The dry-run plan schedules 60 rows: four arms (`no_cortex_baseline`, `simple_hook_baseline`, `cortex_silent_perception`, `cortex_active_policy`) across five task families and three repeats, with isolated workspaces, no root config mutation, no runtime snapshot, and mission objective contract fields on every row. The registered future live command is `CORTEX_CODEX_APP_CLI_EVALUATOR_LIVE_APPROVED=approved python3 lab/cortex_effectiveness_evaluator.py --live-matrix`; running `--live-matrix` without approval returns `not_run_approval_required`, and running it with approval remains deferred until `cortex-simple-hook-baseline-challenger` exists. Simple-hook parity and silent perception success remain no-value, dominance gates remain pre-scoring blockers, and positive value results still require user review. No live Codex run occurred; no product host behavior, model-visible text, SRE law, matcher threshold, fixture scoring, hidden scoring, root hook, or AlphaEvolve mutation loop changed.
- Next product train after the current focus: `cortex-simple-hook-baseline-challenger`
...
- Train: `cortex-simple-hook-baseline-challenger`
- Surface: no-live lab/proof evaluator baseline
- Executive benefit: Add the deliberately small independent simple-hook challenger so rich Cortex policies must beat a serious low-complexity baseline before any value claim.
...
- Primary metric: Implement a runnable simple-hook baseline under lab/evaluator ownership, target under 500 LOC, independent of cortex/core, cortex/sre, and cortex/aux, with only task-standard capture, one reminder/context path, and one closure check; prove it can be scheduled as the simple_hook_baseline arm without changing evaluator scoring or product behavior.
...
- Kill rule: Fail if the simple hook imports cortex/core, cortex/sre, or cortex/aux; exceeds the LOC target without explicit status justification; implements Cortex scoring lattice, AUX memory, Core commitment law, or multi-hook policy search; changes evaluator scoring/fixtures; starts a live run; or authorizes AlphaEvolve-style mutation before the first real four-arm matrix is ready.
