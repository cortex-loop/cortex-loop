...
Current train: `codex-app-cli-hook-native-stop-activation-probe`. Next train: `codex-app-cli-hook-native-stop-live-canary`. Shipping default: `openai.codex_app_cli`. Keep Cortex truth, brain-wiring truth, conformance truth, shipping truth, and live behavior-lift claims separate; structural proof alone does not earn model-output lift.
...
- Current tracked train: `codex-app-cli-hook-native-stop-activation-probe`
- Active quality/risk focus: The current focus added the isolated OpenAI Codex App/CLI product Stop-hook client and Gate 0 activation harness: seam-1 identity text drift is corrected, runtime snapshots decode product-shaped expectation state, simulated Stop payloads return exact block JSON through the product coordinator, silence/fail-open cases are captured, and root repo Mission Reflection guardrails remain untouched. Live canary and behavior lift are not earned.
- Next product train after the current focus: `codex-app-cli-hook-native-stop-live-canary`
...
- Train: `codex-app-cli-hook-native-stop-live-canary`
- Surface: product
- Executive benefit: Run an explicitly approved isolated live Stop canary so Cortex can prove whether the Codex App/CLI product hook client receives real host lifecycle payloads and returns model-visible block JSON through the native hook actuator.
- Why now: The structural Gate 0 activation probe proves client/coordinator enactment without touching root hook config; the next highest-leverage move is the narrow live canary that separates Codex App and Codex CLI actuator evidence before broader lifecycle wiring.
- Primary metric: A fresh isolated subject run observes a real Stop hook payload, records exact rendered-text hash and block JSON, captures continuation evidence or a scoped hook-load negative, and shows title/clean-control events are not falsely blocked.
- Guardrail: Do not set live spend approval automatically, do not reuse repo Mission Reflection hooks, do not generalize Codex App evidence to Codex CLI, and do not treat the generic runtime snapshot as product perception or behavior lift.
- Kill rule: Stop if live activation requires root hook config changes, if Stop block text fires without transcript-backed prior act, if Codex CLI does not load project hooks and the finding is not scoped, or if model-visible text differs from the product renderer output.
