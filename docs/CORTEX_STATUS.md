...
Current train: `codex-app-cli-hook-native-behavior-comparison-live-run`. Next train: `codex-app-cli-behavior-fixture-refresh`. Shipping default: `openai.codex_app_cli`. Keep Cortex truth, brain-wiring truth, conformance truth, shipping truth, and live behavior-lift claims separate; structural proof alone does not earn model-output lift.
...
- Current tracked train: `codex-app-cli-hook-native-behavior-comparison-live-run`
- Active quality/risk focus: The current focus ran the approved Codex CLI hook-native behavior comparison baseline gate after Gate 0 passed. The live run used product lifecycle perception with no runtime snapshot and unchanged root guardrails, but neither primary family reproduced baseline failure strongly enough for paired comparison: truth-gap reproduced 0/3 and output-quality reproduced 1/3. This earns fixture-strength evidence only, not behavior lift or architecture failure.
- Next product train after the current focus: `codex-app-cli-behavior-fixture-refresh`
...
- Train: `codex-app-cli-behavior-fixture-refresh`
- Surface: product
- Executive benefit: Refresh or replace the Codex CLI behavior-comparison fixtures so at least one primary family reproduces a baseline failure strongly enough to test Cortex lift.
- Why now: The approved live behavior-comparison baseline gate ran, but truth-gap reproduced 0/3 failures and output-quality reproduced 1/3, so no paired comparison could honestly test Cortex lift.
- Primary metric: At least one primary behavior family reproduces a silent-only baseline failure in 2 of 3 trials while product perception remains snapshot-free and hidden verifier facts remain scoring-only.
- Guardrail: Do not tune Cortex speech, selector law, renderer text, product perception, hidden-verifier perception, task identity, runtime snapshots, or fixture continuation prompts to force a result.
- Kill rule: If no primary family can reproduce a 2/3 silent-only baseline failure without lab leakage, stop and run an architecture/evaluation decision turn before more implementation.
