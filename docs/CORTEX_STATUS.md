...
- Current tracked train: `cortex-executive-effectiveness-evaluator-build`
- Active quality/risk focus: The Cortex executive effectiveness evaluator build passed as a no-live lab/proof seam. The dedicated `lab/cortex_effectiveness_evaluator.py --build --require-pass` path now emits `evaluator_design.json`, `episode_table.jsonl`, `summary.json`, and `leaderboard.json` under `.cortex/live_validation/cortex_effectiveness_evaluator_build/`. The episode table preserves all four arms (`no_cortex_baseline`, `simple_hook_baseline`, `cortex_silent_perception`, `cortex_active_policy`), proves simple-hook parity and silent perception success are no-value, applies dominance gates before value scoring, and preserves the historical PostToolUse paired-value artifact `task_standard_posttooluse_paired_value_live_20260508T120907Z` as `failure_no_value` without counting it as a new live run. The overnight runner now records cycle state, enforces the 22:00-07:00 window, emits allowed command lists, blocks repeated clean-main no-op cycles, requires exact registered live command/env pairs, budgets non-test LOC outside evaluator-build seams, and requires contraction candidates when policy/lab LOC grows outside evaluator-build. No live Codex run occurred; no product host behavior, model-visible text, SRE law, matcher threshold, fixture scoring, hidden scoring, root hook, or AlphaEvolve mutation loop changed.
...
- Primary metric: Implement a no-live Gate 1 interface that registers exact future live command/env pairs, refuses without approval, proves the four required arms and task families can be scheduled, writes a dry-run live plan without running Codex, and keeps simple-hook parity, silent contamination, overcontrol, trace ambiguity, root/runtime mutation, and hidden-verifier leakage as dominance gates.
...
- Kill rule: Fail if the interface omits any required arm, lacks the simple-hook challenger, registers a live command without exact env approval requirements, starts a live run, weakens dominance gates, changes fixtures/scoring, or authorizes AlphaEvolve-style mutation before live Gate 1 proof passes.
