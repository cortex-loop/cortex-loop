...
- Current tracked train: `codex-app-cli-posttooluse-causal-trace-ids`
- Active quality/risk focus: The Codex App/CLI PostToolUse causal-trace seam passed no-live Gate 0 by replacing ordinal hook-row-to-terminal-command interpretation with stable tool-event references for future live readout. Product hook diagnostics now preserve `tool_use_id`; stdout-derived command records preserve command item ids as `tool_event_ref`; and `_posttooluse_context_trace(...)` joins a context-emitting PostToolUse row to the stdout command record only through that reference. A synthetic event-ref trace is non-ambiguous, identifies `printf 'alpha beta omega' > exact_result.txt` candidate artifact creation as the context source command, and identifies `od -An -t x1 -v exact_result.txt` as the later next action. Replay of historical artifact `task_standard_posttooluse_live_20260507T153242Z` is now marked ambiguous because the stored rows lack `tool_use_id`; the harness no longer infers preceding or next tools by ordinal position. This preserves the earlier `cortex/hosts/openai/posttooluse_task_standard_actuator.py` extraction and hook chronology repair for the PostToolUse task-standard actuator, including the `failure_overcontrol` / `clean_evidenced` overcontrol evidence boundary, the firing-boundary remediation, the live-equivalent phase-aware PostToolUse completion predicate, exit/status marker coverage, private `pre_artifact_candidate_missing` and `phase_check_failed` silence, the live-equivalent `cat: illegal option -- A` / `usage:` failed-check control, and the `od -An -t x1 -v exact_result.txt` direct-check readout, without claiming live clean-control safety. Existing failed-check, missing-artifact, markerless, clean/control, blocker, waiting, unrelated, mismatch candidate/readback, session cap, and dedup outcomes stay unchanged; no runtime snapshot, Stop block, PreToolUse denial, PermissionRequest policy, hidden scoring perception, or transport path appeared; signed UserPromptSubmit text, PostToolUse text, Stop text, SRE law, scored matcher, thresholds, fixtures, scoring, root hooks, hidden-verifier boundaries, Sinkhorn/transport, `behavior_lift_claim_allowed`, no broad Cortex lift, and no live behavior lift status were unchanged. The phase-aware narrow live rerun remains blocked until shared tool-evidence classification also lands.
- Next product train after the current focus: `codex-app-cli-posttooluse-shared-tool-evidence-classification`
...
- Train: `codex-app-cli-posttooluse-shared-tool-evidence-classification`
- Surface: product host actuator plus SRE substrate proof
- Executive benefit: Make the Codex PostToolUse task-standard actuator and host-agnostic SRE task-standard evidence path consume one typed tool-evidence classification so a future live rerun is not interpreting candidate/readback/failure phases through divergent marker lists; this follows firing-boundary remediation and keeps the later requirement that clean-evidenced controls stay silent.
...
- Primary metric: A shared SRE-owned typed classifier represents missing artifact, failed check, failed tool, markerless/generic, candidate artifact creation, and readback phases; the Codex PostToolUse actuator and SRE task-standard evidence recording import it; existing product and lab cases keep the same silence/context/evidence-class outcomes, including clean evidenced controls.
- Guardrail: No live Codex run and no `--require-pass` live command in this seam; the later live rerun still requires explicit current-turn approval and explicit current-turn live approval. Must not change signed UserPromptSubmit text, PostToolUse text, Stop text, SRE law, scored matcher threshold, fixtures, scoring semantics, root hooks, hidden-verifier boundary, Sinkhorn/transport, PreToolUse denial, PermissionRequest policy, or three-arm behavior comparison state. Preserve existing overcontrol, firing-boundary, phase-aware, and causal-trace Gate 0 outcomes. Behavior lift remains unearned and the phase-aware narrow live rerun stays blocked until this shared classifier passes no-live proof.
- Kill rule: If the shared classifier changes clean/control rows silent, mismatch context emission, firing-boundary behavior, task-standard evidence class assignment, context text, matcher threshold behavior, or any Stop/PreToolUse/PermissionRequest behavior, stop for architecture decision rather than tuning markers locally. If it passes, queue the approval-gated phase-aware narrow live rerun; if it fails, queue the matching classifier-boundary remediation.
