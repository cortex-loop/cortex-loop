...
- Current tracked train: `debt-drag-into-brake-and-route`
- Active quality/risk focus: Roadmap Seam 4 is active: Cortex is wiring resolution deficit and persistence-weighted goal debt into silent route/brake control so unsupported forward motion becomes more guarded while inspection, verification, asking, and blocker-surfacing remain available. This train changes structural runtime control only: debt can raise tonic guardedness and route policy diagnostics, but debt alone must not latch, block, alter Claude hooks, emit model-visible text, invoke AUX publication, or change shipping truth. Truth earned by this train is Cortex/conformance structural truth only; live model behavior lift remains unearned until the OpenAI silent-control probe compares baseline, shaped, and clean controls.
