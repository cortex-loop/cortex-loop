...
- Current tracked train: `cortex-stop-only-retained-spine-gate0`
- Active quality/risk focus: The Stop-only retained spine Gate 0 recorded verdict `pass_cortex_stop_only_retained_spine_gate0` in `docs/recon/cortex_stop_only_retained_spine_gate0.md` and wrote `.cortex/live_validation/cortex_stop_only_retained_spine_gate0/stop_only_spine_contract.json`, `gate0_report.json`, and `summary.json`. The active policy candidate is `stop_only_closure_continuation_spine`; the only active product model-I/O path is Codex Stop `hookSpecificOutput` or block stdout continuation. UserPromptSubmit `hookSpecificOutput.additionalContext`, PostToolUse task-standard context, PreToolUse denial, PermissionRequest policy, runtime snapshots, and AlphaEvolve candidate mutation are explicitly excluded from active value. Allowed support law is non-model-visible only: `TaskStandardSpine`, `cortex/sre/tool_evidence.py`, transcript/tool evidence, and closure-claim state. Evidence preserved: `run_20260509T112542Z` `failure_no_value`, `run_20260509T192719Z` raw/corrected retained-spine evidence, `run_20260510T122608Z` `pass_clean_control_stable`, `decision_contract_retained_spine_to_stop_only`, prior Stop-gating `pass_gating_observed`, and `task_standard_posttooluse_paired_value_live_20260508T120907Z` PostToolUse paired `failure_no_value`. This earns Stop-only contract readiness only: no Cortex value, no retained-spine value, no behavior lift, no product progress, no shipping promotion, no PostToolUse reactivation, no UserPromptSubmit active value reactivation, and no AlphaEvolve candidate-evolution permission. The next train is `cortex-stop-only-retained-spine-live-gate1`.
- Next product train after the current focus: `cortex-stop-only-retained-spine-live-gate1`
...
- Train: `cortex-stop-only-retained-spine-live-gate1`
- Surface: no-live Stop-only retained-spine live-matrix interface gate
- Executive benefit: Wire the Stop-only retained spine into a no-live dry-run future matrix before any live run: prove `stop_only_closure_continuation_spine` can be scheduled against no-Cortex, simple-hook, and silent controls with Codex Stop as the only active product model-I/O path.
...
- Primary metric: No-live Gate 1 writes a dry-run four-arm Stop-only plan, preserves the Stop-only contract, proves active rows use only Stop closure/continuation model I/O, and registers approval refusal for a later live command.
- Guardrail: No live Codex run; no exact registered command/env pair may be executed in this Gate 1; no product deletion; no product code deletion. No product behavior change. Do not change product host behavior, model-visible Cortex text, evaluator scoring or fixtures, hidden-verifier boundary, root hook, SRE law, UserPromptSubmit active value policy, PostToolUse policy, PreToolUse denial, PermissionRequest policy, runtime snapshot dependency, or AlphaEvolve candidate evolution. Gate 1 may register a future approval-gated live command shape only; it must not execute it or claim value.
- Kill rule: Fail or stop if the seam runs live, lets active rows use UserPromptSubmit additionalContext or PostToolUse context, changes scoring or fixtures, treats Stop-only as value without live proof, reactivates PostToolUse, queues candidate evolution, or omits the Stop-only Gate 0 contract.
