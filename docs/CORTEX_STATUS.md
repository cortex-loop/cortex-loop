...
- Current tracked train: `codex-app-cli-posttooluse-task-standard-phase-aware-narrow-live-rerun`
- Active quality/risk focus: The approved Codex App/CLI PostToolUse phase-aware narrow live rerun after shared tool-evidence classification produced `fail` with failure reason `repeated_posttooluse_context_loop`: the mismatch case captured three task-standard items, observed PostToolUse lifecycle rows, and observed candidate artifact prerequisite work, but emitted three Codex-native PostToolUse `additionalContext` rows. Rows 10 and 11 repeated the same `task-standard:work_standard:929237a14cf4d738` item, row 13 emitted `task-standard:closure_evidence:7608236188fd9be6`, and only row 16 stayed silent with `posttooluse_context_session_cap_reached`. Clean evidenced, honest blocker, waiting-on-user, and unrelated-tool controls received zero PostToolUse contexts, root config stayed unchanged, no runtime snapshot loaded, hidden scoring stayed absent/scoring-only, and `behavior_lift_claim_allowed=false`. The mismatch trace was also not interpretable as next-action evidence because `posttooluse_context_trace_ambiguous=true` with `posttooluse_tool_event_ref_not_found_in_stdout`: hook rows carried `call_*` tool ids while stdout-derived command rows carried `item_*` refs, so the harness correctly refused ordinal inference. This earns no behavior lift, no exactness value lift, no PostToolUse next-action effect claim, no clean-control safety promotion beyond this failed run, no Codex App parity, and no shipping promotion. It queues no-live context-loop and live-trace remediation before any further live probe, without changing signed UserPromptSubmit text, PostToolUse text, Stop text, SRE law, matcher thresholds, fixtures, scoring semantics, root hooks, hidden-verifier boundaries, Sinkhorn/transport, PreToolUse denial, PermissionRequest policy, output-law centralization, typed intervention pressure, or host-runtime extraction. Historical context retained for boundary checks: the prior shared tool-evidence classification seam used `cortex/sre/tool_evidence.py`, `ToolEvidenceObservation`, and `ToolEvidencePhase`; it preserved status-only handling, prior causal-trace Gate 0, candidate artifact creation, readback, firing-boundary remediation, live-equivalent phase-aware PostToolUse completion predicate, `phase_check_failed`, `cat: illegal option -- A`, `pre_artifact_candidate_missing`, exit/status marker handling, and no-live behavior-lift boundaries. No live Codex run occurred in that prior Gate 0 seam; this current seam did run live and failed. Boundary-test carry-forward terms: PostToolUse task-standard, no broad Cortex lift, no live behavior lift, `failure_overcontrol`, prior pass, signed UserPromptSubmit text. Additional current-run anchors: clean_evidenced control stayed silent in this run, task_standard_exactness remains the only earned surface, and the previous live failures included candidate_artifact_without_posttooluse_context. Missing-artifact timing remains part of the carried phase-aware evidence.
- Next product train after the current focus: `codex-app-cli-posttooluse-task-standard-context-loop-trace-remediation`
...
- Train: `codex-app-cli-posttooluse-task-standard-context-loop-trace-remediation`
- Surface: product host actuator plus lab trace no-live remediation
- Executive benefit: Repair the two blockers surfaced by `task_standard_posttooluse_live_20260507T213732Z`: prevent repeated PostToolUse task-standard context loops under live-equivalent timing and make hook/stdout event-ref joining non-ambiguous before any next-action interpretation. The surface remains `task_standard_exactness` / evidence recovery. This is required before a rerun can be interpreted. It preserves the firing-boundary remediation while fixing the later loop/trace blockers.
...
- Primary metric: A no-live Gate 0 proves that a live-equivalent mismatch can emit at most one PostToolUse context for the same unresolved task-standard obligation, session cap/deduplication hold across consecutive PostToolUse rows, clean/control rows remain silent, and hook rows join to stdout-derived command rows by a stable reference or are marked ambiguous without ordinal inference. Candidate artifact creation and readback rows remain the exact live-equivalent timing surface under test. `behavior_lift_claim_allowed` remains false.
- Guardrail: No live Codex run occurs in this remediation seam. Do not change signed UserPromptSubmit text, PostToolUse text, Stop text, SRE law, scored matcher threshold, fixtures, scoring semantics, root hooks, hidden-verifier boundary, Sinkhorn/transport, PreToolUse denial, PermissionRequest policy, output-law centralization, typed intervention pressure, or host-runtime extraction. Under-fire remains preferable to context loops or clean-control overcontrol. Any later live rerun still requires explicit current-turn approval and explicit current-turn live approval, must not use `--require-pass`, and remains not a three-arm behavior comparison. The seam must not change those protected surfaces.
- Kill rule: If no-live context-loop and trace remediation passes, queue the approval-gated phase-aware narrow live rerun again. If repeated context, ambiguous live-equivalent trace, clean/control context, boundary-breach text, runtime snapshot, root mutation, or policy/text/scoring drift appears, queue the matching architecture/remediation decision and do not run live. Passing proof must preserve clean/control rows silent before any later live rerun. Firing-boundary regressions fail the remediation. Boundary failures queue an architecture decision before more tuning.
