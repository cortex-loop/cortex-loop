...
- Current tracked train: `cortex-retained-active-policy-spine-gate0`
- Active quality/risk focus: The no-live retained-spine Gate 0 seam produced `pass_cortex_retained_active_policy_spine_gate0` and wrote `.cortex/live_validation/cortex_retained_active_policy_spine_gate0/retained_spine_contract.json`, `gate0_report.json`, and `summary.json`. It defines the retained spine as `userpromptsubmit_stop_taskstandard_spine`: `UserPromptSubmit task-standard formation`, `Stop closure/continuation gate`, `TaskStandardSpine` state law, and shared `SRE tool-evidence classifier`. The owner modules are `cortex/hosts/openai/codex_app_cli_hook_coordinator.py`, `cortex/hosts/openai/codex_app_cli_hook_client.py`, `cortex/sre/task_standard.py`, and `cortex/sre/tool_evidence.py`. Product model-I/O paths are named for Codex `UserPromptSubmit` `hookSpecificOutput.additionalContext` and Codex `Stop` `hookSpecificOutput` or block stdout continuation; state/classifier paths are explicitly no-direct-model-I/O support law that reaches the model only through host decisions. The seam preserves source decisions `docs/recon/cortex_active_policy_contraction_decision.md`, `docs/recon/cortex_posttooluse_proof_surface_role_demotion.md`, v1 artifact `run_20260508T221352Z` (`failure_silent_perception_contamination`) without rescoring, v2 artifact `run_20260509T112542Z` (`failure_no_value`, `60 live rows`, all arms tied, active `family_wins=0`, no dominance boundary failure), and PostToolUse paired-value artifact `task_standard_posttooluse_paired_value_live_20260508T120907Z` (`failure_no_value`, active PostToolUse beat silent in `0/5` exactness mismatch pairs). PostToolUse task-standard context remains `role_demoted_non_current_support_history` and is not reactivated as current strategy, product progress, or earned active policy. Simple-hook parity and silent perception success continue to block value. This seam did not run live, delete product code, change product host behavior, model-visible Cortex text, evaluator scoring, fixtures, hidden-verifier boundaries, root hooks, SRE law, active policy, or candidate policy. It does not earn Cortex value, behavior lift, exactness value lift, broad Cortex lift, Codex App parity, shipping promotion, product progress, or AlphaEvolve candidate-evolution permission. The next train is `cortex-retained-active-policy-spine-live-gate1`.
- Next product train after the current focus: `cortex-retained-active-policy-spine-live-gate1`
...
- Train: `cortex-retained-active-policy-spine-live-gate1`
- Surface: no-live retained-spine evaluator dry-run gate
- Executive benefit: Wire the retained UserPromptSubmit + Stop + TaskStandardSpine spine into a no-live four-arm evaluator dry-run before any live retained-spine run, program database, bounded candidate generation, or AlphaEvolve automation loop.
...
- Primary metric: Produce a no-live dry-run matrix plan and artifact set for `userpromptsubmit_stop_taskstandard_spine` with four arms, simple-hook parity blocking, silent-perception contamination blocking, exact future live command/env registration, approval refusal, and no Cortex value or candidate-evolution permission.
- Guardrail: No live Codex run, no product code deletion, no product behavior change, no product host behavior change, no model-visible Cortex text change, no evaluator scoring or fixtures change, no hidden-verifier boundary change, no root hook change, no SRE law change, no active policy mutation, no PostToolUse reactivation, and no AlphaEvolve candidate policy. Do not change product behavior, model-visible Cortex text, evaluator scoring or fixtures, hidden-verifier boundaries, root hooks, SRE law, active policy, or candidate policy. Do not claim Cortex value, behavior lift, exactness value lift, broad Cortex lift, Codex App parity, shipping promotion, product progress, or candidate-evolution permission.
- Kill rule: Fail or stop if the seam runs live, reactivates PostToolUse task-standard context as earned active policy, changes scoring or fixtures to favor Cortex, treats retained-spine dry-run proof as value lift, omits the simple-hook baseline or silent negative control, queues candidate evolution, or cannot register an exact future live command/env pair.
