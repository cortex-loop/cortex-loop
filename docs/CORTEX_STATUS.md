...
- Current tracked train: `cortex-automation-product-boundary-contract`
- Active quality/risk focus: The Cortex automation product-boundary contract passed as a no-live support-surface hardening seam. `lab/cortex_effectiveness_evaluator.py` now requires evaluator design and episode rows to carry a mission objective contract with `executive_function`, `loop_stage`, `control_mode`, `truth_scope`, `model_io_path`, `product_spine`, and `contraction_implication`. Lab-only rows with `model_io_path=none_lab_proof_only` cannot claim product progress, Cortex value, behavior lift, exactness value lift, broad Cortex lift, Codex App parity, or shipping promotion; product/value claims require a non-empty model-I/O path and product spine. Simple-hook parity and silent perception success remain no-value. `internal/automation/cortex_overnight_loop.py` now treats lab/eval/workflow changes as support, blocks `cortex/**` candidate changes without product spine, model-I/O path, and current-truth authorization, blocks candidate mutation of scoring/fixtures/hidden verifier/Core law/packet law/workflow gates/generated product docs, forces user review for structured positive value/shipping fields, and writes morning digest fields for executive function, loop stage, model-I/O path, simple-hook result, and contraction implication. No live Codex run occurred; no product host behavior, model-visible text, SRE law, matcher threshold, fixture scoring, hidden scoring, root hook, or AlphaEvolve mutation loop changed.
- Next product train after the current focus: `cortex-executive-effectiveness-evaluator-live-gate1`
...
- Train: `cortex-executive-effectiveness-evaluator-live-gate1`
- Surface: no-live lab/proof evaluator live-interface gate
- Executive benefit: Add the approval-gated live evaluator interface that can later run the four-arm Cortex effectiveness matrix without changing product policy or starting candidate mutation.
...
- Primary metric: Implement a no-live Gate 1 interface that registers exact future live command/env pairs, refuses without approval, proves the four required arms and task families can be scheduled, writes a dry-run live plan without running Codex, preserves the mission objective contract on future episode rows, and keeps simple-hook parity, silent contamination, overcontrol, trace ambiguity, root/runtime mutation, and hidden-verifier leakage as dominance gates.
- Guardrail: No live Codex run in this seam. No product host behavior change. No candidate mutation. Do not change signed UserPromptSubmit text, PostToolUse text, Stop text, SRE law, matcher thresholds, fixture scoring, root hooks, hidden-verifier boundary, Sinkhorn/transport, PreToolUse denial, PermissionRequest policy, output-law centralization, typed intervention pressure, or host-runtime extraction.
- Kill rule: Fail if the interface omits any required arm, lacks the simple-hook challenger, registers a live command without exact env approval requirements, starts a live run, weakens dominance gates, drops the mission objective/product-boundary contract, changes fixtures/scoring, or authorizes AlphaEvolve-style mutation before live Gate 1 proof passes.
