...
- Current tracked train: `cortex-simple-hook-baseline-challenger`
- Active quality/risk focus: The Cortex simple-hook baseline challenger passed as a no-live lab/proof baseline seam. `lab/cortex_simple_hook_baseline.py` now provides a deliberately small independent `simple_hook_baseline`: visible-task standard capture, one model-visible reminder/context renderer, and one closure check that accepts explicit evidence or blocker reporting while rejecting unsupported closure. The source is under the 500 nonblank noncomment LOC target, imports no `cortex/**` modules, uses no Core/SRE/AUX law, no Cortex scoring lattice, no AUX memory, no hidden verifier, no fixture-specific scoring, and no multi-hook policy search. `lab/cortex_effectiveness_evaluator.py --simple-hook-baseline-gate0 --require-pass` writes `simple_hook_baseline.json`, `gate0_report.json`, and `summary.json` under `.cortex/live_validation/cortex_simple_hook_baseline_challenger/`; the report proves the baseline is runnable as the `simple_hook_baseline` arm without changing evaluator scoring or product behavior. `--live-matrix` remains approval-gated and still does not run live inside this seam; with approval it now defers to `cortex-executive-effectiveness-evaluator-live-matrix-run` rather than missing-baseline remediation. Simple-hook parity and silent perception success remain no-value, dominance gates remain pre-scoring blockers, and positive value results still require user review. No live Codex run occurred; no product host behavior, model-visible Cortex text, SRE law, matcher threshold, fixture scoring, hidden scoring, root hook, or AlphaEvolve mutation loop changed.
- Next product train after the current focus: `cortex-executive-effectiveness-evaluator-live-matrix-run`
...
- Train: `cortex-executive-effectiveness-evaluator-live-matrix-run`
- Surface: approval-gated lab/proof evaluator live matrix
- Executive benefit: Run the first real four-arm Cortex effectiveness matrix now that the simple-hook challenger exists, so Cortex is judged against no-Cortex, simple-hook, silent-perception, and active-policy arms before any candidate evolution.
...
- Primary metric: Run the exact registered evaluator live matrix command/env and produce `episode_table.jsonl`, `summary.json`, `leaderboard.json`, and `failure_analysis.json` for no-Cortex, simple-hook, silent-Cortex, and active-Cortex arms across the registered task families/repeats. Value is possible only if active Cortex beats the simple hook with no dominance-gate failure; simple-hook parity and silent success remain no-value.
- Guardrail: Live Codex is allowed only through the exact registered evaluator command/env. Do not change product host behavior, signed UserPromptSubmit text, PostToolUse text, Stop text, candidate policy, evaluator scoring, fixtures, hidden-verifier boundaries, root hooks, SRE law, matcher threshold, Sinkhorn/transport, PreToolUse denial, PermissionRequest policy, output-law centralization, typed-intervention, or host-runtime extraction.
- Kill rule: Fail if any arm is missing, simple-hook parity is credited as Cortex value, silent perception success is credited as Cortex value, clean/control rows overcontrol, trace ambiguity appears, hidden-verifier leakage appears, root config mutates, runtime snapshot loads, or a positive value/lift/shipping claim is auto-merged without user review.
