...
- Current tracked train: `cortex-stop-only-retained-spine-live-gate1`
- Active quality/risk focus: The Stop-only retained spine live Gate 1 recorded verdict `pass_cortex_stop_only_retained_spine_live_gate1` in `docs/recon/cortex_stop_only_retained_spine_live_gate1.md` and wrote `.cortex/live_validation/cortex_stop_only_retained_spine_live_gate1/stop_only_spine_contract.json`, `evaluator_design.json`, `v2_case_registry.json`, `live_plan.json`, `episode_table.jsonl`, `summary.json`, `leaderboard.json`, `failure_analysis.json`, and `registered_live_command.json`. The dry-run plan has 60 rows: four arms x five v2 task families x three repeats. Active rows use `stop_only_closure_continuation_spine` only, with Codex Stop `hookSpecificOutput` or block stdout continuation as the only active product model-I/O path. UserPromptSubmit `hookSpecificOutput.additionalContext` and PostToolUse task-standard context are disabled as active paths; non-active rows use `model_io_path=none_lab_proof_only`, simple-hook prompt context remains support metadata, and silent rows emit no model-visible Cortex output. The future live command is `python3 lab/cortex_effectiveness_evaluator.py --stop-only-retained-spine-live-matrix` with env `CORTEX_CODEX_APP_CLI_STOP_ONLY_RETAINED_SPINE_LIVE_APPROVED=approved`; without approval it returns `not_run_approval_required`, and with approval in this seam it returns `not_run_registered_future_live_only`. Simple-hook parity and silent success still block value. This earns Stop-only dry-run readiness only: no Cortex value, no retained-spine value, no behavior lift, no product progress, no shipping promotion, no PostToolUse reactivation, no UserPromptSubmit active value reactivation, and no AlphaEvolve candidate-evolution permission. The next train is `cortex-stop-only-retained-spine-live-run`.
- Next product train after the current focus: `cortex-stop-only-retained-spine-live-run`
...
- Train: `cortex-stop-only-retained-spine-live-run`
- Surface: approval-gated Stop-only retained-spine live matrix
- Executive benefit: Run the registered Stop-only retained spine live matrix after explicit approval: test whether `stop_only_closure_continuation_spine` beats no-Cortex, simple-hook, and silent controls with Codex Stop as the only active product model-I/O path.
...
- Primary metric: Approval-gated Stop-only live run writes a 60-row artifact, preserves active rows as Stop-only, keeps UserPromptSubmit/PostToolUse excluded, and records whether active Stop-only beats no-Cortex and simple-hook without silent contamination or boundary dominance.
- Guardrail: Live Codex is allowed only through the exact registered command/env pair and only with explicit current-turn approval. No product deletion; no product code deletion. No product behavior change. Do not change product host behavior, model-visible Cortex text, evaluator scoring or fixtures, hidden-verifier boundary, root hook, SRE law, UserPromptSubmit active value policy, PostToolUse policy, PreToolUse denial, PermissionRequest policy, runtime snapshot dependency, or AlphaEvolve candidate evolution. Positive value evidence requires user review and cannot become broad Cortex lift or shipping truth.
- Kill rule: Fail or stop if the seam runs a command other than the registered Stop-only live command, lets active rows use UserPromptSubmit additionalContext or PostToolUse context, changes scoring or fixtures, treats Stop-only as value without live proof, reactivates PostToolUse, queues candidate evolution, omits Gate 1 artifacts, or reports broad lift/product progress.
