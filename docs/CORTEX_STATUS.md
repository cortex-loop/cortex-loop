...
- Current tracked train: `cortex-effectiveness-strategy-reset`
- Active quality/risk focus: The Cortex effectiveness strategy reset records that the PostToolUse-only task-standard value path should stop before any further hook-local remediation or same-shape live rerun. The approval-gated paired live probe `task_standard_posttooluse_paired_value_live_20260508T120907Z` produced `failure_no_value`: active PostToolUse context beat silent in 0/5 mismatch pairs against the 4/5 threshold, with repeats 2-4 as `tie_no_value` and repeats 1 and 5 as active `failure_no_context`. This shows improved proof visibility but no measured effectiveness. The reset classifies the recent line as not yet AlphaEvolve-style optimization because the hard objective and paired evaluator arrived late, only one hook policy was defended, and substantial proof machinery accumulated before value was tested. No behavior lift, exactness value lift, broad Cortex lift, Codex App parity, or shipping promotion is earned. PostToolUse remains feasibility evidence only; the next train is a no-live Cortex executive effectiveness evaluator Gate 0 with baseline/silent/active lifecycle policy candidates, hard dominance gates, and contraction obligations for stale PostToolUse proof paths.
- Next product train after the current focus: `cortex-executive-effectiveness-evaluator-gate0`
...
- Train: `cortex-executive-effectiveness-evaluator-gate0`
- Surface: no-live evaluator architecture gate
- Executive benefit: Regroup around Cortex's actual product objective by defining a hard paired evaluator for lifecycle executive effectiveness across candidate policies, instead of continuing the PostToolUse-only value path.
...
- Primary metric: Gate 0 passes only if it defines a hard Cortex effectiveness score, registers baseline/silent/active lifecycle policy candidates, preserves the negative PostToolUse paired result as no-value evidence, names allowed arm deltas, and includes automatic dominance gates for overcontrol, repeated intervention, trace ambiguity, root config mutation, runtime snapshots, hidden verifier leakage, and silent-baseline success. It must treat silent success as no Cortex value and must include a contraction rule for retiring or role-demoting stale PostToolUse proof paths as the new evaluator becomes owner.
- Guardrail: No live Codex run. No product host behavior change. No PostToolUse text or policy tuning. No same-shape PostToolUse live rerun. Do not change signed UserPromptSubmit text, PostToolUse text, Stop text, SRE law, matcher thresholds, fixture scoring, root hooks, hidden-verifier boundary, Sinkhorn/transport, PreToolUse denial, PermissionRequest policy, output-law centralization, typed intervention pressure, or host-runtime extraction.
- Kill rule: Fail the seam if it merely renames the PostToolUse value probe, proposes another same-shape rerun, lacks baseline/silent controls, lacks multiple lifecycle policy candidates, gives credit when silent also succeeds, omits overcontrol/root/runtime/hidden-boundary dominance, or does not specify which stale PostToolUse proof surfaces will be retired, archived, or role-demoted.
