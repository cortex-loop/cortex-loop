...
- Current tracked train: `cortex-posttooluse-proof-surface-role-demotion`
- Active quality/risk focus: The no-live role-demotion seam implemented `decision_role_demote_posttooluse_specific_proof_train` from the contraction decision `decision_contract_posttooluse_proof_surfaces`. It preserved v2 artifact `run_20260509T112542Z` (`failure_no_value`, `60 live rows`, all arms tied, active `family_wins=0`, no dominance boundary failure), preserves v1 artifact `run_20260508T221352Z` (`failure_silent_perception_contamination`) without rescoring, and preserved the PostToolUse paired-value artifact `task_standard_posttooluse_paired_value_live_20260508T120907Z` (`failure_no_value`, active PostToolUse beat silent in `0/5` exactness mismatch pairs). It also preserves the five earlier PostToolUse live artifacts: `task_standard_posttooluse_live_20260507T100836Z`, `task_standard_posttooluse_live_20260507T142129Z`, `task_standard_posttooluse_live_20260507T153242Z`, `task_standard_posttooluse_live_20260507T213732Z`, and `task_standard_posttooluse_live_20260507T225019Z`. The role-demotion table records that the PostToolUse-specific Gate 0/live proof train is non-current support/history: PostToolUse-specific Gate 0 modes, narrow live/rerun modes, paired-value live mode, and repair recons are non-current support/history; `lab/cortex_effectiveness_evaluator.py::historical_posttooluse_failure_no_value_rows(...)` remains the general evaluator replay owner for paired PostToolUse no-value evidence; and `cortex/hosts/openai/posttooluse_task_standard_actuator.py` plus coordinator product behavior are explicitly untouched. This seam does not run live, delete product code, change product host behavior, model-visible Cortex text, evaluator scoring, fixtures, hidden-verifier boundaries, root hooks, SRE law, active policy, or candidate policy. It does not earn Cortex value, behavior lift, exactness value lift, broad Cortex lift, Codex App parity, shipping promotion, product progress, or AlphaEvolve candidate-evolution permission. The next train is `cortex-retained-active-policy-spine-gate0`.
- Next product train after the current focus: `cortex-retained-active-policy-spine-gate0`
...
- Train: `cortex-retained-active-policy-spine-gate0`
- Surface: no-live retained-spine proof gate
- Executive benefit: Define the smallest retained active-policy spine after PostToolUse proof-surface role-demotion: UserPromptSubmit task-standard formation, Stop closure/continuation, TaskStandardSpine state law, and shared SRE tool-evidence classification.
...
- Primary metric: Produce a no-live Gate 0 recon and artifact that names the retained spine owner modules, existing proof surfaces, model-I/O paths, value-unearned status, simple-hook parity constraints, and exact forbidden claims; it must not reactivate PostToolUse task-standard context as earned active policy.
- Guardrail: No live Codex run, no product code deletion, no product behavior change, no product host behavior change, no model-visible Cortex text change, no evaluator scoring or fixtures change, no hidden-verifier boundaries change, no root hook change, no SRE law change, no active policy mutation, and no AlphaEvolve candidate policy. Do not change product behavior, model-visible Cortex text, evaluator scoring, fixtures, hidden-verifier boundaries, root hooks, SRE law, active policy, or candidate policy. Do not claim Cortex value, behavior lift, exactness value lift, broad Cortex lift, Codex App parity, shipping promotion, product progress, or candidate-evolution permission.
- Kill rule: Fail or stop if the seam reactivates PostToolUse task-standard context as current strategy, treats retained spine proof as value lift, mutates product behavior or scoring, deletes product code, queues live or candidate evolution, or cannot distinguish retained product/evidence law from product progress.
