...
- Train: `brake-tonic-quiescence-exit-reconciliation`
- Surface: `product + conformance`
- Executive benefit: Close the SRE_2 §7.5 doctrine/code mismatch on brake exit hysteresis — either consume `tonic_quiescence` in the exit gate with a bounded exit threshold and symmetric resume recovery, or narrow the doctrine to threshold-hysteresis-only and cleanly retire the dead field — so brake law is dead-weight-free and doctrine and code say the same thing.
- Why now: Now that asymmetric error cost and tonic hysteresis are earned on the shared executive, the only remaining brake-law asymmetry is the unimplemented exit-side smoothing; the `executive_completion.next_raise` note explicitly points the next leverage at code-quality, proof-confidence, dead-weight elimination, and live-run reliability rather than new skill expansion, and this seam is exactly that.
- Primary metric: brake exit hysteresis is either gate-consumed via `tonic_quiescence` with a bounded exit threshold that keeps kill-rule-e green, or `tonic_quiescence` is retired from the carrier and diagnostics payload so docs/CORTEX_V2_SRE_2.md §7.5 and the landed code say the same thing; no dead telemetry remains
- Guardrail: keep the brake law bounded and phasic-first, consume no new memory carrier, add no cross-host exit policy fork, preserve the locked `rho = 0.60` EMA regression, and never introduce sticky guardedness under sustained calm or a quiescence-gated exit that regresses kill-rule-e
- Kill rule: cut the seam if the chosen reconciliation requires a new persistence surface, forces a host-specific exit fork, introduces sticky guardedness under sustained calm, or degrades the existing quiet-productive-flow and sustained-calm decay unit tests
...
- Work the active brake tonic quiescence exit reconciliation seam as a bounded doctrine/code alignment only: either consume `tonic_quiescence` in the guarded-to-quiescent exit gate with a bounded exit threshold and symmetric resume recovery, or narrow the SRE_2 §7.5 doctrine to threshold-hysteresis-only and retire the dead field from the carrier and diagnostics payload; the chosen reconciliation must preserve the locked `rho = 0.60` EMA regression, keep the quiet-productive-flow and sustained-calm decay unit tests green, introduce no new memory carrier, and add no host-specific exit policy fork.
