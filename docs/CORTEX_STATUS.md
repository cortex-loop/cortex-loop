...
- Current tracked train: `codex-app-cli-product-perception-loop`
- Active quality/risk focus: The current focus built structural Codex App/CLI product perception for the hook-native Stop path: UserPromptSubmit, verification-like tool events, failures, and transcript-backed Stop closure claims now update private per-session state; when no caller runtime snapshot is supplied, the coordinator derives an ExpectationLedger-backed runtime snapshot and selects or suppresses grounded intervention from product-observable lifecycle facts. The product-perception Gate 0 passed without a runtime snapshot fixture and with root repo guardrails unchanged. This earns structural perception proof only; live payload sufficiency and model-output behavior lift are not earned.
- Next product train after the current focus: `codex-app-cli-product-perception-live-probe`
...
- Train: `codex-app-cli-product-perception-live-probe`
- Surface: product
- Executive benefit: Run a no-snapshot Codex App/CLI hook-native live probe to prove real lifecycle payloads populate product perception state and feed the proven Stop actuator.
- Why now: The coordinator can now derive Stop intervention state from simulated product lifecycle payloads; before behavior-lift comparisons, the next risk is whether real Codex App/CLI sessions emit the needed UserPromptSubmit, tool, and Stop payloads in the hook client.
- Primary metric: A live isolated subject run passes no --runtime-snapshot argument, records real lifecycle rows, derives or suppresses Stop intervention from persisted product state, emits exact product-rendered text only when due, and captures model stdout/transcript response after any block.
- Guardrail: Do not use hidden-verifier facts, task identity, fixed prompt fixtures, generic canary snapshots, root repo Mission Reflection hooks, or behavior-lift claims as perception evidence.
- Kill rule: Stop if real Codex payloads do not expose enough product-observable lifecycle state, if the hook client needs a prewritten runtime snapshot to speak, if continuation response cannot be captured, or if model-visible text differs from the product renderer output.
