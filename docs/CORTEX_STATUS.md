...
- Current tracked train: `cortex-overnight-evaluator-automation-hardening`
- Active quality/risk focus: The Cortex overnight evaluator automation hardening seam adds `python3 internal/automation/cortex_overnight_loop.py --once` as the repo-owned local overnight runner contract. Each cycle acquires a lock, inspects clean synced `main` or a managed session branch, reads `internal/truth/cortex_status.json`, classifies only evaluator-authorized next work, records one ignored local digest under `.cortex/automation/overnight/YYYY-MM-DD/digest.md`, and stops for dirty main, non-managed branches, non-evaluator trains, strategic user-review boundaries, product law revision, fixture/scoring or hidden-verifier mutation, external paid/service-lane credentials, unsafe live runs, old PostToolUse task-specific harness growth, or positive value/lift claims outside the registered evaluator. The runner records LOC added/deleted, changed files, new policy paths, duplicate-policy removal, contraction-debt increase, candidate provenance fields, and contraction candidates when a policy loses to the simple baseline twice. Codex CLI live runs remain allowed only when current truth registers them inside the evaluator plan; external paid APIs remain disallowed. Routine overnight digests are local ignored artifacts, not active docs. The next train remains `cortex-executive-effectiveness-evaluator-build`; no product host behavior, model-visible text, SRE law, matcher threshold, fixture scoring, hidden scoring, root hook, or AlphaEvolve mutation loop changed.
- Next product train after the current focus: `cortex-executive-effectiveness-evaluator-build`
...
- Train: `cortex-executive-effectiveness-evaluator-build`
- Surface: no-live lab/proof evaluator build
- Executive benefit: Build the automatic paired evaluator that can decide whether Cortex lifecycle policies improve model behavior over both no-Cortex and the mandatory simple-hook challenger before any more actuator expansion.
...
- Primary metric: Build a dedicated evaluator module and episode table, not an expansion of the old PostToolUse-specific harness. The build passes only if it emits `evaluator_design.json`, `episode_table.jsonl`, `summary.json`, and `leaderboard.json`; preserves the PostToolUse `failure_no_value` historical replay; proves synthetic pass/no-value/overcontrol/trace-ambiguous/root-mutated/runtime-snapshot/hidden-leakage rows; treats simple-hook parity and silent perception success as no Cortex value; and records contraction candidates for repeatedly losing machinery.
- Guardrail: No live Codex run. No product host behavior change. No PostToolUse text or policy tuning. No AlphaEvolve mutation loop yet. Do not change signed UserPromptSubmit text, PostToolUse text, Stop text, SRE law, matcher thresholds, fixture scoring, root hooks, hidden-verifier boundary, Sinkhorn/transport, PreToolUse denial, PermissionRequest policy, output-law centralization, typed intervention pressure, or host-runtime extraction.
- Kill rule: Fail the seam if it omits the simple-hook baseline, gives Cortex value when simple or silent succeeds equally, extends the 7K-line PostToolUse harness as the new owner instead of a general episode table, cannot preserve known PostToolUse negative evidence while scoring clean passes, lacks dominance gates, starts a live run, or starts AlphaEvolve-style candidate mutation before the evaluator passes no-live proof.
