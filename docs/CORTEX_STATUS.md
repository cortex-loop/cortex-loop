...
Current train: `cortex-retained-spine-clean-control-replication-gate1`. Next train: `cortex-retained-spine-clean-control-replication-live-run`. Shipping default: `openai.codex_app_cli`. Keep Cortex truth, brain-wiring truth, conformance truth, shipping truth, and live behavior-lift claims separate; structural proof alone does not earn model-output lift.
...
- Current tracked train: `cortex-retained-spine-clean-control-replication-gate1`
- Active quality/risk focus: The retained-spine clean-control replication live-run seam replaced the refusal-only `--retained-spine-clean-control-replication-live` placeholder with an executable, resume-safe runner for the registered narrow clean-control check after Gate 0 classified `run_20260509T192719Z` repeat 1 as `no_cortex_closure_readout_instability`. The prior Gate 1 command `python3 lab/cortex_effectiveness_evaluator.py --retained-spine-clean-control-replication-gate1 --require-pass` still passes and preserves the 20-row plan for `clean_verified_work_control_v2`: four arms (`no_cortex_baseline`, `simple_hook_baseline`, `cortex_silent_perception`, `cortex_active_policy`) across five repeats, deterministic row IDs, and matched workspace seeds. Active rows use only `userpromptsubmit_stop_taskstandard_spine`; PostToolUse task-standard context remains disabled and role-demoted; simple-hook support I/O stays lab-only metadata with non-active mission rows using `model_io_path=none_lab_proof_only` and `product_spine=[]`. The approval-refusal command `python3 lab/cortex_effectiveness_evaluator.py --retained-spine-clean-control-replication-live` returns `not_run_approval_required`; the exact approved command remains `CORTEX_CODEX_APP_CLI_RETAINED_SPINE_CLEAN_CONTROL_REPLICATION_APPROVED=approved python3 lab/cortex_effectiveness_evaluator.py --retained-spine-clean-control-replication-live`. With approval, the runner writes `clean_control_replication_plan.json`, `registered_live_command.json`, `episode_table.jsonl`, `summary.json`, `leaderboard.json`, `failure_analysis.json`, `latest_run.json`, and per-row `trials/` under `.cortex/live_validation/cortex_retained_spine_clean_control_replication_live/<run_id>/`. The replication-specific verdicts are `pass_clean_control_stable`, `failure_no_cortex_readout_instability`, `failure_silent_arm_leakage`, `failure_boundary_dominance`, or `fail`; simple-hook parity and silent success remain no-value blockers, and this seam earns runner readiness only, not Cortex value, retained-spine value, retained-spine no-value parity, behavior lift, exactness value lift, broad Cortex lift, Codex App parity, shipping promotion, product progress, live result interpretation, full retained-spine matrix rerun permission, PostToolUse reactivation, or AlphaEvolve candidate-evolution permission. No live trials have run because the exact approval env has not been set in this turn; the next train remains `cortex-retained-spine-clean-control-replication-live-run`.
- Next product train after the current focus: `cortex-retained-spine-clean-control-replication-live-run`

## Document Authority Map

Document roles keep Cortex planning surfaces from drifting into one another. The status registry remains the operational roadmap authority; generated views are read-only; scoreboards and retained specs may inform planning but cannot queue work by themselves.

| Role | Meaning | Update Rule | Docs |
| --- | --- | --- | --- |
| `front_door` | Human entrypoint and document inventory. | Keep links current when any non-archive doc is added, moved, or role-changed. | `docs/README.md` |
| `identity_authority` | Canonical Cortex identity, mission fit, and product-fit rejection authority. | Update only when Cortex identity or generated status sections need correction; generated sections must come from the registry. | `docs/CORTEX.md` |
| `packet_law` | Formal V2 packet law and math. | Revise explicitly when packet law changes; implementation seams must cite the relevant packet terms. | `docs/CORTEX_V2_CORE_2.md`, `docs/CORTEX_V2_SRE_2.md`, `docs/CORTEX_V2_AUX_2.md` |
| `generated_operational_view` | Human-readable generated view of the status registry. | Do not edit directly; regenerate from internal/truth/cortex_status.json. | `docs/CORTEX_STATUS.md` |
| `planning_scoreboard` | Long-horizon product planning scoreboard and evidence-gap tracker; subordinate to the status registry. | May hold candidate backlog and scoreboard guidance, but a seam becomes current only when promoted into next_product_train. | `docs/CORTEX_EXECUTIVE_RUNTIME_TRACKER.md` |
| `retained_context` | Retained prior program specs and readiness notes; useful context but not current roadmap authority. | Do not queue work from these docs directly; extract current decisions into the tracker or status registry first. | `docs/CORTEX_EXECUTIVE_RUNTIME_PROGRAM_SPEC.md`, `docs/CORTEX_EXECUTIVE_RUNTIME_PHASE_5_READINESS.md` |
| `workflow_authority` | Repo workflow, chat-boundary reflection, and anti-drift rules. | Update with matching workflow/internal tests when branch, closeout, reflection, or anti-drift rules change. | `docs/internal/REPO_WORKFLOW.md`, `docs/internal/MISSION_REFLECTION_CONTRACT.md`, `docs/internal/ANTI_DRIFT_RULES.md` |
| `support_context` | Support dossiers, plugin sketches, runtime-context examples, and audits that inform but do not define current product truth. | Use as evidence or design context only; product truth still requires a status/recon/code/proof seam. | `docs/audit/runtime_context_vs_grounded_intervention.md`, `docs/cortex_plugin/DESIGN.md`, `docs/cortex_plugin/ADAPTER.md`, `docs/cortex_plugin/EVIDENCE_SYNTHESIS.md`, `docs/cortex_plugin/communication_problem/01_problem_statement.md`, `docs/cortex_plugin/communication_problem/02_cortex_identity_and_doctrine.md`, `docs/cortex_plugin/communication_problem/03_maths_to_code.md`, `docs/cortex_plugin/communication_problem/04_cortex_internal_state.md`, `docs/cortex_plugin/communication_problem/05_claude_communication_surface.md`, `docs/cortex_plugin/communication_problem/06_hook_surface_and_evidence.md`, `docs/cortex_plugin/communication_problem/07_strange_loop_frame.md`, `docs/cortex_plugin/communication_problem/08_anti_patterns_and_failed_solutions.md`, `docs/runtime_context/EVAL_RUBRIC.md`, `docs/runtime_context/BASELINE_SHAPED_EXAMPLES.md`, `docs/runtime_context/CROSS_HOST_SKETCH.md`, `docs/recon/cortex_codex_app_cli_hook_contract_capture_boundary_remediation.md`, `docs/recon/cortex_codex_app_cli_task_standard_context_live_rerun.md`, `docs/recon/cortex_codex_app_cli_communication_boundary_audit_and_hardening.md`, `docs/recon/cortex_codex_app_cli_task_standard_pretool_transcript_capture.md`, `docs/recon/cortex_codex_app_cli_task_standard_live_capture_rerun.md`, `docs/recon/cortex_codex_app_cli_task_standard_stop_gating_calibration_probe.md`, `docs/recon/cortex_codex_app_cli_task_standard_stop_gating_live_run.md`, `docs/recon/cortex_codex_app_cli_task_standard_behavior_comparison_harness.md`, `docs/recon/cortex_codex_app_cli_task_standard_behavior_comparison_live_run.md`, `docs/recon/cortex_codex_app_cli_task_standard_evidence_gating_remediation.md`, `docs/recon/cortex_codex_app_cli_task_standard_pre_live_audit_roadmap_update.md`, `docs/recon/cortex_codex_app_cli_task_standard_offline_replay_readiness_gate.md`, `docs/recon/cortex_codex_app_cli_task_standard_raw_vs_silent_artifact_readout.md`, `docs/recon/cortex_codex_app_cli_lifecycle_actuator_map_roadmap_update.md`, `docs/recon/cortex_codex_app_cli_lifecycle_actuator_map.md`, `docs/recon/cortex_codex_app_cli_posttooluse_task_standard_next_step_correction.md`, `docs/recon/cortex_codex_app_cli_posttooluse_task_standard_calibration_decision.md`, `docs/recon/cortex_codex_app_cli_raw_vs_silent_artifact_readout_roadmap_update.md`, `docs/recon/cortex_codex_app_cli_task_standard_stack_publication_hygiene.md`, `docs/recon/cortex_codex_app_cli_posttooluse_task_standard_narrow_live_probe.md`, `docs/recon/cortex_codex_app_cli_posttooluse_task_standard_narrow_live_run.md`, `docs/recon/cortex_codex_app_cli_posttooluse_task_standard_actuator_architecture_decision.md`, `docs/recon/cortex_codex_app_cli_posttooluse_task_standard_phase_aware_calibration_gate0.md`, `docs/recon/cortex_codex_app_cli_posttooluse_task_standard_phase_aware_narrow_live_run.md`, `docs/recon/cortex_codex_app_cli_posttooluse_task_standard_firing_boundary_remediation.md`, `docs/recon/cortex_codex_app_cli_posttooluse_task_standard_phase_aware_narrow_live_rerun.md`, `docs/recon/cortex_codex_app_cli_posttooluse_task_standard_overcontrol_remediation.md`, `docs/recon/cortex_codex_app_cli_posttooluse_task_standard_actuator_trace_repair.md`, `docs/recon/cortex_codex_app_cli_posttooluse_causal_trace_ids.md`, `docs/recon/cortex_codex_app_cli_posttooluse_shared_tool_evidence_classification.md` |
| `recon_evidence` | Recon evidence records and live/lab findings. | Add a recon when a proof or decision is earned; do not treat recon alone as shipping truth without status promotion. | `docs/recon/lifecycle_first_surface_matrix.md`, `docs/recon/codex_app_hook_probe.md`, `docs/recon/claude_code_desktop_pretooluse_probe.md`, `docs/recon/claude_code_user_scope_plugin_pretooluse_probe.md`, `docs/recon/claude_code_user_scope_plugin_managed_worktree_probe.md`, `docs/recon/claude_code_cortex_runtime_context_connectivity_probe.md`, `docs/recon/claude_code_cortex_stop_closure_connectivity_probe.md`, `docs/recon/claude_code_cortex_headless_cli_equivalence_probe.md`, `docs/recon/claude_code_cortex_bridge_translation_headless_probe.md`, `docs/recon/claude_code_cortex_mac_pending_goal_divergence_retest.md`, `docs/recon/claude_code_cortex_posttool_failure_to_stop_loop_probe.md`, `docs/recon/claude_code_cortex_userpromptsubmit_verified_work_probe.md`, `docs/recon/cortex_openai_operator_silent_control_live_probe.md`, `docs/recon/cortex_openai_operator_debt_control_enactment.md`, `docs/recon/cortex_openai_operator_silent_control_live_probe_retry.md`, `docs/recon/cortex_openai_operator_output_quality_fixture_refresh.md`, `docs/recon/cortex_openai_operator_verification_debt_continuation.md`, `docs/recon/cortex_openai_operator_visible_intervention_live_probe.md`, `docs/recon/cortex_visible_intervention_product_perception_hardening.md`, `docs/recon/cortex_openai_operator_visible_intervention_hardened_rerun.md`, `docs/recon/cortex_codex_app_cli_hook_native_stop_activation_probe.md`, `docs/recon/cortex_codex_app_cli_hook_native_stop_live_canary.md`, `docs/recon/cortex_codex_app_cli_product_perception_loop.md`, `docs/recon/cortex_codex_app_cli_product_perception_live_probe.md`, `docs/recon/cortex_codex_app_cli_product_event_capture_remediation.md`, `docs/recon/cortex_codex_app_cli_stop_continuation_resolution_loop.md`, `docs/recon/cortex_codex_app_cli_hook_native_behavior_comparison.md`, `docs/recon/cortex_codex_app_cli_astro_three_arm_fixture_refresh.md`, `docs/recon/cortex_codex_app_cli_value_ablation_audit.md`, `docs/recon/cortex_codex_app_cli_task_standard_spine.md`, `docs/recon/cortex_task_standard_sre_correspondence_reconciliation.md`, `docs/recon/cortex_task_standard_executive_doctrine_math_refinement.md`, `docs/recon/cortex_codex_app_cli_task_standard_live_probe.md`, `docs/recon/cortex_codex_app_cli_task_standard_live_run.md`, `docs/recon/cortex_semantic_contraction_audit.md`, `docs/recon/claude_code_desktop_lifecycle_spine_branch_disposition.md`, `docs/recon/cortex_codex_app_cli_posttooluse_task_standard_phase_aware_narrow_live_rerun_after_shared_tool_evidence.md`, `docs/recon/cortex_codex_app_cli_posttooluse_task_standard_context_loop_trace_remediation.md`, `docs/recon/cortex_codex_app_cli_posttooluse_task_standard_phase_aware_narrow_live_rerun_after_context_loop_trace_remediation.md`, `docs/recon/cortex_codex_app_cli_posttooluse_task_standard_closure_reporting_architecture_decision.md`, `docs/recon/cortex_codex_app_cli_posttooluse_strategy_failure_audit.md`, `docs/recon/cortex_codex_app_cli_posttooluse_task_standard_measurement_stack_rebuild_gate0.md`, `docs/recon/cortex_codex_app_cli_posttooluse_task_standard_final_closure_readout_remediation_gate0.md`, `docs/recon/cortex_codex_app_cli_posttooluse_task_standard_exactness_only_paired_value_probe_gate0.md`, `docs/recon/cortex_codex_app_cli_posttooluse_task_standard_exactness_only_paired_value_live_probe.md`, `docs/recon/cortex_effectiveness_strategy_reset.md`, `docs/recon/cortex_executive_effectiveness_evaluator_gate0.md`, `docs/recon/cortex_overnight_evaluator_automation_hardening.md`, `docs/recon/cortex_executive_effectiveness_evaluator_build.md`, `docs/recon/cortex_automation_product_boundary_contract.md`, `docs/recon/cortex_executive_effectiveness_evaluator_live_gate1.md`, `docs/recon/cortex_simple_hook_baseline_challenger.md`, `docs/recon/cortex_executive_effectiveness_evaluator_live_matrix_run.md`, `docs/recon/cortex_effectiveness_measurement_stack_rebuild_gate0.md`, `docs/recon/cortex_effectiveness_v2_case_registry_gate0.md`, `docs/recon/cortex_effectiveness_v2_live_matrix_gate1.md`, `docs/recon/cortex_effectiveness_v2_live_matrix_run.md`, `docs/recon/cortex_active_policy_contraction_decision.md`, `docs/recon/cortex_posttooluse_proof_surface_role_demotion.md`, `docs/recon/cortex_retained_active_policy_spine_gate0.md`, `docs/recon/cortex_retained_active_policy_spine_live_gate1.md`, `docs/recon/cortex_retained_active_policy_spine_live_run.md`, `docs/recon/cortex_retained_spine_live_matrix_materialization_remediation.md`, `docs/recon/cortex_retained_spine_measurement_stack_remediation.md`, `docs/recon/cortex_retained_spine_clean_control_stability_gate0.md`, `docs/recon/cortex_retained_spine_clean_control_replication_gate1.md` |
...
- Train: `cortex-retained-spine-clean-control-replication-live-run`
- Surface: approval-gated retained-spine clean-control replication live run
- Executive benefit: Run the registered narrow clean-control replication check before interpreting retained-spine evidence. Gate 1 proved a 20-row plan for `clean_verified_work_control_v2` only, with four arms, five repeats, matched workspace seeds, retained-spine active rows only, and PostToolUse disabled. The live seam tests whether no-Cortex closure/evidence readout instability reproduces without rerunning the full retained-spine matrix or changing product/scoring/fixtures.
...
- Primary metric: Approval-gated clean-control-only replication live run executes exactly 20 rows, preserves four-arm pairing and retained-spine/PostToolUse boundaries, then records whether no-Cortex closure/evidence readout instability reproduces or the clean-control row is stable.
- Guardrail: Live Codex may run only through the exact registered command/env pair and only after explicit current-turn approval. No full retained-spine matrix rerun. No product behavior change. No product code deletion. Do not change product host behavior, model-visible Cortex text, evaluator scoring or fixtures, hidden-verifier boundaries, root hooks, SRE law, active policy, PostToolUse policy or PostToolUse reactivation, or AlphaEvolve candidate policy. Do not reinterpret no-Cortex closure readout instability as retained-spine value or retained-spine no-value parity before the replication result.
- Kill rule: Fail or stop if the seam changes scoring or fixtures, reruns more than the clean-control-only 20-row replication, mutates product policy, reactivates PostToolUse, treats prior no-Cortex closure readout instability as Cortex value or retained-spine no-value parity, omits preservation of the raw `run_20260509T192719Z` failure and corrected replay diagnosis, loads a runtime snapshot, mutates root config, leaks hidden verifier data, or queues candidate evolution.
...
- `Codex App/CLI PostToolUse task-standard architecture decision` -> `docs/recon/cortex_codex_app_cli_posttooluse_task_standard_actuator_architecture_decision.md`
  Purpose: Classifies the `failure_context_ignored` live result as PostToolUse timing/selection failure and queues phase-aware no-live calibration before another live run.
...
- `docs/recon/cortex_codex_app_cli_task_standard_offline_replay_readiness_gate.md`
- `docs/recon/cortex_codex_app_cli_task_standard_raw_vs_silent_artifact_readout.md`
- `docs/recon/cortex_codex_app_cli_lifecycle_actuator_map_roadmap_update.md`
...
- `docs/recon/cortex_semantic_contraction_audit.md`
...
- `docs/recon/cortex_effectiveness_strategy_reset.md`
