...
- Current tracked train: `runtime-expectation-debt`
- Active quality/risk focus: Roadmap Phase 1 is active: Cortex is adding a bounded runtime expectation ledger and resolution-deficit diagnostic so structured forward commitments can record what uncertainty reduction was owed and whether generic evidence, continuity, certification, retraction, blocker, or waiting-on-user signals paid it down. This train is silent structural state only: it does not change brake law, route selection, Claude hooks, model-visible runtime context, AUX publication, support memory, or shipping defaults. The seam intentionally uses high-precision structured cues only (dispatch reason tags, commitment result kind, task mode, progress classes, warning codes, and explicit structured commitment metadata); arbitrary assistant prose does not open debt, and ambiguous prose defaults to no expectation. Truth earned by this train is Cortex/conformance structural truth only: live model behavior lift remains unearned until later debt-drag and empirical seams connect the diagnostic to route/brake/model I/O and prove improvement.
- Next product train after the current focus: `debt-drag-into-brake-and-route`
...
- Train: `debt-drag-into-brake-and-route`
- Surface: product
- Executive benefit: Connect the runtime expectation ledger's resolution-deficit diagnostic to executive control so unpaid expectation debt makes new unsupported forward commitments harder while preserving check, inspect, ask, and blocker-surfacing moves.
...
- Primary metric: A `GoalDebtDrag` or equivalent control-pressure scalar derived from resolution deficit and persistence-weighted goal debt that biases away from execute/close and toward inspect/check/seek-context/ask without changing certification standards or surfacing model-visible text without a grounded anchor.
- Guardrail: Tonic debt pressure must not latch by itself, must not suppress verification or user-wait moves, must stay silent when no claim/evidence/obligation anchor exists, and must preserve the no-AUX/no-hook/no-shipping-promotion boundary until live evidence earns behavior lift.
- Kill rule: Cut or revise the seam if debt pressure freezes useful verification, increases false blocks on honest partial progress, changes route behavior without diagnostics proving the path, or smuggles internal debt/brake terms into model-visible output.
...
- The runtime-expectation-debt seam is the active focus from the executive-runtime roadmap: the shared runtime gains bounded expectation records and resolution-deficit diagnostics from structured forward commitments only. This is structural state, not behavior lift; brake, route, hooks, AUX, and model-visible communication remain unchanged until a later debt-drag seam explicitly connects the diagnostic to model I/O.
