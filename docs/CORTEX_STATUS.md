...
- Current tracked train: `astro-three-arm-fixture-refresh-rerun`
- Active quality/risk focus: The current focus fixed the Astro output-quality fixture so verifier-only files and hidden npm scripts are absent from subject workspaces, then reran raw Codex, Cortex silent perception, and fully enabled Cortex on Codex CLI. Gate 0 passed and the live run completed 15 trials with hidden scoring external, no runtime snapshots, no root hook drift, no hidden-verifier probing, and no timeouts. The verdict was mixed_signal: raw passed hidden quality 2/5, silent 1/5, full 2/5, and Cortex emitted 0 model-visible Stop blocks. This earns fixture-repair and live no-speech-lift evidence, not behavior lift.
- Next product train after the current focus: `codex-app-cli-output-quality-perception-decision`
...
- Train: `codex-app-cli-output-quality-perception-decision`
- Surface: product
- Executive benefit: Decide whether output-quality failures on the Codex App/CLI hook-native lane need deeper product-visible perception, a cleaner task family, PreToolUse motor inhibition, or an explicit scope boundary before more implementation.
- Why now: The refreshed Astro three-arm run removed hidden-verifier leakage but produced mixed results with zero Cortex blocks, so more code would risk optimizing around a noisy fixture instead of proving executive-function lift.
- Primary metric: A recorded decision names the next substrate and a falsifiable live metric: either a product-visible perception anchor for output-quality gaps, a refreshed task family with reproducing baseline failure, or a scoped exclusion for hidden-quality cases the current Stop-only loop cannot lawfully perceive.
- Guardrail: Do not tune Cortex speech, selector law, hidden-verifier perception, task identity, fixture prompts, or lab oracle signals to force an Astro lift result; hidden verifier facts remain scoring-only.
- Kill rule: If no product-visible anchor can distinguish the hidden-quality gap from adequately verified visible work, do not implement Astro-specific remediation; choose a different failure family or mark this class outside the current Stop-only substrate.
