...
- Current tracked train: `codex-app-cli-product-event-capture-remediation`
- Active quality/risk focus: The current focus remediated the Codex App/CLI product event-capture path. The harness now registers UserPromptSubmit, PreToolUse, PostToolUse, and Stop in an isolated subject git root with product hook config only and no --runtime-snapshot. The live Codex CLI run passed full lifecycle capture: 1 UserPromptSubmit row, 2 PreToolUse rows, 2 PostToolUse rows, 2 Stop rows, 1 exact product-rendered block row, no non-Stop stdout, no runtime snapshot, and unchanged root guardrails. This earns live product event-capture and Stop-actuator evidence only, not behavior lift; the continuation repair loop remains open because the post-block check did not resolve the active expectation before stop_hook_active suppression.
- Next product train after the current focus: `codex-app-cli-stop-continuation-resolution-loop`
...
- Train: `codex-app-cli-stop-continuation-resolution-loop`
- Surface: product
- Executive benefit: Close the Codex App/CLI hook-native Stop loop so a blocked verification expectation can be resolved or kept open from product-observable continuation evidence instead of relying only on stop_hook_active suppression.
- Why now: The event-capture remediation proved real UserPromptSubmit/tool/Stop payloads reach Cortex and the first Stop can block from product state; the same run showed the continuation can perform checks while the active expectation remains unresolved in Cortex state.
- Primary metric: An isolated live run opens due verification from a first premature closure, emits exact identity-continuous Stop text, captures continuation tool evidence, and reaches a final Stop whose silence reason is pressure_below_visible_threshold or whose active gap remains explicitly named from product state, without snapshots or hidden verifier facts.
- Guardrail: Do not use task identity, hidden-verifier answers, fixture prompts, runtime snapshots, parent repo workflow hooks, or new model-visible wording as repair evidence; keep non-Stop hooks silent unless a later motor-inhibition seam explicitly earns text or block behavior.
- Kill rule: If Codex continuation payloads and transcripts cannot supply enough product-visible evidence to resolve or preserve the blocked expectation, stop and queue a perception-classifier or event-stream adapter remediation instead of weakening stop_hook_active discipline.
