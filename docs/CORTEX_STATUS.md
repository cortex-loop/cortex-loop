...
- Current tracked train: `codex-app-cli-hook-native-stop-live-canary`
- Active quality/risk focus: The current focus ran the isolated OpenAI Codex App/CLI product Stop-hook live canary after structural Gate 0: a real `codex exec` subject run produced 3 Stop hook rows, 1 exact block JSON row with the locked identity-continuous text, and 2 `stop_hook_active=true` continuation rows that stayed silent. Root repo Mission Reflection guardrails remained untouched. This earns live actuator delivery only; product perception and behavior lift are not earned.
- Next product train after the current focus: `codex-app-cli-product-perception-loop`
...
- Train: `codex-app-cli-product-perception-loop`
- Surface: product
- Executive benefit: Build the Codex App/CLI product perception loop so lifecycle events produce the runtime snapshot that the proven Stop actuator consumes.
- Why now: The live Stop canary proves native hook delivery from a stimulus snapshot; the next highest-leverage gap is deriving that snapshot from product-observable lifecycle state rather than lab or canary scaffolding.
- Primary metric: UserPromptSubmit, tool, failure, and Stop events update per-session product state enough for Stop to select or suppress grounded intervention from runtime evidence, with clean/title/continuation controls staying silent.
- Guardrail: Do not use lab or hidden-verifier facts, task identity, fixed prompt fixtures, generic canary snapshots, root repo Mission Reflection hooks, or behavior-lift claims as perception evidence.
- Kill rule: Stop if the loop cannot derive due expectations and prior-act anchors from product-observable Codex lifecycle payloads, if it needs fixture-supplied state to speak, or if any model-visible text differs from the product renderer output.
