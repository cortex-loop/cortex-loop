...
- Current tracked train: `cortex-active-policy-contraction-decision`
- Active quality/risk focus: The no-live contraction decision used v2 evaluator artifact `run_20260509T112542Z` (`failure_no_value`) and preserved v1 artifact `run_20260508T221352Z` (`failure_silent_perception_contamination`) without rescoring. Verdict: `decision_contract_posttooluse_proof_surfaces`. The decision map records that `run_20260509T112542Z` completed `60 live rows`, all four arms tied on every v2 family, active Cortex had `family_wins=0`, and no boundary dominance was recorded: root config stayed unchanged, no runtime snapshot loaded, no hidden-verifier leakage, trace ambiguity, repeated intervention loop, or clean-control overcontrol appeared. The retained smaller active-policy spine is UserPromptSubmit task-standard formation, Stop closure/continuation, shared `TaskStandardSpine` state law, and shared SRE tool-evidence classification; this spine is retained as value-unearned product/evidence law, not as Cortex value. PostToolUse task-standard context is classified as redesign-only before any renewed active use, and the PostToolUse-specific Gate 0/live proof train is classified as archive/role-demotion material once the general evaluator preserves its historical failure evidence. This seam does not run live, delete product code, change product host behavior, model-visible Cortex text, evaluator scoring, fixtures, hidden-verifier boundaries, root hooks, SRE law, active product policy, or candidate policy. It does not earn Cortex value, behavior lift, exactness value lift, broad Cortex lift, Codex App parity, shipping promotion, product progress, or AlphaEvolve candidate-evolution permission. The next train is `cortex-posttooluse-proof-surface-role-demotion`.
- Next product train after the current focus: `cortex-posttooluse-proof-surface-role-demotion`
...
- Train: `cortex-posttooluse-proof-surface-role-demotion`
- Surface: no-live archive and role-demotion
- Executive benefit: Shrink the active-policy evidence stack by role-demoting or archiving PostToolUse-specific proof surfaces now that the general four-arm evaluator owns value comparison and the v2 run produced failure_no_value.
...
- Primary metric: Produce a no-live role-demotion/archive map for PostToolUse-specific Gate 0 modes, live-probe docs, and historical artifacts that names which evidence remains in the general evaluator replay, which docs stay historical, which commands become non-current support, and which product files are explicitly untouched.
- Guardrail: No live Codex run, no product code deletion, no product host behavior change, and no product behavior change. Do not change model-visible Cortex text, evaluator scoring or fixtures, hidden-verifier boundaries, root hooks, SRE law, active product policy, or AlphaEvolve candidate policy. Do not claim Cortex value, behavior lift, exactness value lift, broad Cortex lift, Codex App parity, shipping promotion, or product progress. Do not remove or hide historical negative evidence; role-demotion must preserve evidence traceability.
- Kill rule: Fail or stop if the seam deletes product code, rewrites evaluator scoring, removes historical evidence without a preserved replay/reference, treats role-demotion as product progress, queues a live rerun or candidate evolution, or changes PostToolUse runtime behavior instead of proof-surface status.
