...
- Active quality/risk focus: V2 communication optimization is now the baseline and the active train is intervention-policy payoff: Cortex must show that compressed/dynamic guidance changes outcomes, not merely that the full 17-row denominator is visible. The payoff artifact now normalizes every paired run into a stable shape covering host, surface, task pack, scenario, repeat index, variant, guidance burden, latency, task success, truthful closure, blocker surfacing, verification quality, scope churn, unnecessary intervention, reviewer notes, forbidden claims, and a composite executive-success score. Tier 1 behavioral proof is Claude CLI plus true Codex CLI (`codex` provider); OpenAI app-server/operator evidence remains support evidence under `openai` and must not be substituted for Codex CLI proof. The product default profile is `normal`, which renders `compressed_dynamic`; `full` remains audit/denominator mode and `raw` remains eval baseline. The current live evidence remains bounded: Claude and Codex CLI full-communication non-regression passed on run `20260425T132046Z0000`, but promotion now requires repeated Tier 1 causal payoff, non-regression versus full V2, at least 35% median guidance-burden reduction, stricter intervention-timing gates, no hard forbidden claims, and 2:1 adoption preference evidence from expert/dogfood review.
