...
- Current tracked train: `cortex-retained-spine-clean-control-stability-gate0`
- Active quality/risk focus: The no-live retained-spine clean-control stability Gate 0 diagnosed `clean_verified_work_control_v2` repeat 1 from `run_20260509T192719Z` without rerunning live or changing product/scoring/fixtures. Gate 0 command `python3 lab/cortex_effectiveness_evaluator.py --retained-spine-clean-control-stability-gate0 --require-pass` passed and wrote `clean_control_stability_report.json`, `no_cortex_readout_diagnosis.json`, `arm_isolation_report.json`, `gate0_report.json`, and `summary.json` under `.cortex/live_validation/cortex_retained_spine_clean_control_stability_gate0/`. The raw live artifact remains preserved as registered verdict `fail` with `mission_contract_error`; the corrected replay verdict remains `failure_silent_perception_contamination` with failure reason `silent_perception_beat_no_cortex`. The clean-control repeat 1 row is classified as `no_cortex_closure_readout_instability`: no-Cortex had `task_success=true`, `v2_verifier_exit_code=0`, `truthful_closure=false`, and `evidence_recovery=false`, while simple-hook, silent Cortex, and active Cortex each reported task success, truthful closure, and evidence recovery. Evidence depth is `stdout_stderr_and_verifier_artifacts`, not `metrics_only`; the no-Cortex stdout reported `command not found: python` and did not report the successful `python3` fallback that comparison arms reported. Silent-arm isolation stayed clean with `model_io_path=none_lab_proof_only` and `product_spine=[]`, PostToolUse remained disabled and role-demoted, active rows remained `userpromptsubmit_stop_taskstandard_spine`, root config stayed unchanged, no runtime snapshot loaded, and hidden verifier did not leak. The next train is `cortex-retained-spine-clean-control-replication-gate1`. This seam earns clean-control measurement diagnosis only, not retained-spine value, retained-spine no-value parity, Cortex value, behavior lift, exactness value lift, broad Cortex lift, Codex App parity, shipping promotion, product progress, live matrix rerun permission, PostToolUse reactivation, or AlphaEvolve candidate-evolution permission.
- Next product train after the current focus: `cortex-retained-spine-clean-control-replication-gate1`
...
- Train: `cortex-retained-spine-clean-control-replication-gate1`
- Surface: no-live retained-spine clean-control replication gate
- Executive benefit: Register a narrow clean-control replication gate before any retained-spine value interpretation. The clean-control stability Gate 0 classified `clean_verified_work_control_v2` repeat 1 as `no_cortex_closure_readout_instability`: no-Cortex passed verifier evidence but under-reported closure/evidence, while simple, silent, and active arms reported it. The next seam must prove a replication plan can test that instability without rerunning the full retained-spine matrix or changing product/scoring/fixtures.
...
- Primary metric: No-live clean-control replication Gate 1 proves the future replication interface is narrow, approval-gated, clean-control-only, and preserves the no-Cortex readout-instability hypothesis without scoring, fixture, product-policy, or active-policy changes.
- Guardrail: No live Codex run unless a later seam explicitly registers and receives approval for a narrow clean-control replication command. No product behavior change. No product code deletion. Do not change product host behavior, model-visible Cortex text, evaluator scoring or fixtures to favor Cortex, hidden-verifier boundaries, root hooks, SRE law, active policy, PostToolUse policy or PostToolUse reactivation, or AlphaEvolve candidate policy. Do not reinterpret no-Cortex closure readout instability as retained-spine value or retained-spine no-value parity.
- Kill rule: Fail or stop if the seam reruns the full retained-spine live matrix, changes scoring or fixtures to favor Cortex, mutates product policy, reactivates PostToolUse, treats `no_cortex_closure_readout_instability` as Cortex value or retained-spine no-value parity, omits preservation of the raw `run_20260509T192719Z` failure and corrected replay diagnosis, loads a runtime snapshot, mutates root config, or queues candidate evolution.
