...
Version `0` still precommits coverage from the current scenario catalog and preserves explicit exclusions, and it now records three reference-only experimental thrash pairs, three reference-only experimental uncertainty pairs, and three Gemini-only experimental uncertainty pairs.
