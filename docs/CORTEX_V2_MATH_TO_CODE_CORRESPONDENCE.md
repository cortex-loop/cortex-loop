...
Forbidden leaks: the runtime shell may compose existing reference-host, core, and SRE carriers, but it may not expand Core, introduce multi-host abstraction, invent a second continuity model outside the landed branch/goal carriers, or launder provisional neutral selection into a computed executive loop. Cheap-path default must remain intact, commitment result kinds may only surface from the existing certification lattice, and illegal continuity transitions may not silently mutate session truth. The first outward projection may serialize landed runtime truth for local CLI use, including a compact executive-state summary and one-process continuity state, but it may not invent continuity, commit kinds, or runtime-on executive state that the step kernel did not actually produce.
