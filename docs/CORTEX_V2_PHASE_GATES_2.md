...
| `L3` OpenAI live validation | `docs/CORTEX_V2_LIVE_VALIDATION_PROGRAM_0.md`; `docs/CORTEX_V2_LIVE_VALIDATION_VERDICT_0.md`; `make live-preflight`; `make live-provider-baselines`; `make live-openai-app-server`; `make live-cortex-host-control` | closed | landed | OpenAI direct-API canonical truth is now re-earned for current scope on this machine: the `canonical_anchor` suite clears `pass_minimal`, `truth_gap`, and `restart_continuity` twice, and `make live-compare` now reports canonical truth re-earned for current scope while the App Server lane remains watchlist-only lifecycle evidence |
...
| `L4` lifecycle-first payoff verdict | `docs/CORTEX_V2_LIVE_VALIDATION_VERDICT_0.md`; `make live-compare` | keep broader payoff partial until another direct-API host lands or package scope is explicitly narrowed | partial | after the R1 reset, broader payoff is read from the canonical direct-API lane first; OpenAI now re-earns that lane for current scope, Claude now shares the same canonical suite implementation but remains auth-blocked, and cross-host runtime payoff therefore remains partial |
