...
| `R3` reference live continuity slice | `docs/CORTEX_V2_REFERENCE_RUNTIME_PROGRAM_0.md`; `tests/integration/test_reference_runtime_continuity.py` | `R3B` -> `R3C` | partial | the first one-process live continuity law is landed on the reference-host CLI shell; explicit continuity enforcement and conservative re-audit still remain open |
