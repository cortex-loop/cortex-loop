...
| `R3` reference live continuity slice | `docs/CORTEX_V2_REFERENCE_RUNTIME_PROGRAM_0.md`; `tests/integration/test_reference_runtime_continuity.py`; `tests/integration/test_reference_runtime_cli.py` | corrective closeout | drifted | the first one-process live continuity law and explicit rejection enforcement remain the active shell center, but corrective re-hardening is open because the audit found a pending-goal anchor-loss bug and living-correspondence drift that must be resolved before `R3` is re-closed |
