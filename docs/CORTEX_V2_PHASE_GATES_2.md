...
| `O4R` OpenAI verified-work restoration lane | `docs/CORTEX_V2_OPENAI_VERIFIED_WORK_PROGRAM_0.md`; `make revalidate-openai-host-control`; repeated `python3 tools/cortex_conformance.py --mode active --brain openai` | closed | landed | the shared verified-work law, runtime-native verification binding, deterministic `full_files` verifier/profile registry, and one bounded repair turn remain implemented on top of the existing OpenAI host-control family, the invariant verified-work runtime helpers now live in a neutral runtime home, the OpenAI verified-work path now attaches a bounded read-only workspace context bundle over the current writable-file contents plus the contract tests, the bounded workspace verifier now installs `-e .[test] pytest` so both lawful task profiles execute cleanly, three repeated targeted local OpenAI reruns pass the bookmarks pack on attempt `1`, and repeated targeted local OpenAI reruns on the normalize-port breadth pack now also pass on attempt `1`; `O4R` is therefore landed for the current shipping-default scope while remaining explicitly outside the current canonical-anchor proof bundle |
