...
| `O4R` OpenAI verified-work restoration lane | `docs/CORTEX_V2_OPENAI_VERIFIED_WORK_PROGRAM_0.md`; `make revalidate-openai-host-control` | repeat-stable local larger-task reruns still required before closeout | partial | the shared verified-work law, runtime-native verification binding, deterministic `full_files` verifier/profile, and one bounded repair turn are now implemented on top of the existing OpenAI host-control family, and one live local bookmarks rerun confirmed that runtime truth can drive a real repair continuation without benchmark-local control; however the verified-work path has not yet earned repeat-stable lift over one-shot behavior, so `O4R` remains partial and is explicitly outside the current canonical-anchor proof bundle |
...
Claude and Gemini may remain blocked as future host-expansion backlog without blocking the current OpenAI-only product claim.

| Gate row | Current evidence | Owner / next closeout | Status | Notes |
| --- | --- | --- | --- | --- |
| `L1` Claude live validation | `docs/CORTEX_V2_LIVE_VALIDATION_PROGRAM_0.md`; `docs/CORTEX_V2_LIVE_VALIDATION_VERDICT_0.md`; `make live-preflight`; `make live-provider-baselines`; focused Claude operator reruns; `python3 tools/live_cortex_host_control.py --lane automation --provider claude --suite canonical_anchor` | future host-expansion backlog only; reopen Claude direct-API truth in a later explicit host-expansion train on a capable machine | blocked | Claude currently contributes positive watchlist evidence on the headless-CLI lane, the shared `canonical_anchor` direct-API suite remains implemented for Claude future host-expansion plumbing, and canonical truth remains blocked on missing `ANTHROPIC_API_KEY`; this does not block the accepted OpenAI-only product scope |
...
| `L2` Gemini live validation | `docs/CORTEX_V2_LIVE_VALIDATION_PROGRAM_0.md`; `docs/CORTEX_V2_LIVE_VALIDATION_VERDICT_0.md`; `make live-preflight`; `make live-provider-baselines`; focused Gemini operator reruns; `make live-cortex-host-control` | keep Gemini watchlist-only until direct API/service auth exists; investigate local-vs-accepted watchlist drift only as a bounded support seam | blocked | Gemini remains the noisiest headless-CLI watchlist line, current local reruns may drift from the accepted watchlist baseline, and Gemini is intentionally outside the accepted OpenAI-only product scope until a later host-expansion train deliberately opens its service lane |
...
| `L6A` Claude service live proof | `docs/CORTEX_V2_LIVE_SERVICE_PROOF_0.md`; `make live-preflight`; `python3 tools/live_cortex_host_control.py --lane automation --provider claude --suite canonical_anchor`; `make live-compare` | future host-expansion backlog only; rerun on a capable machine only when a later explicit Claude host-expansion train opens | blocked | the current machine remains out of scope for actual Claude service proof because machine auth readiness is not yet satisfied; the shared `canonical_anchor` direct-API suite is implemented for Claude future host-expansion plumbing, but signed-in Claude CLI truth still does not count as service-lane auth and the row is non-blocking to the accepted OpenAI-only product scope |
| `L6B` Gemini service live proof | `docs/CORTEX_V2_LIVE_SERVICE_PROOF_0.md`; `make live-preflight`; `python3 tools/live_cortex_host_control.py --lane automation --provider gemini`; `make live-compare` | future host-expansion backlog only; rerun on a capable machine only when a later explicit Gemini host-expansion train opens | blocked | the current machine remains out of scope for actual Gemini service proof because machine auth readiness is not yet satisfied; signed-in Gemini CLI truth still does not count as service-lane auth, and the row is non-blocking to the accepted OpenAI-only product scope |
