...
| Gate row | Current evidence | Owner / next closeout | Status | Notes |
| --- | --- | --- | --- | --- |
| `L1` Claude live validation | `docs/CORTEX_V2_LIVE_VALIDATION_PROGRAM_0.md`; `docs/CORTEX_V2_LIVE_VALIDATION_VERDICT_0.md`; `make live-preflight`; `make live-provider-baselines`; `make live-host-native-product-paths`; `make live-cortex-host-control` | re-audit the current one-turn Claude CLI harness, then rerun `L1` | partial | Claude operator probe and baseline are now clean on `claude-sonnet-4-6`, and `restart_continuity` succeeds, but the current one-turn CLI harness still stops at tool-use before `pass_minimal` and `truth_gap` can close; the automation lane is still blocked on missing `ANTHROPIC_API_KEY` |
| `L2` Gemini live validation | `docs/CORTEX_V2_LIVE_VALIDATION_PROGRAM_0.md`; `docs/CORTEX_V2_LIVE_VALIDATION_VERDICT_0.md`; `make live-preflight`; `make live-provider-baselines`; `make live-host-native-product-paths`; `make live-cortex-host-control` | stabilize Gemini fallback or narrow the watchlist, then rerun `L2` | partial | Gemini operator probe and baseline are now clean on explicit fallback `gemini-2.5-flash`, but the heavier operator coding lane still emits `capacity_exhausted` on `pass_minimal` and `restart_continuity` even when the local workspace diff and target test succeed; the automation lane is still blocked on missing ADC or `GEMINI_API_KEY` |
