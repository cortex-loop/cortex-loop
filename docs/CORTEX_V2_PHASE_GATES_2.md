...
| `CT2` active verified-work tri-brain conformance | `docs/CORTEX_V2_ACTIVE_WORKSTREAM.md`; `make conformance-preflight`; `make conformance-fast`; local artifacts under `.cortex/live_validation/conformance/` | keep partial until the active verified-work pack improves on the remaining divergent surface or earns a cut/reframe decision | partial | the corrected active bookmarks verified-work pack now reads OpenAI `service_api` partial because the bounded repair path still finished `test_failed`, Claude `operator_cli` divergent because the repair attempt did not complete cleanly and the final result fell back to raw output plus `output_invalid`, and Gemini `operator_cli` partial because structured `json_object` extraction now works and the result still finishes `test_failed`; current next decision remains `fix_wiring_only` because the corrected divergence does not repeat across brains |
