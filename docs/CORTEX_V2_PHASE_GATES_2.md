...
| Gate row | Current evidence | Owner / next closeout | Status | Notes |
| --- | --- | --- | --- | --- |
| `L1` Claude live validation | `docs/CORTEX_V2_LIVE_VALIDATION_PROGRAM_0.md`; `docs/CORTEX_V2_LIVE_VALIDATION_VERDICT_0.md`; `make live-preflight`; `make live-provider-baselines`; `make live-host-native-product-paths`; `make live-cortex-host-control` | refresh Claude auth and rerun `L1` | blocked | Claude now has the correct operator-lane surface (`claude`), the correct local-only artifact policy, and a real shared coding harness, but the signed-in operator path is still blocked by `auth_expired` and the automation lane is still blocked on missing `ANTHROPIC_API_KEY` |
| `L2` Gemini live validation | `docs/CORTEX_V2_LIVE_VALIDATION_PROGRAM_0.md`; `docs/CORTEX_V2_LIVE_VALIDATION_VERDICT_0.md`; `make live-preflight`; `make live-provider-baselines`; `make live-host-native-product-paths`; `make live-cortex-host-control` | stabilize Gemini fallback or narrow the watchlist, then rerun `L2` | partial | Gemini now has an explicit signed-in operator fallback to `gemini-2.5-flash`; operator smoke succeeds on that fallback, but the heavier operator coding lane is still an operational watchlist and the automation lane is still blocked on missing ADC or `GEMINI_API_KEY` |
| `L3` OpenAI live validation | `docs/CORTEX_V2_LIVE_VALIDATION_PROGRAM_0.md`; `docs/CORTEX_V2_LIVE_VALIDATION_VERDICT_0.md`; `make live-preflight`; `make live-provider-baselines`; `make live-host-native-product-paths`; `make live-cortex-host-control` | add automation credentials and rerun `L3` | partial | OpenAI now uses the correct signed-in operator surface (`codex`), not the `openai` utility CLI; the operator lane succeeds on `pass_minimal`, preserves `truth_gap`, and succeeds on `restart_continuity`, while the automation/service lane remains blocked on missing `OPENAI_API_KEY` |
| `L4` lifecycle-first payoff verdict | `docs/CORTEX_V2_LIVE_VALIDATION_VERDICT_0.md`; `make live-compare` | operator-auth and automation-credential stabilization, then rerun `L1`-`L3` | partial | the current comparator now says lifecycle-first is promising but under-instrumented: one signed-in host-native lane is clearly valuable, one is blocked by auth freshness, one remains a watchlist, and the current service lane is still all-blocked on credentials |
