...
| `L2` Gemini live validation | `docs/CORTEX_V2_LIVE_VALIDATION_PROGRAM_0.md`; `docs/CORTEX_V2_LIVE_VALIDATION_VERDICT_0.md`; `make live-preflight`; `make live-provider-baselines`; focused Gemini operator reruns; `make live-cortex-host-control` | either accept Gemini as the remaining explicit partial host line, or open one bounded Gemini smoke-baseline stabilization/downgrade seam | partial | Gemini local truth is now sharper but still mixed: the installed CLI rejects `gemini-2.5-auto`, current repeated preflight probes fall through to a usable `gemini-2.5-flash-lite`, repeated operator smoke baselines on `gemini-2.5-flash` are currently `capacity_exhausted`, `pass_minimal` succeeds twice with warning-preserving `capacity_exhausted`, `truth_gap` remains `smoothed_incomplete` even on `gemini-2.5-flash-lite`, `restart_continuity` succeeds on `gemini-2.5-flash-lite` with warning-preserving `capacity_exhausted`, and exploratory `gemini-2.5-pro` smoke remains capacity-blocked; the automation lane is still blocked on missing ADC or `GEMINI_API_KEY` |
| `L2A` Gemini hook-backed operator lifecycle proof | `docs/CORTEX_V2_LIVE_VALIDATION_PROGRAM_0.md`; `docs/CORTEX_V2_LIVE_VALIDATION_VERDICT_0.md`; focused Gemini operator reruns | partial | partial | the documented Gemini hook surface is re-earned on the signed-in operator lane: `SessionStart`, `BeforeTool`, `AfterTool`, and `SessionEnd` are captured, but truth-gap honesty is still unearned, so the hook-backed line remains partial rather than landed |
...
| `L4` lifecycle-first payoff verdict | `docs/CORTEX_V2_LIVE_VALIDATION_VERDICT_0.md`; `make live-compare` | provide machine auth / spend approval and rerun the bounded service-proof lane | partial | the broader verdict remains partial because operator truth is strong on Claude and OpenAI while Gemini remains an explicit partial host line whose repeated smoke baselines are currently host-fragile, and the current service lane is still all-blocked on missing machine auth |
| `L5` cross-host operator payoff audit | `docs/CORTEX_V2_LIVE_OPERATOR_PAYOFF_AUDIT_0.md`; `make live-compare`; `make live-operator-payoff-audit` | closed | landed | operator-only payoff audit remains landed for current scope after strict reruns: repeated `make live-operator-payoff-audit` plus repeated `make live-compare` hold the same operator-only verdict, and the audit remains compatible with Gemini baseline fragility because it treats Gemini as explicit partial truth rather than a closed host line |
