...
| `R5` reference short-window feedback and sustained-pressure slice | `docs/CORTEX_V2_REFERENCE_FEEDBACK_PROGRAM_1.md`; `tests/unit/test_reference_feedback_window.py`; `tests/unit/test_reference_runtime_step.py`; `tests/unit/test_reference_executive_builder.py`; `tests/integration/test_reference_runtime_cli.py` | `R5B` | open | the next bounded opening is a three-step reference-only feedback window, a bounded prior-window summary, and the matching runtime/CLI trace surface; scorer law, host/runtime scope, runtime AUX activation, offline consolidation, and mediation remain closed in this program |
