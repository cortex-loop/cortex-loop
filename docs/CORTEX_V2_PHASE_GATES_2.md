...
| `L2` Gemini live validation | `docs/CORTEX_V2_LIVE_VALIDATION_PROGRAM_0.md`; `docs/CORTEX_V2_LIVE_VALIDATION_VERDICT_0.md`; `make live-preflight`; `make live-provider-baselines`; focused Gemini operator reruns; `make live-cortex-host-control` | accept Gemini as the remaining explicit partial host line on this machine unless host conditions materially change | partial | Gemini local truth is now better on both smoke and deeper operator surfaces: operator preflight and repeated smoke baselines are clean in CLI auto mode with no pinned `-m` model argument; the best-practice re-audit confirmed settings are already aligned, explicit project-scoped resume IDs are valid on the current CLI, and a lighter first-turn `plan` / resumed `yolo` split is now landed, but `restart_continuity` still fails to become repeat-stable because successful resumed completions coexist with recurring first-turn `capacity_exhausted` failures; exploratory `gemini-2.5-pro` smoke remains capacity-blocked and an exploratory `gemini-2.5-flash` continuity sidecar also failed on first-turn `capacity_exhausted`; the automation lane is deferred on this machine |
| `L2A` Gemini hook-backed operator lifecycle proof | `docs/CORTEX_V2_LIVE_VALIDATION_PROGRAM_0.md`; `docs/CORTEX_V2_LIVE_VALIDATION_VERDICT_0.md`; focused Gemini operator reruns | partial | partial | the documented Gemini hook surface is re-earned on the signed-in operator lane: `SessionStart`, `BeforeTool`, `AfterTool`, and `SessionEnd` are captured, `truth_gap` is now truthful on the latest auto-mode reruns, but `restart_continuity` remains mixed after the best-practice re-audit, so the hook-backed line stays partial rather than landed |
...
| `L4` lifecycle-first payoff verdict | `docs/CORTEX_V2_LIVE_VALIDATION_VERDICT_0.md`; `make live-compare` | keep service deferred on this machine; if work continues now, prefer a different bounded north-star seam rather than more Gemini micro-tuning | partial | the broader verdict remains partial because operator truth is strong on Claude and OpenAI while Gemini remains an explicit partial host line: the best-practice Gemini re-audit improved our diagnosis but did not close repeat stability, and the March 29 `N1` service reruns still leave the service lane deferred on this machine |
| `L5` cross-host operator payoff audit | `docs/CORTEX_V2_LIVE_OPERATOR_PAYOFF_AUDIT_0.md`; `make live-compare`; `make live-operator-payoff-audit` | closed | landed | operator-only payoff audit remains landed for current scope after the bounded `N1` service reruns: repeated `make live-operator-payoff-audit` plus repeated `make live-compare` keep the same operator-only verdict, and the compare surface now preserves accepted operator truth when this seam reruns automation-only artifacts |
