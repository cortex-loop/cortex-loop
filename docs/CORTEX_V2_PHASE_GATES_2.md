...
| `L6A` Claude service live proof | `docs/CORTEX_V2_LIVE_SERVICE_PROOF_0.md`; `make live-preflight`; `python3 tools/live_cortex_host_control.py --lane automation --provider claude`; `make live-compare` | deferred for current machine; if intentionally reopened later, provide machine auth or spend approval and rerun `L6A` twice | blocked | March 29 `N1` reruns confirm the Claude service lane is still auth-missing on this machine; signed-in Claude CLI truth does not satisfy service-lane auth, and the lane is now treated as deferred rather than the active next blocker |
| `L6B` Gemini service live proof | `docs/CORTEX_V2_LIVE_SERVICE_PROOF_0.md`; `make live-preflight`; `python3 tools/live_cortex_host_control.py --lane automation --provider gemini`; `make live-compare` | deferred for current machine; if intentionally reopened later, provide machine auth or spend approval and rerun `L6B` twice | blocked | March 29 `N1` reruns confirm the Gemini service lane is still auth-missing on this machine; signed-in Gemini CLI truth does not satisfy service-lane auth, and the lane is now treated as deferred rather than the active next blocker |
| `L6C` OpenAI service live proof | `docs/CORTEX_V2_LIVE_SERVICE_PROOF_0.md`; `make live-preflight`; `python3 tools/live_cortex_host_control.py --lane automation --provider openai`; `make live-compare` | deferred for current machine; if intentionally reopened later, provide machine auth or spend approval and rerun `L6C` twice | blocked | March 29 `N1` reruns confirm the OpenAI service lane is still auth-missing on this machine; signed-in Codex truth does not satisfy service-lane auth, and the lane is now treated as deferred rather than the active next blocker |
| `L6D` package-level service proof | `docs/CORTEX_V2_LIVE_SERVICE_PROOF_0.md`; `make live-compare` | `L6A`, `L6B`, and `L6C` must all land first if service proof is intentionally reopened later | blocked | package-level service proof remains unearned after the March 29 `N1` reruns, but the service lane is now explicitly deferred on this machine instead of driving the immediate next move |
