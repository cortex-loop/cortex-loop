...
| `L3` OpenAI live validation | `docs/CORTEX_V2_LIVE_VALIDATION_PROGRAM_0.md`; `docs/CORTEX_V2_LIVE_VALIDATION_VERDICT_0.md`; `make live-preflight`; `make live-provider-baselines`; `make live-openai-app-server`; `make live-cortex-host-control` | re-earn the first direct-API truth anchor on a capable machine and keep OpenAI CLI results watchlist-only until then | blocked | OpenAI currently contributes the strongest positive watchlist signal on the headless-CLI lane, but canonical direct-API truth is still blocked on missing `OPENAI_API_KEY` |
...
| `L4` lifecycle-first payoff verdict | `docs/CORTEX_V2_LIVE_VALIDATION_VERDICT_0.md`; `make live-compare` | move to a capable machine and re-earn the first canonical direct-API truth anchor before making broader payoff claims | blocked | after the R1 reset, broader payoff is read from the canonical direct-API lane first; this machine still blocks that lane on auth readiness, so watchlist-positive operator evidence does not close runtime payoff |
