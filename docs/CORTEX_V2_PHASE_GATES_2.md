...
| `L4` lifecycle-first payoff verdict | `docs/CORTEX_V2_LIVE_VALIDATION_VERDICT_0.md`; `make live-compare` | keep broader payoff partial until another direct-API host lands or package scope is explicitly narrowed | partial | after the R1 reset, broader payoff is read from the canonical direct-API lane first; OpenAI now re-earns that lane for current scope, but cross-host runtime payoff remains partial because Claude and Gemini are still unearned on the canonical lane |
...
| `L6A` Claude service live proof | `docs/CORTEX_V2_LIVE_SERVICE_PROOF_0.md`; `make live-preflight`; `python3 tools/live_cortex_host_control.py --lane automation --provider claude`; `make live-compare` | rerun `L6A` on a capable machine with `ANTHROPIC_API_KEY` and explicit spend approval; require two successful bounded reruns before closure | blocked | the current machine remains out of scope for actual Claude service proof because machine auth readiness is not yet satisfied; signed-in Claude CLI truth still does not count as service-lane auth |
