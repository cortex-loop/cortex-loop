...
| `G1` Gemini documented host-event runtime shell | `docs/CORTEX_V2_GEMINI_RUNTIME_PROGRAM_0.md`; `tests/unit/test_gemini_runtime_session_io.py`; `tests/unit/test_gemini_runtime_step.py`; `tests/unit/test_gemini_runtime_ownership.py`; `tests/integration/test_gemini_runtime_cli.py`; `tests/integration/test_gemini_runtime_continuity.py` | local branch closeout pending acceptance | partial | Gemini-specific runtime/session carriers plus persisted artifact are real on branch-local G1 candidate truth over accepted K3 baseline `efe003e`; raw documented Gemini host events drive a host-specific CLI shell and reuse the accepted K3 allocation diagnostics without opening a second SRE doctrine |
| `G2` Gemini raw-transcript ingress shell | `docs/CORTEX_V2_GEMINI_INGRESS_PROGRAM_0.md`; `tests/unit/test_gemini_ingress.py`; `tests/integration/test_gemini_ingress_cli.py`; `tests/integration/test_gemini_ingress_continuity.py` | local branch closeout pending acceptance | partial | Gemini raw-transcript ingress parsing is real on branch-local G1 candidate truth; transcript records with `type` drive the current-line Gemini runtime shell, wrapper-shaped and mixed wrapper/transcript records are explicitly rejected, and split-run ingress continuity is proven for current scope |
| `G3` Gemini loopback service shell | `docs/CORTEX_V2_GEMINI_SERVICE_PROGRAM_0.md`; `tests/unit/test_gemini_service.py`; `tests/integration/test_gemini_service.py`; `tests/integration/test_gemini_service_continuity.py` | local branch closeout pending acceptance | partial | loopback-only Gemini HTTP is real on branch-local G1 candidate truth; `POST /v1/events` drives the current-line Gemini ingress parser and Gemini runtime shell, artifact import/export remains JSON-only, and one active session per process is real for current scope |
| `G4` Gemini bounded outbound host-control lane | `docs/CORTEX_V2_GEMINI_HOST_CONTROL_PROGRAM_0.md`; `tests/unit/test_gemini_host_control.py`; `tests/integration/test_gemini_host_control_service.py`; `tests/integration/test_gemini_host_control_continuity.py` | local branch closeout pending acceptance | partial | the first bounded outbound Gemini host-control lane is real on branch-local G1 candidate truth; `POST /v1/actions/interaction-stream` is text-only and strict-whitelist for current scope, returned host events re-enter the current-line Gemini ingress parser and Gemini runtime shell directly, and the stdlib transport has an internal fixture mode so the canonical bundle requires no live network |
