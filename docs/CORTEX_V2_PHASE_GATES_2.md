...
| `O4R` OpenAI verified-work restoration lane | `docs/CORTEX_V2_OPENAI_VERIFIED_WORK_PROGRAM_0.md`; `make revalidate-openai-host-control` | repeat-stable local larger-task reruns still required before closeout | partial | the shared verified-work law, runtime-native verification binding, deterministic `full_files` verifier/profile, and one bounded repair turn remain implemented on top of the existing OpenAI host-control family, the invariant verified-work runtime helpers now live in a neutral runtime home, and the corrected conformance rerun still left the OpenAI `service_api` shipping default at `test_failed` after bounded repair; `O4R` therefore remains partial and explicitly outside the current canonical-anchor proof bundle |
...
| `CT2` active verified-work tri-brain conformance | `docs/CORTEX_V2_ACTIVE_WORKSTREAM.md`; `make conformance-preflight`; `make conformance-fast`; local artifacts under `.cortex/live_validation/conformance/` | keep partial until the OpenAI shipping-default lane either earns repeat-stable larger-task lift or is explicitly cut back; reopen Claude wiring only if it re-enters the divergent bucket after provider overload clears | partial | the corrected active bookmarks verified-work pack now reads OpenAI `service_api` partial because the bounded repair path still finishes `test_failed`, Claude `operator_cli` no longer divergent because truthful staged-workspace reruns pass while repeated full tri-brain reruns currently fail only with Anthropic `529 overloaded_error` and therefore count as `env_blocked`, and Gemini `operator_cli` conformant because staged workspace truth removes the earlier `read_file` miss; current next decision is `improve_shipping_default` because no non-shipping divergent surface remains |
