...
| `O2` OpenAI raw-transcript ingress shell | `docs/CORTEX_V2_OPENAI_INGRESS_PROGRAM_0.md`; `tests/unit/test_openai_ingress.py`; `tests/integration/test_openai_ingress_cli.py`; `tests/integration/test_openai_ingress_continuity.py` | local branch closeout pending acceptance | partial | raw-transcript ingress parsing now exists on `codex/o2-openai-ingress-shell`, transcript records with `type` drive the accepted `O1` shell, wrapper-shaped and mixed wrapper/transcript records are explicitly rejected, canonical Cortex event names are explicitly rejected at ingress, and split-run ingress continuity is proven against the recorded `O2` contract; accepted baseline truth remains anchored at accepted `O1` closeout head `93f7093` until a later accepted baseline update lands |
