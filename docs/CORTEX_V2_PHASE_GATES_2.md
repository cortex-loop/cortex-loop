...
| `L5` cross-host operator payoff audit | `docs/CORTEX_V2_LIVE_OPERATOR_PAYOFF_AUDIT_0.md`; `make live-compare`; `make live-operator-payoff-audit` | refresh operator evidence and rerun `L5` twice | partial | operator-only payoff audit is now the current train; it may land even while `L2` remains partial, because `L5` interprets frozen operator truth rather than claiming new host closure |
