...
| `L2` Gemini live validation | `docs/CORTEX_V2_LIVE_VALIDATION_PROGRAM_0.md`; `docs/CORTEX_V2_LIVE_VALIDATION_VERDICT_0.md`; `make live-preflight`; `make live-provider-baselines`; focused Gemini operator reruns; `make live-cortex-host-control` | rerun on `gemini-2.5-flash` again or narrow the local fallback to `gemini-2.5-flash-lite`, then rerun `L2` | partial | Gemini local truth is now narrower and clearer: the installed CLI rejects `gemini-2.5-auto`, probe and baseline are clean on explicit fallback `gemini-2.5-flash`, `pass_minimal` succeeds twice with warning-preserving `capacity_exhausted`, `truth_gap` remains `smoothed_incomplete`, and `restart_continuity` still blocks on `capacity_exhausted`; the automation lane is still blocked on missing ADC or `GEMINI_API_KEY` |
| `L2A` Gemini hook-backed operator lifecycle proof | `docs/CORTEX_V2_LIVE_VALIDATION_PROGRAM_0.md`; `docs/CORTEX_V2_LIVE_VALIDATION_VERDICT_0.md`; focused Gemini operator reruns | partial | partial | the documented Gemini hook surface is now re-earned on the signed-in operator lane: `SessionStart`, `BeforeTool`, `AfterTool`, and `SessionEnd` are captured, but the lane is still not repeat-stable enough to close because truth-gap honesty and restart continuity remain unearned |
