...
| Gate row | Current evidence | Owner / next closeout | Status | Notes |
| --- | --- | --- | --- | --- |
| `L1` Claude live validation | `docs/CORTEX_V2_LIVE_VALIDATION_PROGRAM_0.md`; `docs/CORTEX_V2_LIVE_VALIDATION_VERDICT_0.md`; `make live-preflight`; `make live-provider-baselines`; focused Claude operator reruns; `python3 tools/live_cortex_host_control.py --lane automation --provider claude --suite canonical_anchor` | re-earn Claude direct-API truth on a capable machine and keep Claude CLI results watchlist-only until then | blocked | Claude currently contributes positive watchlist evidence on the headless-CLI lane, the shared `canonical_anchor` direct-API suite is implemented for Claude on the accepted line, and canonical truth remains blocked on missing `ANTHROPIC_API_KEY` |
...
| `L4` lifecycle-first payoff verdict | `docs/CORTEX_V2_LIVE_VALIDATION_VERDICT_0.md`; `make live-compare` | keep broader payoff partial until another direct-API host lands or package scope is explicitly narrowed | partial | after the R1 reset, broader payoff is read from the canonical direct-API lane first; OpenAI now re-earns that lane for current scope through three positive current-machine `canonical_anchor` cycles, Claude shares the same canonical suite implementation but remains auth-blocked, and cross-host runtime payoff therefore remains partial |
...
| `L6A` Claude service live proof | `docs/CORTEX_V2_LIVE_SERVICE_PROOF_0.md`; `make live-preflight`; `python3 tools/live_cortex_host_control.py --lane automation --provider claude --suite canonical_anchor`; `make live-compare` | rerun `L6A` on a capable machine with `ANTHROPIC_API_KEY` and explicit spend approval; require two successful bounded reruns before closure | blocked | the current machine remains out of scope for actual Claude service proof because machine auth readiness is not yet satisfied; the shared `canonical_anchor` direct-API suite is implemented for Claude on the accepted line, but signed-in Claude CLI truth still does not count as service-lane auth |
...
| `L6D` package-level service proof | `docs/CORTEX_V2_LIVE_SERVICE_PROOF_0.md`; `make live-compare` | `L6A` and `L6B` must still land on a capable machine first | blocked | package-level service proof remains unearned even though `L6C` now lands; Claude and Gemini still block cross-host service closure, with Claude now implemented on the shared `canonical_anchor` path but still missing direct auth |
