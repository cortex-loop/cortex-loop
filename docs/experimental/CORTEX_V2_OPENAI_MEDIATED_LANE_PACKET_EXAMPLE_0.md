# CORTEX_V2_OPENAI_MEDIATED_LANE_PACKET_EXAMPLE_0

Surface: experimental

Status: committed OpenAI-mediated lane packet example (`active`, evidence)
Date: 2026-03-21

Purpose:
- record one real contradiction-preserving mediated OpenAI-host packet example built from landed surfaces,
...
Source test:
- `tests/integration/test_openai_mediated_lane_packet_example.py::test_openai_mediated_lane_current_pair_packet_example_matches_committed_doc`

Earned in this example:
- the same OpenAI-host candidate-bearing event plus full-commitment event -> commitment verdict -> event trace artifact -> current-pair fragment -> eval harness -> eval packet path as the baseline example,
...
```json
{
  ...
  "event_trace": {
    "trace_id": "openai-mediated-lane:openai-host-packet-candidate-1",
    "event_refs": [
