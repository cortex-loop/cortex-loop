...
This document records the raw-transcript ingress slice inside the accepted G1 Claude runtime/product parity line.

The chosen next move is:

- one Claude raw-transcript ingress parser,
- one Claude ingress CLI over raw host transcript records,
- one bounded ingress continuity proof on top of the current-line Claude runtime shell,
...
- the current line now includes one real Claude host-event shell,
...
- and it is a smaller truthful step than Claude breadth or multi-agent orchestration.
...
- `python3 -m experimental.runtime.claude_ingress_cli`
...
The Claude ingress shell exposes:

- `python3 -m experimental.runtime.claude_ingress_cli`

Input remains JSONL via stdin or `--event-file`.

Each input line must be one raw Claude-style transcript object with:
...
Output remains exactly the current-line Claude runtime record shape and field order.

## Runtime law for this program

The ingress shell may:

- parse raw transcript objects into `ClaudeHostEventEnvelope`,
- call accepted `run_claude_runtime_step(envelope.event_type, envelope.payload, session)`,
- reuse the current-line Claude session persistence unchanged,
- and emit the existing Claude runtime projection without widening the runtime shell itself.
...
For a transcript mechanically equivalent to the current-line Claude dev-shell event stream, `A2` equivalence means:
...
As in `C1` and `A1`, exact byte-for-byte replay of `session_summary.budget_history` and `session_summary.brake_history` is not required.
...
`A2` is only honestly closed when all are true:
...
- documented raw transcript records drive the current-line `A1` shell without changing its output contract,
...
- `python3 -m experimental.runtime.claude_ingress_cli` now drives the current-line `A1` runtime shell from raw transcript records
...
- repeated direct reruns plus repeated `make revalidate-claude-ingress` passed for current scope
