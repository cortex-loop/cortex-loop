...
- `python3 -m experimental.runtime.gemini_service`
...
- Gemini host-control doctrine beyond this bounded service shell,
...
The Gemini loopback service shell exposes:
...
As in `C1`, `G1`, and `G2`, exact byte-for-byte replay of `session_summary.budget_history` and `session_summary.brake_history` is not required.
...
- `GeminiServiceState`, `handle_gemini_service_request()`, `export_gemini_service_session()`, `import_gemini_service_session()`, and `python3 -m experimental.runtime.gemini_service` are now landed `G3` surfaces, implemented at G1 proof head `fe33a7e`
