...
The reference host-realization pair is now backed by `docs/CORTEX_V2_MEDIATION_REFERENCE_HOST_REALIZATION_REPLICATION_NOTE_0.md`, which records the first counted reference host-realization pair and its fairness law.
One reference-only mediation-specific host-realization pair is now recorded for `scenario_host_reference_01`, but the cell remains `insufficient` because one pair is below the three-pair minimum.
