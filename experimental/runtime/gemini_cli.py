"""Developer-facing local CLI for the Gemini documented host-event runtime shell."""
...
from experimental.drivers.gemini_host import is_raw_gemini_host_event_name

from .gemini import GeminiRuntimeSession, GeminiRuntimeStepResult, run_gemini_runtime_step
...
def build_gemini_cli_record(step_result: GeminiRuntimeStepResult) -> dict[str, Any]:
    if not isinstance(step_result, GeminiRuntimeStepResult):
        actual_type = type(step_result).__name__
        raise TypeError(
            "build_gemini_cli_record.step_result must be GeminiRuntimeStepResult, "
            f"got {actual_type}."
        )
    payload_metadata = {
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        prog="python3 -m experimental.runtime.gemini_cli",
        description="Process raw Gemini-host runtime events from JSONL input.",
    )
    parser.add_argument(
        "--event-file",
        type=Path,
    ...
    try:
        ...
        records, final_session = _run_gemini_cli_lines_with_session(lines, initial_session)
        ...
        print(f"gemini_cli error: {exc}", file=sys.stderr)
        return 1
    return 0


def _run_gemini_cli_lines(lines: Iterable[str]) -> list[dict[str, Any]]:
    records, _ = _run_gemini_cli_lines_with_session(lines)
    return records


def _run_gemini_cli_lines_with_session(
    lines: Iterable[str],
    ...
    for event_name, payload in _iter_gemini_cli_events(lines):
        step_result = run_gemini_runtime_step(event_name, payload, current_session)
        record = build_gemini_cli_record(step_result)
        if tuple(record) != _OUTPUT_KEYS:
            raise ValueError("Gemini CLI record must preserve the locked output field order.")
...
def _iter_gemini_cli_events(lines: Iterable[str]) -> Iterator[tuple[str, dict[str, Any]]]:
    for line_number, raw_line in enumerate(lines, start=1):
        ...
        if not is_raw_gemini_host_event_name(event_name):
            raise ValueError(
                f"line {line_number}: event_name must be a raw Gemini host event name, "
...
def _coerce_cli_session(session: GeminiRuntimeSession | None) -> GeminiRuntimeSession:
    ...
    if not isinstance(session, GeminiRuntimeSession):
        ...
        raise TypeError(
            "gemini_cli initial session must be GeminiRuntimeSession | None, "
            f"got {actual_type}."
        )
    return session
