"""Self-contained verified-work protocol and verifier runner for Cortex v3."""
...
def build_verified_work_input_text(
    ...
    *,
    ...
    writable_file_map: Mapping[str, str] | None = None,
    visible_context_paths: tuple[str, ...] | None = None,
) -> str:
    ...
    return (
        ...
        f"{_build_verified_work_context_bundle(work_contract, context_mode=context_mode, writable_file_map=writable_file_map, visible_context_paths=visible_context_paths)}"
    )
...
def build_verified_work_repair_prompt(
    task_prompt: str,
    repair_contract: WorkContract,
    preservation_state: PreservationState,
    *,
    writable_file_map: Mapping[str, str] | None,
    context_mode: VerifiedWorkContextMode = "default",
    visible_context_paths: tuple[str, ...] | None = None,
) -> str:
    repair_intro = (
        f"{task_prompt.strip()}\n\n"
        "The previous attempt failed external verification. Repair the candidate workspace using the factual ticket below.\n\n"
        f"{build_verified_work_repair_ticket(preservation_state)}"
    )
    return build_verified_work_input_text(
        repair_intro,
        repair_contract,
        context_mode=context_mode,
        writable_file_map=writable_file_map,
        visible_context_paths=visible_context_paths,
    )
...
def verify_verified_work_result(
    ...
    *,
    ...
    file_map, blocked_outcome = parse_verified_work_result(result_text, work_contract)
    if blocked_outcome is not None:
        return file_map, blocked_outcome
    assert file_map is not None
    effective_file_map = overlay_verified_work_file_map(
        preserved_file_map,
        file_map,
    )
...
def overlay_verified_work_file_map(
...
def parse_verified_work_result(
    result_text: str | None,
    work_contract: WorkContract,
...
def _build_verified_work_context_bundle(
    ...
    *,
    ...
    normalized_visible_paths = _normalized_context_paths(
        visible_context_paths or work_contract.allowed_write_paths
    )
    if context_mode == "writable_files_only":
        context_paths = normalized_visible_paths
    else:
        context_paths = normalized_visible_paths + profile.read_only_context_paths
    rendered_blocks: list[str] = []
    for relative_path in context_paths:
        file_text = _context_file_text(
            relative_path,
            profile,
            writable_file_map=writable_file_map,
        )
...
def _normalized_context_paths(paths: tuple[str, ...]) -> tuple[str, ...]:
    normalized: list[str] = []
    for path in paths:
        if not (isinstance(path, str) and path.strip()):
            raise ValueError(
                "_normalized_context_paths.paths must contain only non-empty strings."
            )
        stripped = path.strip()
        if stripped.startswith("/") or stripped.startswith("../") or "/../" in stripped:
            raise ValueError(
                "_normalized_context_paths.paths must contain only bounded relative paths."
            )
        if stripped not in normalized:
            normalized.append(stripped)
    if not normalized:
        raise ValueError("_normalized_context_paths.paths must be non-empty.")
    return tuple(normalized)


def _context_file_text(
    relative_path: str,
    profile: VerifiedWorkProfileSpec,
    *,
    writable_file_map: Mapping[str, str] | None,
) -> str:
    if writable_file_map is not None and relative_path in writable_file_map:
        content = writable_file_map[relative_path]
        if not isinstance(content, str):
            raise TypeError(
                "_context_file_text.writable_file_map values must be strings."
            )
        return content
...
def _run_verified_work_verifier(
    ...
    profile = verified_work_profile_spec(work_contract)
    with tempfile.TemporaryDirectory(prefix="cortex-v3-verified-work-") as tmpdir:
...
def verified_work_profile_spec(work_contract: WorkContract) -> VerifiedWorkProfileSpec:
    try:
...
__all__ = [
    ...
    "overlay_verified_work_file_map",
    "parse_verified_work_result",
    "verified_work_profile_spec",
