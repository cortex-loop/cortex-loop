...
def test_build_scenario_catalog_exposes_l2_harness_contract() -> None:
    ...
    assert catalog["shared_template_root"] == "tests/lab/fixtures/live_validation/project_template"
    ...
    assert catalog["codex_dogfood_profiles"]["repo_any_task"]["task_scope"] == "any_repo_work"
    assert (
        catalog["codex_dogfood_profiles"]["repo_any_task"]["session_start_prompt"]
        == "repo_codex_app_dogfood_session_start.md"
    )
    assert (
        catalog["codex_dogfood_profiles"]["repo_any_task"]["closeout_prompt"]
        == "repo_codex_app_dogfood_closeout.md"
    )
    assert (
        catalog["codex_dogfood_profiles"]["repo_any_task"]["start_session_command"]
...
def test_read_workstream_baseline_resolves_commit_from_branch_lookup(monkeypatch, tmp_path: Path) -> None:
    status = tmp_path / "cortex_status.json"
    status.write_text(
        '{\n'
        '  "resting_state": {\n'
        '    "branch": "main",\n'
    ...
    monkeypatch.setattr(live_validation_common, "_resolve_git_ref", lambda branch: f"{branch}-resolved")
