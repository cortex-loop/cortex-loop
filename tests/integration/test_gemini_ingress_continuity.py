"""Integration tests for Gemini ingress continuity over raw transcript records."""
...
FIXTURE_PATH = (
    ...
    / "integration"
    / "fixtures"
    / "gemini_ingress_continuity_session.jsonl"
)
...
def test_gemini_ingress_split_session_is_g2_equivalent_to_uninterrupted_run(tmp_path: Path) -> None:
    ...
    one_process_completed = _run_gemini_ingress_cli(
        "--event-file",
        str(FIXTURE_PATH),
        "--save-session",
        str(one_process_artifact),
    )
    split_first_completed = _run_gemini_ingress_cli(
        "--save-session",
        str(split_seed_artifact),
        input_text="\n".join(fixture_lines[:2]) + "\n",
    )
    split_second_completed = _run_gemini_ingress_cli(
        "--load-session",
        str(split_seed_artifact),
        "--save-session",
...
def test_gemini_ingress_continuity_rejection_survives_restart(tmp_path: Path) -> None:
    artifact_path = tmp_path / "session.json"

    first_completed = _run_gemini_ingress_cli(
        "--save-session",
        str(artifact_path),
        input_text='{"type":"content.delta","session_id":"gm-ingress-reject","interaction_id":"gm-int-1","branch_operation":"open","branch_track_ref":"branch-alpha","delta":"open"}\n',
    )
    second_completed = _run_gemini_ingress_cli(
        "--load-session",
        str(artifact_path),
        input_text='{"type":"content.delta","session_id":"gm-ingress-reject","interaction_id":"gm-int-1","branch_operation":"resume","branch_track_ref":"branch-alpha","delta":"resume"}\n',
    )
...
def test_gemini_ingress_host_warning_and_certified_commitment_can_coexist_across_restart(
    tmp_path: Path,
) -> None:
    artifact_path = tmp_path / "session.json"
    ...
    first_completed = _run_gemini_ingress_cli(
        "--save-session",
        str(artifact_path),
        input_text='{"type":"content.delta","session_id":"gm-ingress-warning","interaction_id":"gm-int-1","delta":"seed"}\n',
    )
    second_completed = _run_gemini_ingress_cli(
        "--load-session",
        str(artifact_path),
        input_text='{"type":"content.tool_event","session_id":"gm-ingress-warning","interaction_id":"gm-int-1","commitment_id":"gm-ingress-warning-commit","externally_consequential":true,"result_artifact_ref":"gm-ingress-warning-artifact"}\n',
    )
...
def _run_gemini_ingress_cli(
    *args: str,
    ...
    return subprocess.run(
        [sys.executable, "-m", "experimental.runtime.gemini_ingress_cli", *args],
        cwd=REPO_ROOT,
        input=input_text,
        capture_output=True,
        text=True,
