"""Integration tests for OpenAI ingress continuity over raw transcript records."""
...
def test_openai_ingress_split_session_is_o2_equivalent_to_uninterrupted_run(tmp_path: Path) -> None:
    ...
    one_process_completed = _run_openai_ingress_cli(
        "--event-file",
        str(FIXTURE_PATH),
        "--save-session",
        str(one_process_artifact),
    )
    split_first_completed = _run_openai_ingress_cli(
        "--save-session",
        str(split_seed_artifact),
        input_text="\n".join(fixture_lines[:2]) + "\n",
    )
    split_second_completed = _run_openai_ingress_cli(
        "--load-session",
        str(split_seed_artifact),
        "--save-session",
...
def test_openai_ingress_continuity_rejection_survives_restart(tmp_path: Path) -> None:
    ...
    first_completed = _run_openai_ingress_cli(
        "--save-session",
        str(artifact_path),
        input_text='{"type":"response.output_text.delta","session_id":"oa-ingress-reject","response_id":"resp-1","branch_operation":"open","branch_track_ref":"branch-alpha","delta":"open"}\n',
    )
    second_completed = _run_openai_ingress_cli(
        "--load-session",
        str(artifact_path),
        input_text='{"type":"response.output_text.delta","session_id":"oa-ingress-reject","response_id":"resp-1","branch_operation":"resume","branch_track_ref":"branch-alpha","delta":"resume"}\n',
    )
...
def test_openai_ingress_host_warning_and_certified_commitment_can_coexist_across_restart(
    ...
    first_completed = _run_openai_ingress_cli(
        "--save-session",
        str(artifact_path),
        input_text='{"type":"response.output_text.delta","session_id":"oa-ingress-warning","response_id":"resp-1","delta":"seed"}\n',
    )
    second_completed = _run_openai_ingress_cli(
        "--load-session",
        str(artifact_path),
        input_text='{"type":"response.tool_event","session_id":"oa-ingress-warning","response_id":"resp-1","commitment_id":"oa-ingress-warning-commit","externally_consequential":true,"result_artifact_ref":"oa-ingress-warning-artifact"}\n',
    )
