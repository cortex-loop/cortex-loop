...
def test_gemini_host_realization_mediated_packet_matches_committed_doc() -> None:
    committed_packet = packet_without_path(
        parse_run_packet(REPO_ROOT / GEMINI_HOST_REALIZATION_MEDIATED_PACKET_PATH)
    )

    assert build_gemini_host_realization_mediated_packet() == committed_packet
...
def test_gemini_host_realization_pair_remains_fair_and_changes_only_specialization() -> None:
    ...
    baseline_packet = build_gemini_host_realization_baseline_packet()
    mediated_packet = build_gemini_host_realization_mediated_packet()
    comparator = build_gemini_host_realization_comparator_snapshot()
    ...
    assert comparator["baseline_trace_id"] != comparator["mediated_trace_id"]
