"""Integration tests for Claude runtime continuity over documented host events."""
...
FIXTURE_PATH = (
    ...
    / "integration"
    / "fixtures"
    / "claude_runtime_continuity_session.jsonl"
)


def test_claude_runtime_split_session_is_g1_equivalent_to_uninterrupted_run(tmp_path: Path) -> None:
    ...
    one_process_completed = _run_claude_cli(
        "--event-file",
        str(FIXTURE_PATH),
        "--save-session",
        str(one_process_artifact),
    )
    split_first_completed = _run_claude_cli(
        "--save-session",
        str(split_seed_artifact),
        input_text="\n".join(fixture_lines[:2]) + "\n",
    )
    split_second_completed = _run_claude_cli(
        "--load-session",
        str(split_seed_artifact),
        "--save-session",
...
def test_claude_runtime_continuity_rejection_survives_restart(tmp_path: Path) -> None:
    artifact_path = tmp_path / "session.json"

    first_completed = _run_claude_cli(
        "--save-session",
        str(artifact_path),
        input_text='{"event_name":"content_block_delta","payload":{"session_id":"cl-reject","message_id":"cl-msg-1","branch_operation":"open","branch_track_ref":"branch-alpha","delta":"open"}}\n',
    )
    second_completed = _run_claude_cli(
        "--load-session",
        str(artifact_path),
        input_text='{"event_name":"content_block_delta","payload":{"session_id":"cl-reject","message_id":"cl-msg-1","branch_operation":"resume","branch_track_ref":"branch-alpha","delta":"resume"}}\n',
    )
...
def test_claude_runtime_host_warning_and_certified_commitment_can_coexist_across_restart(
    tmp_path: Path,
) -> None:
    artifact_path = tmp_path / "session.json"
    ...
    first_completed = _run_claude_cli(
        "--save-session",
        str(artifact_path),
        input_text='{"event_name":"content_block_delta","payload":{"session_id":"cl-host-warning","message_id":"cl-msg-1","delta":"seed"}}\n',
    )
    second_completed = _run_claude_cli(
        "--load-session",
        str(artifact_path),
        input_text='{"event_name":"content_block_magic","payload":{"session_id":"cl-host-warning","message_id":"cl-msg-1","commitment_id":"cl-host-warning-commit","externally_consequential":true,"result_artifact_ref":"cl-host-warning-artifact"}}\n',
    )
