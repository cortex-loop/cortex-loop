"""Integration revalidation for the first OpenAI-only mediated host-realization pair."""
...
from tests.integration._openai_mediation_host_realization_experimental import (
    OPENAI_HOST_REALIZATION_MEDIATED_PACKET_DOC_BUILDERS,
    OPENAI_HOST_REALIZATION_MEDIATED_PACKET_PATH,
    build_openai_host_realization_comparator_snapshot,
    build_openai_host_realization_mediated_packet,
    emit_openai_mediated_host_realization_candidate,
)


def test_openai_host_realization_mediated_packet_matches_committed_doc() -> None:
    committed_packet = packet_without_path(
        parse_run_packet(REPO_ROOT / OPENAI_HOST_REALIZATION_MEDIATED_PACKET_PATH)
    )

    assert build_openai_host_realization_mediated_packet() == committed_packet


def test_openai_host_realization_pair_remains_fair_and_changes_only_specialization() -> None:
    ...
    baseline_packet = build_openai_host_realization_baseline_packet()
    mediated_packet = build_openai_host_realization_mediated_packet()
    comparator = build_openai_host_realization_comparator_snapshot()

    assert baseline_packet["header"]["scenario_id"] == mediated_packet["header"]["scenario_id"]
    assert baseline_packet["header"]["paired_episode_set_id"] == "pair_openai_host_001"
    assert mediated_packet["header"]["paired_episode_set_id"] == "pair_openai_host_001"
    assert baseline_packet["header"]["run_id"] == "openai_host_realization_baseline_run_001"
    assert mediated_packet["header"]["run_id"] == "openai_host_realization_mediated_run_001"
    assert baseline_packet["variant_metadata"]["host_family"] == "openai"
    ...
    assert contradiction_ref["source_tag"] == "openai-host-publication-check"
    assert (
        contradiction_ref["summary"]
        == "OpenAI host publication evidence remains partially withheld"
    )
    assert degradation_ref["reason_code"] == "openai-host-publication-partial"
    assert comparator["session_id"] == "openai-host-packet-session-1"
    assert comparator["candidate_id"] == "openai-host-packet-candidate-1"
    assert comparator["commitment_id"] == "openai-host-packet-commit-1"
    assert comparator["provenance_artifact_id"] == "openai-host-artifact-1"


def test_openai_host_realization_mediated_candidate_emitter_prints_markdown(
    capsys, tmp_path: Path
) -> None:
    emit_openai_mediated_host_realization_candidate()
    captured = capsys.readouterr().out

    for relative_path in OPENAI_HOST_REALIZATION_MEDIATED_PACKET_DOC_BUILDERS:
        assert f"--- {relative_path}" in captured

    emitted_docs = _parse_emitted_docs(captured)
    assert set(emitted_docs) == set(OPENAI_HOST_REALIZATION_MEDIATED_PACKET_DOC_BUILDERS)
