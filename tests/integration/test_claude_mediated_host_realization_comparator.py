"""Integration revalidation for the first Claude-only mediated host-realization pair."""
...
from tests.integration._claude_mediation_baseline_packets import (
    CLAUDE_HOST_REALIZATION_BASELINE_PACKET_PATHS,
    build_claude_host_realization_baseline_packet,
)
from tests.integration._claude_mediation_host_realization_experimental import (
    CLAUDE_HOST_REALIZATION_MEDIATED_PACKET_DOC_BUILDERS,
    CLAUDE_HOST_REALIZATION_MEDIATED_PACKET_PATHS,
    build_claude_host_realization_comparator_snapshot,
    build_claude_host_realization_mediated_packet,
    emit_claude_mediated_host_realization_candidate,
)


def test_claude_host_realization_mediated_packet_matches_committed_doc() -> None:
    for pair_key in CLAUDE_HOST_REALIZATION_PAIR_KEYS:
        committed_packet = packet_without_path(
            parse_run_packet(REPO_ROOT / CLAUDE_HOST_REALIZATION_MEDIATED_PACKET_PATHS[pair_key])
        )

        assert build_claude_host_realization_mediated_packet(pair_key) == committed_packet


def test_claude_host_realization_pair_remains_fair_and_changes_only_specialization() -> None:
    ...
    for pair_key in CLAUDE_HOST_REALIZATION_PAIR_KEYS:
        ...
        baseline_packet = build_claude_host_realization_baseline_packet(pair_key)
        mediated_packet = build_claude_host_realization_mediated_packet(pair_key)
        comparator = build_claude_host_realization_comparator_snapshot(pair_key)
        ...
        assert baseline_packet["variant_metadata"]["host_family"] == "claude"
        ...
        assert comparator["message_id"] == spec.message_id
        ...
        seen_ids["message_ids"].add(spec.message_id)
        seen_ids["candidate_ids"].add(spec.commitment_id)
    ...
    for values in seen_ids.values():
        assert len(values) == len(CLAUDE_HOST_REALIZATION_PAIR_KEYS)


def test_claude_host_realization_mediated_candidate_emitter_prints_markdown(
    capsys, tmp_path: Path
) -> None:
    emit_claude_mediated_host_realization_candidate()
    captured = capsys.readouterr().out

    for relative_path in CLAUDE_HOST_REALIZATION_MEDIATED_PACKET_DOC_BUILDERS:
        assert f"--- {relative_path}" in captured

    emitted_docs = _parse_emitted_docs(captured)
    assert set(emitted_docs) == set(CLAUDE_HOST_REALIZATION_MEDIATED_PACKET_DOC_BUILDERS)
...
def _parse_emitted_docs(output: str) -> dict[str, str]:
    ...
    for line in output.splitlines():
        if line.startswith("--- docs/mediation_evidence/claude/"):
