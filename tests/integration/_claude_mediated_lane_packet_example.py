"""Build or emit candidate Claude mediated-lane packet-example evidence."""
...
from tests.integration._claude_mediation_common import claude_environment_handle
...
def build_claude_host_realization_specialization_snapshot(
    *,
...
def build_claude_mediated_lane_packet_example_snapshot(
    ...
    event_trace = EventTraceArtifact(
        trace_id=f"claude-mediated-lane:{result.candidate.candidate_id}",
        event_refs=(
...
def emit_claude_mediated_lane_packet_example_snapshot() -> None:
    json.dump(build_claude_mediated_lane_packet_example_snapshot(), sys.stdout, indent=2)
    sys.stdout.write("\n")
...
if __name__ == "__main__":
    emit_claude_mediated_lane_packet_example_snapshot()
