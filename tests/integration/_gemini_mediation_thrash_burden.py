"""Build Gemini-only AUX burden artifacts for thrash mediation evidence."""
...
def build_gemini_thrash_burden_artifact(
    *,
    pair_id: str,
    pair_key: str,
    run_id: str,
    ...
    burden = AuxBurdenReport(
        ...
        metadata=(
            MetadataField("scenario_id", "scenario_thrash_gemini_01"),
...
def render_gemini_thrash_burden_artifact(
    relative_path: str,
...
def emit_gemini_thrash_burden_artifacts(
    ...
    for index, (relative_path, builder) in enumerate(builders.items()):
        ...
        sys.stdout.write(render_gemini_thrash_burden_artifact(relative_path, builder()))
...
def _scope_text(variant: str) -> str:
    if variant == "baseline_non_mediated":
        return (
            "This committed AUX burden artifact records one Gemini-host baseline-only "
            "thrash burden measurement within the committed Gemini thrash paired-run "
    ...
    return (
        "This committed AUX burden artifact records one Gemini-host experimental "
        "mediated thrash burden measurement within the committed Gemini thrash "
