"""Unit tests for bounded Gemini runtime session artifact I/O."""
...
def test_gemini_runtime_session_artifact_roundtrips_bounded_residue() -> None:
    ...
    assert payload["artifact_kind"] == "gemini-runtime-session"
...
def test_gemini_runtime_session_artifact_normalizes_legacy_continuity_reminders() -> None:
...
def test_gemini_runtime_session_artifact_accepts_legacy_continuity_truth_shape() -> None:
    payload = _base_payload()
...
def test_gemini_runtime_session_artifact_rejects_unknown_keys_and_invalid_enums() -> None:
    payload = _base_payload()
    payload["extra"] = {}
    with pytest.raises(ValueError, match="extra"):
        parse_gemini_runtime_session_artifact(payload)
...
def test_gemini_runtime_session_artifact_one_sided_last_feedback_normalizes_through_session_constructor() -> None:
    payload = _base_payload()
...
def test_gemini_runtime_session_artifact_same_path_overwrite_safety(tmp_path) -> None:
    session = GeminiRuntimeSession(
        session_id="gm-file",
        event_index=1,
    ...
    write_gemini_runtime_session_artifact(path, session)
    original_payload = json.loads(path.read_text(encoding="utf-8"))
    restored = read_gemini_runtime_session_artifact(path)

    assert original_payload["artifact_kind"] == "gemini-runtime-session"
    assert restored.session_id == "gm-file"
    ...
    write_gemini_runtime_session_artifact(path, updated_session)
...
def test_gemini_runtime_session_artifact_roundtrips_feedback_task_mode_and_probe_result_class() -> None:
