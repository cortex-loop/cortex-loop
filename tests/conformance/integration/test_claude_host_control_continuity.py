"""Integration tests for outbound Claude host-control continuity."""

from __future__ import annotations

from pathlib import Path

from cortex.hosts.claude import host_transport as claude_host_transport

from tests.conformance.integration._claude_service_harness import run_claude_service


REPO_ROOT = Path(__file__).resolve().parents[3]
FIXTURE_DIR = REPO_ROOT / "tests" / "conformance" / "fixtures"
CONTINUITY_FIXTURE_PATH = FIXTURE_DIR / "claude_host_control_continuity.json"


def test_claude_host_control_export_import_preserves_control_truth() -> None:
...
def _run_uninterrupted() -> tuple[list[dict[str, object]], dict[str, object]]:
    env = {
        claude_host_transport._FIXTURE_PATH_ENV: str(CONTINUITY_FIXTURE_PATH),
    }
    with run_claude_service(env=env) as service:
...
def _run_split() -> tuple[list[dict[str, object]], dict[str, object]]:
    first_env = {
        claude_host_transport._FIXTURE_PATH_ENV: str(CONTINUITY_FIXTURE_PATH),
        claude_host_transport._FIXTURE_START_INDEX_ENV: "0",
    }
    with run_claude_service(env=first_env) as first_service:
    ...
    second_env = {
        claude_host_transport._FIXTURE_PATH_ENV: str(CONTINUITY_FIXTURE_PATH),
        claude_host_transport._FIXTURE_START_INDEX_ENV: "1",
    }
    with run_claude_service(env=second_env) as second_service:
