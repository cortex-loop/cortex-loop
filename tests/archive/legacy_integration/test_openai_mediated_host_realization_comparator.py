...
from tests.archive.legacy_integration._openai_host_realization_pair import (
    OPENAI_HOST_REALIZATION_PAIR_KEYS,
    OPENAI_HOST_REALIZATION_PAIR_SPECS,
)
from tests.archive.legacy_integration._openai_mediated_lane_packet_example import (
...
from tests.archive.legacy_integration._openai_mediation_host_realization_experimental import (
...
def test_openai_host_realization_mediated_packet_matches_committed_doc() -> None:
    for pair_key in OPENAI_HOST_REALIZATION_PAIR_KEYS:
        committed_packet = packet_without_path(
            parse_run_packet(REPO_ROOT / OPENAI_HOST_REALIZATION_MEDIATED_PACKET_PATHS[pair_key])
        )

        assert build_openai_host_realization_mediated_packet(pair_key) == committed_packet
...
def test_openai_host_realization_pair_remains_fair_and_changes_only_specialization() -> None:
    ...
    for pair_key in OPENAI_HOST_REALIZATION_PAIR_KEYS:
        ...
        baseline_packet = build_openai_host_realization_baseline_packet(pair_key)
        mediated_packet = build_openai_host_realization_mediated_packet(pair_key)
        comparator = build_openai_host_realization_comparator_snapshot(pair_key)
    ...
    for values in seen_ids.values():
        assert len(values) == len(OPENAI_HOST_REALIZATION_PAIR_KEYS)
