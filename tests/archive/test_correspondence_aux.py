...
def test_aux_correspondence_text_keeps_geometry_lift_and_publication_rows_explicit() -> None:
    ...
    assert "| AUX-only SQLite episode store over bounded support-memory episodes with fingerprint dedupe and host/time filtering | `SqliteSupportMemoryStore` |" in correspondence_text
    assert "| host-scoped offline distillation over bounded stored episodes back into explicit offline publication | `distill_offline_support_publication()` |" in correspondence_text
