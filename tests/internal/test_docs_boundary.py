...
CURRENT_WORK_SLUG = "cortex-stop-only-retained-spine-live-gate1"
NEXT_PRODUCT_TRAIN_SLUG = "cortex-stop-only-retained-spine-live-run"
...
CODEX_APP_CLI_POSTTOOLUSE_TASK_STANDARD_PHASE_AWARE_NARROW_LIVE_RERUN_AFTER_CONTEXT_LOOP_TRACE_REMEDIATION_PATH = (
    REPO_ROOT
    / "docs"
    / "recon"
    / "cortex_codex_app_cli_posttooluse_task_standard_phase_aware_narrow_live_rerun_after_context_loop_trace_remediation.md"
)

CODEX_APP_CLI_POSTTOOLUSE_TASK_STANDARD_CLOSURE_REPORTING_ARCHITECTURE_DECISION_PATH = (
    REPO_ROOT
    / "docs"
    / "recon"
...
CODEX_APP_CLI_POSTTOOLUSE_TASK_STANDARD_MEASUREMENT_STACK_REBUILD_GATE0_PATH = (
    REPO_ROOT
    / "docs"
    / "recon"
...
CODEX_APP_CLI_POSTTOOLUSE_TASK_STANDARD_EXACTNESS_ONLY_PAIRED_VALUE_GATE0_PATH = (
    REPO_ROOT
    / "docs"
    / "recon"
...
CORTEX_EXECUTIVE_EFFECTIVENESS_EVALUATOR_BUILD_PATH = (
    REPO_ROOT
    / "docs"
    / "recon"
...
CORTEX_SIMPLE_HOOK_BASELINE_CHALLENGER_PATH = (
    REPO_ROOT
    / "docs"
    / "recon"
    / "cortex_simple_hook_baseline_challenger.md"
)
CORTEX_EXECUTIVE_EFFECTIVENESS_EVALUATOR_LIVE_MATRIX_RUN_PATH = (
    REPO_ROOT
    / "docs"
    / "recon"
    / "cortex_executive_effectiveness_evaluator_live_matrix_run.md"
)
CORTEX_EFFECTIVENESS_MEASUREMENT_STACK_REBUILD_GATE0_PATH = (
    REPO_ROOT
    / "docs"
    / "recon"
    / "cortex_effectiveness_measurement_stack_rebuild_gate0.md"
)
CORTEX_EFFECTIVENESS_V2_CASE_REGISTRY_GATE0_PATH = (
    REPO_ROOT
    / "docs"
    / "recon"
    / "cortex_effectiveness_v2_case_registry_gate0.md"
)
CORTEX_EFFECTIVENESS_V2_LIVE_MATRIX_GATE1_PATH = (
    REPO_ROOT
    / "docs"
    / "recon"
    / "cortex_effectiveness_v2_live_matrix_gate1.md"
)
CORTEX_EFFECTIVENESS_V2_LIVE_MATRIX_RUN_PATH = (
    REPO_ROOT
    / "docs"
    / "recon"
...
CORTEX_RETAINED_ACTIVE_POLICY_SPINE_GATE0_PATH = (
    REPO_ROOT
    / "docs"
    / "recon"
    / "cortex_retained_active_policy_spine_gate0.md"
)
CORTEX_RETAINED_ACTIVE_POLICY_SPINE_LIVE_GATE1_PATH = (
    REPO_ROOT
    / "docs"
    / "recon"
...
CORTEX_RETAINED_SPINE_LIVE_MATRIX_MATERIALIZATION_REMEDIATION_PATH = (
    REPO_ROOT
    / "docs"
    / "recon"
...
CORTEX_RETAINED_SPINE_CLEAN_CONTROL_STABILITY_GATE0_PATH = (
    REPO_ROOT
    / "docs"
    / "recon"
    / "cortex_retained_spine_clean_control_stability_gate0.md"
)
CORTEX_RETAINED_SPINE_CLEAN_CONTROL_REPLICATION_GATE1_PATH = (
    REPO_ROOT
    / "docs"
    / "recon"
...
CORTEX_STOP_ONLY_RETAINED_SPINE_GATE0_PATH = (
    REPO_ROOT
    / "docs"
    / "recon"
    / "cortex_stop_only_retained_spine_gate0.md"
)
CORTEX_STOP_ONLY_RETAINED_SPINE_LIVE_GATE1_PATH = (
    REPO_ROOT
    / "docs"
    / "recon"
    / "cortex_stop_only_retained_spine_live_gate1.md"
)
...
CODEX_APP_CLI_POSTTOOLUSE_TASK_STANDARD_PHASE_AWARE_NARROW_LIVE_RERUN_AFTER_SHARED_TOOL_EVIDENCE_PATH = (
    ...
    / "docs"
    / "recon"
    / "cortex_codex_app_cli_posttooluse_task_standard_phase_aware_narrow_live_rerun_after_shared_tool_evidence.md"
)
...
CODEX_APP_CLI_POSTTOOLUSE_SHARED_TOOL_EVIDENCE_CLASSIFICATION_PATH = (
    REPO_ROOT
    / "docs"
    / "recon"
...
def _assert_current_posttooluse_strategy_failure_audit_status(
    ...
    assert "stop-only retained spine live gate 1" in work_note
    assert "pass_cortex_stop_only_retained_spine_live_gate1" in work_note
    assert "cortex_stop_only_retained_spine_live_gate1.md" in work_note
    assert "stop_only_spine_contract.json" in work_note
    ...
    assert "four arms x five v2 task families x three repeats" in work_note
    assert "stop_only_closure_continuation_spine" in work_note
    assert "codex stop `hookspecificoutput` or block stdout continuation" in work_note
    assert "userpromptsubmit `hookspecificoutput.additionalcontext`" in work_note
    assert "posttooluse task-standard context" in work_note
    assert "model_io_path=none_lab_proof_only" in work_note
    assert "simple-hook prompt context remains support metadata" in work_note
    assert "silent rows emit no model-visible cortex output" in work_note
    assert "--stop-only-retained-spine-live-matrix" in work_note
    assert "cortex_codex_app_cli_stop_only_retained_spine_live_approved=approved" in work_note
    ...
    assert "silent success" in work_note
    assert "dry-run readiness only" in work_note
    ...
    assert "userpromptsubmit active value reactivation" in work_note
    assert "alphaevolve candidate-evolution permission" in work_note
    assert "next train is `cortex-stop-only-retained-spine-live-run`" in work_note

    next_train = status["next_product_train"]
    assert next_train["slug"] == NEXT_PRODUCT_TRAIN_SLUG
    assert next_train["surface"] == "approval-gated Stop-only retained-spine live matrix"
    assert "run the registered stop-only retained spine live matrix" in next_train[
        "executive_benefit"
    ].lower()
    assert "stop_only_closure_continuation_spine" in next_train[
        "executive_benefit"
    ].lower()
    assert "no-cortex" in next_train["executive_benefit"].lower()
    assert "simple-hook" in next_train["executive_benefit"].lower()
    assert "silent controls" in next_train["executive_benefit"].lower()
    assert "codex stop" in next_train["executive_benefit"].lower()
    assert "pass_cortex_stop_only_retained_spine_live_gate1" in next_train[
        "why_now"
    ].lower()
    assert "60-row dry-run plan" in next_train["why_now"].lower()
    assert "stop closure/continuation model i/o" in next_train["why_now"].lower()
    assert "cortex_codex_app_cli_stop_only_retained_spine_live_approved=approved" in next_train[
        "why_now"
    ].lower()
    assert "--stop-only-retained-spine-live-matrix" in next_train["why_now"].lower()
    assert "separately approved live run" in next_train["why_now"].lower()
    primary_metric = next_train["primary_metric"].lower()
    assert "approval-gated stop-only live run" in primary_metric
    assert "60-row artifact" in primary_metric
    assert "active rows as stop-only" in primary_metric
    assert "userpromptsubmit/posttooluse excluded" in primary_metric
    assert "no-cortex and simple-hook" in primary_metric
    assert "silent contamination or boundary dominance" in primary_metric
    guardrail = next_train["guardrail"].lower()
    assert "exact registered command/env pair" in guardrail
    assert "explicit current-turn approval" in guardrail
    ...
    assert "pretooluse denial" in guardrail
    assert "permissionrequest policy" in guardrail
    assert "runtime snapshot dependency" in guardrail
    assert "alphaevolve candidate evolution" in guardrail
    assert "positive value evidence requires user review" in guardrail
    assert "broad cortex lift or shipping truth" in guardrail
    kill_rule = next_train["kill_rule"].lower()
    assert "registered stop-only live command" in kill_rule
    assert "active rows use userpromptsubmit additionalcontext" in kill_rule
    assert "posttooluse context" in kill_rule
    assert "changes scoring or fixtures" in kill_rule
    assert "treats stop-only as value without live proof" in kill_rule
    assert "reactivates posttooluse" in kill_rule
    assert "queues candidate evolution" in kill_rule
    assert "omits gate 1 artifacts" in kill_rule
    assert "broad lift/product progress" in kill_rule
...
def test_codex_app_cli_hook_contract_capture_boundary_remediation_records_structural_fix() -> None:
    ...
    assert (
        status["work_today"]["slug"] == CURRENT_WORK_SLUG
    )
    assert (
        status["next_product_train"]["slug"]
        == NEXT_PRODUCT_TRAIN_SLUG
...
def test_codex_app_cli_posttooluse_task_standard_next_step_correction_records_gate0() -> None:
    ...
    assert "exact registered command/env pair" in status[
        "next_product_train"
    ]["guardrail"].lower()
...
def test_codex_app_cli_posttooluse_phase_aware_narrow_live_run_records_no_context() -> None:
    ...
    _assert_current_posttooluse_strategy_failure_audit_status(status)
    assert "model-visible cortex text" in status["next_product_train"][
        "guardrail"
    ].lower()
    assert "evaluator scoring or fixtures" in status["next_product_train"]["guardrail"].lower()
...
def test_codex_app_cli_posttooluse_shared_tool_evidence_records_gate0_pass() -> None:
    text = _read(CODEX_APP_CLI_POSTTOOLUSE_SHARED_TOOL_EVIDENCE_CLASSIFICATION_PATH)
    docs_index = _read(DOCS_INDEX_PATH)
    status = _load_status()

    assert "Surface: product host actuator plus SRE substrate proof" in text
    assert "Verdict: `pass_posttooluse_shared_tool_evidence_gate0`." in text
    assert "`ToolEvidenceObservation`" in text
    assert "`ToolEvidencePhase`" in text
    assert "pre_artifact_missing" in text
    assert "phase_check_failed" in text
    assert "candidate artifact creation" in text
    assert "readback" in text
    assert "markerless generic output" in text
    assert "`generic_check`" in text
    assert "`standard_aligned`" in text
    assert "status-only completion remains a generic SRE verification marker" in text
    assert "prior overcontrol, firing-boundary, phase-aware, and causal-trace" in text
    assert "No live Codex run occurred" in text
    assert "no behavior lift" in text.lower()
    assert "codex-app-cli-posttooluse-task-standard-phase-aware-narrow-live-rerun" in text
    assert (
        "recon/cortex_codex_app_cli_posttooluse_shared_tool_evidence_classification.md"
        in docs_index
    )
    assert (
...
def test_codex_app_cli_posttooluse_phase_aware_rerun_after_shared_tool_evidence_records_fail() -> None:
    ...
    assert "Surface: product live proof" in text
    assert "Verdict: `fail` with failure reason `repeated_posttooluse_context_loop`." in text
    assert "task_standard_posttooluse_live_20260507T213732Z" in text
    assert "PostToolUse context count: 3" in text
    assert "posttooluse_context_repeated" in text
    assert "task-standard:work_standard:929237a14cf4d738" in text
    assert "task-standard:closure_evidence:7608236188fd9be6" in text
    assert "posttooluse_context_trace_ambiguous" in text
    assert "posttooluse_tool_event_ref_not_found_in_stdout" in text
    assert "`call_*` tool ids" in text
    assert "`item_*` refs" in text
    assert "`clean_evidenced`: 0 PostToolUse contexts" in text
    assert "`honest_blocker`: 0 PostToolUse contexts" in text
    assert "`waiting_on_user`: 0 PostToolUse contexts" in text
    assert "`unrelated_tool`: 0 PostToolUse contexts" in text
    assert "no PostToolUse next-action effect claim" in text
    assert "codex-app-cli-posttooluse-task-standard-context-loop-trace-remediation" in text
    assert (
        "recon/cortex_codex_app_cli_posttooluse_task_standard_phase_aware_narrow_live_rerun_after_shared_tool_evidence.md"
        in docs_index
    )
    assert (
...
def test_codex_app_cli_posttooluse_context_loop_trace_remediation_records_gate0_pass() -> None:
    text = _read(CONTEXT_LOOP_TRACE_REMEDIATION_RECON_PATH)
    docs_index = _read(DOCS_INDEX_PATH)
    status = _load_status()

    assert "Surface: product host actuator plus lab trace proof" in text
    assert "Verdict: `pass_posttooluse_context_loop_trace_gate0`." in text
    assert "task_standard_posttooluse_live_20260507T213732Z" in text
    assert "posttooluse_context_active_context_pending" in text
    assert "tool_event_fingerprint" in text
    assert "no ordinal fallback" in text
    assert "duplicate or missing fingerprints are marked ambiguous" in text
    assert "behavior_lift_claim_allowed" in text
    assert "no broad Cortex behavior lift" in text
    assert "codex-app-cli-posttooluse-task-standard-phase-aware-narrow-live-rerun" in text
    assert (
        "recon/cortex_codex_app_cli_posttooluse_task_standard_context_loop_trace_remediation.md"
        in docs_index
    )
    assert (
...
def test_codex_app_cli_posttooluse_phase_aware_rerun_after_context_loop_trace_records_failure() -> None:
    ...
    assert "Surface: product live proof" in text
    assert "Verdict: `failure_context_ignored`" in text
    assert "final_closure_did_not_report_context_evidence" in text
    assert "task_standard_posttooluse_live_20260507T225019Z" in text
    assert "PostToolUse context count: 1" in text
    assert "`posttooluse_context_repeated`: `false`" in text
    assert "trace join source: `tool_event_fingerprint`" in text
    assert "`posttooluse_context_trace_ambiguous`: `false`" in text
    assert "`next_tool_matches_context`: `true`" in text
    assert "`final_closure_reports_context_evidence`: `false`" in text
    assert "`clean_evidenced`: 0 PostToolUse contexts" in text
    assert "`honest_blocker`: 0 PostToolUse contexts" in text
    assert "`waiting_on_user`: 0 PostToolUse contexts" in text
    assert "`unrelated_tool`: 0 PostToolUse contexts" in text
    ...
    assert (
        "recon/cortex_codex_app_cli_posttooluse_task_standard_phase_aware_narrow_live_rerun_after_context_loop_trace_remediation.md"
        in docs_index
    )
    assert (
...
def test_codex_app_cli_posttooluse_closure_reporting_architecture_decision_records_readout_underfit() -> None:
    ...
    assert "Surface: product architecture decision" in text
    assert "Verdict: `decision_queue_final_closure_readout_remediation_gate0`." in text
    assert "task_standard_posttooluse_live_20260507T225019Z" in text
    assert "lab final-closure readout underfit" in text
    assert "old live verdict remains `failure_context_ignored`" in text
    assert "PostToolUse context count: 1" in text
    assert "`posttooluse_context_repeated`: `false`" in text
    assert "trace join source: `tool_event_fingerprint`" in text
    assert "`next_tool_matches_context=true`" in text
    ...
    assert "`hex=616c7068612062657461206f6d656761`" in text
    assert "exactly `alpha beta omega`" in text
    assert "clean/control silence: passed" in text
    assert "hidden/root/runtime boundaries: passed" in text
    assert "no broad Cortex behavior lift" in text
    assert "no retroactive pass" in text
    assert (
        "codex-app-cli-posttooluse-task-standard-final-closure-readout-remediation-gate0"
        in text
    )
    assert (
        "recon/cortex_codex_app_cli_posttooluse_task_standard_closure_reporting_architecture_decision.md"
        in docs_index
    )
    assert (
...
def test_codex_app_cli_posttooluse_measurement_stack_gate0_records_episode_table() -> None:
    text = _read(
        CODEX_APP_CLI_POSTTOOLUSE_TASK_STANDARD_MEASUREMENT_STACK_REBUILD_GATE0_PATH
    )
    docs_index = _read(DOCS_INDEX_PATH)
    ...
    assert "Surface: lab proof architecture remediation" in text
    assert "Verdict: `pass_posttooluse_measurement_stack_gate0`." in text
    assert "evidence-recovery episode table" in text
    assert "live_trials_ran=false" in text
    assert "behavior_lift_claim_allowed=false" in text
    for artifact, classification in (
        ("task_standard_posttooluse_live_20260507T100836Z", "`true_next_action_ignore`"),
        ("task_standard_posttooluse_live_20260507T142129Z", "`failure_no_context`"),
        ("task_standard_posttooluse_live_20260507T153242Z", "`failure_overcontrol`"),
        (
    ...
    for phrase in (
        ...
        "`alpha beta omega`",
        "does not retroactively promote",
        "does not change the registered live decision function",
        "No live rerun or exactness value probe is authorized",
    ):
        assert phrase in text
    ...
    assert (
        "recon/cortex_codex_app_cli_posttooluse_task_standard_measurement_stack_rebuild_gate0.md"
        in docs_index
    )
    assert (
        "docs/recon/cortex_codex_app_cli_posttooluse_task_standard_measurement_stack_rebuild_gate0.md"
        in status["active_docs"]
    )
    _assert_current_posttooluse_strategy_failure_audit_status(status)
...
def test_codex_app_cli_posttooluse_exactness_only_paired_value_gate0_records_probe_design() -> None:
    text = _read(
        CODEX_APP_CLI_POSTTOOLUSE_TASK_STANDARD_EXACTNESS_ONLY_PAIRED_VALUE_GATE0_PATH
    )
    docs_index = _read(DOCS_INDEX_PATH)
    ...
    assert "Surface: lab proof paired-value design gate" in text
    assert "Verdict: `pass_posttooluse_exactness_only_paired_value_gate0`." in text
    for phrase in (
        ...
        "same model, same prompt, isolated workspaces",
        "matched `repeat_index`",
        "at least `4/5` mismatch pairs",
        "`active_beats_silent`",
        "`tie_no_value`",
        "not an active win",
        ...
        "task_standard_posttooluse_live_20260507T225019Z",
        "active-arm feasibility evidence only",
    ):
        assert phrase in text
...
def test_codex_app_cli_posttooluse_exactness_only_paired_value_live_records_negative_value() -> None:
    ...
    assert "codex-app-cli-posttooluse-task-standard-value-claims-pause-decision" in text
    assert (
...
def test_cortex_effectiveness_strategy_reset_regroups_after_negative_value() -> None:
    text = _read(CORTEX_EFFECTIVENESS_STRATEGY_RESET_PATH)
    docs_index = _read(DOCS_INDEX_PATH)
    status = _load_status()

    assert "Surface: product strategy reset" in text
    assert "Verdict: `queue_cortex_executive_effectiveness_evaluator_gate0`." in text
    for phrase in (
        "task_standard_posttooluse_paired_value_live_20260508T120907Z",
        "`failure_no_value`",
        "`0/5` mismatch",
        "`4/5` threshold",
        "not an AlphaEvolve-style optimization loop",
        "hard objective first",
        "automatic",
        "evaluator",
    ...
    assert (
        "recon/cortex_effectiveness_strategy_reset.md"
        in docs_index
    )
    assert "docs/recon/cortex_effectiveness_strategy_reset.md" in status["active_docs"]
    _assert_current_posttooluse_strategy_failure_audit_status(status)
...
def test_cortex_executive_effectiveness_evaluator_build_pins_executor() -> None:
    text = _read(CORTEX_EXECUTIVE_EFFECTIVENESS_EVALUATOR_BUILD_PATH)
    docs_index = _read(DOCS_INDEX_PATH)
    status = _load_status()

    assert "Surface: no-live lab/proof evaluator build" in text
    assert (
    ...
    for phrase in (
        ...
        "task_standard_posttooluse_paired_value_live_20260508T120907Z",
        "`failure_no_value`",
        "not counted as a new live run",
        "cycle state file",
        "`latest_cycle_state.json`",
        "22:00-07:00 overnight window",
        "allowed command",
        "non-test LOC growth",
        "policy/lab LOC growth",
        "contraction",
        "exact live command/env pairs",
    ...
    assert (
        "recon/cortex_executive_effectiveness_evaluator_build.md"
        in docs_index
    )
    assert (
        "docs/recon/cortex_executive_effectiveness_evaluator_build.md"
        in status["active_docs"]
    )
    _assert_current_posttooluse_strategy_failure_audit_status(status)
...
def test_cortex_simple_hook_baseline_challenger_pins_low_complexity_baseline() -> None:
    text = _read(CORTEX_SIMPLE_HOOK_BASELINE_CHALLENGER_PATH)
    docs_index = _read(DOCS_INDEX_PATH)
    status = _load_status()

    assert "`pass_cortex_simple_hook_baseline_challenger`" in text
    for phrase in (
        "`lab/cortex_simple_hook_baseline.py`",
        "under 500 nonblank noncomment LOC",
        "imports no `cortex/**` modules",
        "`cortex/core`",
        "`cortex/sre`",
        "`cortex/aux`",
        ...
        "`simple_hook_baseline` evaluator arm",
        "`--live-matrix`",
    ...
    for forbidden in (
        ...
        "does not earn Codex App parity",
        "does not earn shipping promotion",
        "does not earn product progress",
        "No product host behavior changed",
    ...
    assert "recon/cortex_simple_hook_baseline_challenger.md" in docs_index
    assert (
        "docs/recon/cortex_simple_hook_baseline_challenger.md"
        in status["active_docs"]
    )
    assert (
        status["next_product_train"]["slug"]
...
def test_cortex_executive_effectiveness_evaluator_live_matrix_run_records_negative_value() -> None:
    text = _read(CORTEX_EXECUTIVE_EFFECTIVENESS_EVALUATOR_LIVE_MATRIX_RUN_PATH)
    docs_index = _read(DOCS_INDEX_PATH)
    status = _load_status()
    ...
    for phrase in (
        ...
        "clean-control active rows did not overcontrol",
        "Active Cortex did not beat the simple-hook baseline",
        "baseline parity",
        "silent Cortex matched active Cortex",
    ...
    assert (
        "recon/cortex_executive_effectiveness_evaluator_live_matrix_run.md"
        in docs_index
    )
    assert (
...
def test_cortex_effectiveness_measurement_stack_rebuild_gate0_pins_diagnosis() -> None:
    text = _read(CORTEX_EFFECTIVENESS_MEASUREMENT_STACK_REBUILD_GATE0_PATH)
    docs_index = _read(DOCS_INDEX_PATH)
    status = _load_status()
    ...
    for phrase in (
        ...
        "`control_valid`",
        "`silent_contaminated`",
        "silent Cortex and active Cortex both scored `4` against `no_cortex_baseline` at `1`",
        "No current v1 live case was retroactively rescored",
        "Exactness/evidence recovery should require lifecycle evidence",
        ...
        "Continuity should remove prompt/workspace artifacts",
    ...
    assert (
        "recon/cortex_effectiveness_measurement_stack_rebuild_gate0.md"
        in docs_index
    )
    assert (
        "docs/recon/cortex_effectiveness_measurement_stack_rebuild_gate0.md"
        in status["active_docs"]
    )
    _assert_current_posttooluse_strategy_failure_audit_status(status)


def test_cortex_effectiveness_v2_case_registry_gate0_pins_registry() -> None:
    text = _read(CORTEX_EFFECTIVENESS_V2_CASE_REGISTRY_GATE0_PATH)
    docs_index = _read(DOCS_INDEX_PATH)
    status = _load_status()
    ...
    for phrase in (
        ...
        "`summary.json`",
        "`none_lab_proof_only`",
        "No v1 artifact was retroactively rescored",
        "`exactness_evidence_recovery_v2`",
        "`truthful_closure_v2`",
        "`blocker_surfacing_v2`",
        "`continuity_after_interruption_v2`",
        "`clean_verified_work_control_v2`",
        "post-observation correction",
        "generic success",
        "honest unresolved dependency reporting",
        "silent Cortex beat no-Cortex",
    ...
    assert "recon/cortex_effectiveness_v2_case_registry_gate0.md" in docs_index
    assert (
...
def test_cortex_effectiveness_v2_live_matrix_gate1_pins_dry_run_interface() -> None:
    text = _read(CORTEX_EFFECTIVENESS_V2_LIVE_MATRIX_GATE1_PATH)
    docs_index = _read(DOCS_INDEX_PATH)
    status = _load_status()

    for phrase in (
        "`pass_cortex_effectiveness_v2_live_matrix_gate1`",
        "no-live lab/proof seam",
        "cortex_effectiveness_v2_live_matrix_gate1",
        "`evaluator_design.json`",
        "`v2_case_registry.json`",
        "`live_plan.json`",
        "`episode_table.jsonl`",
        ...
        "Workspace seeds are matched across arms",
        "`live_trials_ran=false`",
        "`run_20260508T221352Z`",
        "`failure_silent_perception_contamination`",
        "does not replace\n`LIVE_MATRIX_CASES`",
    ...
    for forbidden in (
        ...
        "does not change product host behavior",
        "does not earn Cortex\nvalue",
        "behavior lift",
        "exactness value lift",
        "broad Cortex lift",
        "Codex App parity",
        "shipping promotion",
        "product progress",
        "AlphaEvolve candidate-evolution\npermission",
    ):
        assert forbidden in text

    assert "recon/cortex_effectiveness_v2_live_matrix_gate1.md" in docs_index
    assert (
...
def test_cortex_effectiveness_v2_live_matrix_run_records_no_value_result() -> None:
    text = _read(CORTEX_EFFECTIVENESS_V2_LIVE_MATRIX_RUN_PATH)
    docs_index = _read(DOCS_INDEX_PATH)
    status = _load_status()
    ...
    for phrase in (
        ...
        "`run_20260508T221352Z`",
        "`failure_silent_perception_contamination`",
        "No v1 artifact was retroactively\nrescored",
        "Active Cortex did not beat the simple-hook baseline or no-Cortex",
        "Every row was baseline parity",
    ...
    assert "recon/cortex_effectiveness_v2_live_matrix_run.md" in docs_index
    assert (
        "docs/recon/cortex_effectiveness_v2_live_matrix_run.md"
        in status["active_docs"]
    )
    _assert_current_posttooluse_strategy_failure_audit_status(status)
...
def test_cortex_retained_active_policy_spine_gate0_records_contract() -> None:
    text = _read(CORTEX_RETAINED_ACTIVE_POLICY_SPINE_GATE0_PATH)
    docs_index = _read(DOCS_INDEX_PATH)
    status = _load_status()
    ...
    for phrase in (
        ...
        "no dominance\nboundary failure",
        "`task_standard_posttooluse_paired_value_live_20260508T120907Z`",
        "active PostToolUse beat silent in `0/5` exactness mismatch",
        ...
        "No direct model-I/O",
        "`retained_but_value_unearned`",
        "`retained_state_law_not_active_value`",
        "`retained_support_law_not_active_value`",
        "`role_demoted_non_current_support_history`",
        "not reactivated as current\nstrategy",
    ...
    assert "recon/cortex_retained_active_policy_spine_gate0.md" in docs_index
    assert (
...
def test_cortex_retained_active_policy_spine_live_gate1_records_dry_run_plan() -> None:
    text = _read(CORTEX_RETAINED_ACTIVE_POLICY_SPINE_LIVE_GATE1_PATH)
    docs_index = _read(DOCS_INDEX_PATH)
    status = _load_status()
    ...
    for phrase in (
        ...
        "`failure_analysis.json`",
        "`60` rows",
        "`4` arms x `5` v2 task families x `3`",
        "`no_cortex_baseline`",
        "`simple_hook_baseline`",
        "`cortex_silent_perception`",
        "`cortex_active_policy`",
        "no active row uses a PostToolUse",
    ...
    assert "recon/cortex_retained_active_policy_spine_live_gate1.md" in docs_index
    assert (
...
def test_cortex_retained_spine_materialization_remediation_records_corrected_replay() -> None:
    text = _read(CORTEX_RETAINED_SPINE_LIVE_MATRIX_MATERIALIZATION_REMEDIATION_PATH)
    docs_index = _read(DOCS_INDEX_PATH)
    status = _load_status()
    ...
    assert (
        "recon/cortex_retained_spine_live_matrix_materialization_remediation.md"
        in docs_index
    )
    assert (
...
def test_cortex_retained_spine_clean_control_stability_gate0_records_diagnosis() -> None:
    text = _read(CORTEX_RETAINED_SPINE_CLEAN_CONTROL_STABILITY_GATE0_PATH)
    docs_index = _read(DOCS_INDEX_PATH)
    status = _load_status()
    ...
    for phrase in (
        ...
        "`clean_verified_work_control_v2` repeat 1",
        "`no_cortex_closure_readout_instability`",
        "`task_success=true`",
        "`v2_verifier_exit_code=0`",
        "`truthful_closure=false`",
        "`evidence_recovery=false`",
        "`stdout_stderr_and_verifier_artifacts`",
        "`command not found: python`",
        "successful `python3` fallback",
        "`model_io_path=none_lab_proof_only`",
        "`product_spine=[]`",
    ...
    assert "recon/cortex_retained_spine_clean_control_stability_gate0.md" in docs_index
    assert (
...
def test_cortex_retained_spine_clean_control_replication_gate1_records_plan() -> None:
    text = _read(CORTEX_RETAINED_SPINE_CLEAN_CONTROL_REPLICATION_GATE1_PATH)
    docs_index = _read(DOCS_INDEX_PATH)
    status = _load_status()
    ...
    for phrase in (
        ...
        "`clean_verified_work_control_v2`",
        "five repeats",
        "matched workspace seeds",
        "`userpromptsubmit_stop_taskstandard_spine`",
        "PostToolUse",
        "task-standard context remains disabled",
        "`not_run_approval_required`",
    ...
    assert (
        "recon/cortex_retained_spine_clean_control_replication_gate1.md"
        in docs_index
    )
    assert (
        "docs/recon/cortex_retained_spine_clean_control_replication_gate1.md"
        in status["active_docs"]
    )
    assert status["work_today"]["slug"] == CURRENT_WORK_SLUG
    assert status["next_product_train"]["slug"] == NEXT_PRODUCT_TRAIN_SLUG
    assert (
        ...
        == "approval-gated Stop-only retained-spine live matrix"
    )
    _assert_current_posttooluse_strategy_failure_audit_status(status)
...
def test_cortex_stop_only_retained_spine_gate0_records_contract() -> None:
    text = _read(CORTEX_STOP_ONLY_RETAINED_SPINE_GATE0_PATH)
    docs_index = _read(DOCS_INDEX_PATH)
    status = _load_status()
    ...
    for phrase in (
        ...
        "`cortex-stop-only-retained-spine-live-gate1`",
        "`stop_only_spine_contract.json`",
        "`gate0_report.json`",
        "`summary.json`",
        "Codex Stop `hookSpecificOutput` or block stdout continuation",
        "UserPromptSubmit task-standard context is not an active value path",
        "`TaskStandardSpine` state law",
        "`cortex/sre/tool_evidence.py` evidence classification",
        "transcript/tool evidence and closure-claim state",
    ...
    assert "recon/cortex_stop_only_retained_spine_gate0.md" in docs_index
    assert (
        "docs/recon/cortex_stop_only_retained_spine_gate0.md"
        in status["active_docs"]
    )
    assert status["work_today"]["slug"] == CURRENT_WORK_SLUG
    assert status["next_product_train"]["slug"] == NEXT_PRODUCT_TRAIN_SLUG
...
def test_cortex_stop_only_retained_spine_live_gate1_records_dry_run_plan() -> None:
    text = _read(CORTEX_STOP_ONLY_RETAINED_SPINE_LIVE_GATE1_PATH)
    docs_index = _read(DOCS_INDEX_PATH)
    status = _load_status()
    ...
    for phrase in (
        ...
        "failure_analysis.json",
        "registered_live_command.json",
        "60 rows: four arms x five v2 task families x three repeats",
        ...
        "Silent rows emit no model-visible Cortex output",
        "Simple-hook parity blocks value",
        "Silent success blocks value",
        "Queue `cortex-stop-only-retained-spine-live-run`",
        "This Gate 1 does not authorize live execution",
    ):
        assert phrase in text
    ...
    assert "recon/cortex_stop_only_retained_spine_live_gate1.md" in docs_index
    assert (
        "docs/recon/cortex_stop_only_retained_spine_live_gate1.md"
        in status["active_docs"]
    )
    assert status["work_today"]["slug"] == CURRENT_WORK_SLUG
    assert status["next_product_train"]["slug"] == NEXT_PRODUCT_TRAIN_SLUG
...
def test_status_registry_is_complete_and_stable() -> None:
    ...
    _assert_current_posttooluse_strategy_failure_audit_status(status)
    assert "exact registered command/env pair" in status["next_product_train"][
        "guardrail"
    ].lower()
...
def test_generated_status_doc_includes_system_map_and_next_product_train() -> None:
    ...
    assert "cortex_stop_only_retained_spine_gate0.md" in text
    assert "cortex_stop_only_retained_spine_live_gate1.md" in text
    assert "`cortex-stop-only-retained-spine-live-gate1`" in text
    assert "`cortex-stop-only-retained-spine-live-run`" in text
    assert "stop_only_closure_continuation_spine" in text
    assert "Codex Stop" in text
    assert "UserPromptSubmit `hookSpecificOutput.additionalContext`" in text
    assert "pass_cortex_stop_only_retained_spine_live_gate1" in text
    assert "Gate 1 passed" in text
    assert "stop_only_spine_contract.json" in text
    assert "evaluator_design.json" in text
    ...
    assert "PostToolUse task-standard context" in text
    assert "PreToolUse denial" in text
    assert "PermissionRequest policy" in text
    assert "PostToolUse reactivation" in text
    assert "UserPromptSubmit active value reactivation" in text
    assert "not_run_approval_required" in text
    assert "not_run_registered_future_live_only" in text
    assert "CORTEX_CODEX_APP_CLI_STOP_ONLY_RETAINED_SPINE_LIVE_APPROVED=approved" in text
    assert (
        "- Next product train after the current focus: "
        "`cortex-stop-only-retained-spine-live-run`"
    ) in text
    assert "- Train: `cortex-stop-only-retained-spine-live-run`" in text
    assert "approval-gated Stop-only retained-spine live matrix" in text
    assert "60-row dry-run plan" in text
    assert "registered approval refusal" in text
    assert "separately approved live run" in text
