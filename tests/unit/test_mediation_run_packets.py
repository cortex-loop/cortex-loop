"""Mechanical checks for committed mediation reference run packets."""
...
def test_experimental_reference_thrash_packet_matches_catalog_and_stays_experimental() -> None:
    scenarios = load_scenarios()
    failure_tags = load_failure_tags()
    packet = parse_run_packet(REFERENCE_THRASH_MEDIATED_PACKET_PATH)
    scenario = scenarios["scenario_thrash_reference_01"]

    assert REFERENCE_THRASH_MEDIATED_PACKET_PATH.is_file()
    assert packet["status"] == "reviewed_evidence"
    assert packet["header"]["scenario_id"] == "scenario_thrash_reference_01"
    assert packet["header"]["run_id"] == "reference_thrash_mediated_run_001"
    assert packet["header"]["paired_episode_set_id"] == "pair_reference_thrash_001"
    ...
    assert "one paired run is not enough" in reviewer_note


def test_reference_packet_directory_contains_three_baselines_and_one_experimental_packet() -> None:
