"""Mechanical checks for the committed OpenAI branch-discipline basis."""
...
def test_openai_branch_discipline_basis_docs_exist() -> None:
    ...
    assert status(BASIS_PATH) == "openai branch discipline basis satisfied"
    assert "three lawful openai branch-discipline comparator pairs are recorded" in text.lower()
...
def test_openai_branch_discipline_pairs_are_recorded() -> None:
    rows = parse_markdown_table(section(read(LEDGER_PATH), "Recorded Paired Runs"))
    branch_rows = [row for row in rows if row["scenario_id"] == "scenario_branch_openai_01"]
