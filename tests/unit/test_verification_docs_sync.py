...
def test_resume_protocol_and_active_workstream_contract_exist() -> None:
    ...
    assert "Current campaign: `L2e Gemini closure and pro comparison`" in workstream_text
    assert "Current candidate seam: Gemini flash/flash-lite closure mapping plus bounded pro comparison" in workstream_text
    ...
    assert "accept Gemini as the remaining partial host line for current scope" in workstream_text
...
def test_erika_visualizations_are_framed_as_support_surfaces() -> None:
    ...
    assert "The current open train is L2e Gemini closure and pro comparison." in markdown_text
...
def test_runtime_restack_program_lock_is_recorded() -> None:
    ...
    assert "now records the current L2e Gemini closure and pro-comparison seam" in theory_text
