...
def test_resume_protocol_and_active_workstream_contract_exist() -> None:
    ...
    assert "Accepted baseline branch: `codex/r2c-runtime-shell-integration`" in workstream_text
    assert "Accepted baseline commit: `50c37b0`" in workstream_text
    assert "Current working branch at ledger update: `codex/r2c-runtime-shell-integration`" in workstream_text
    assert "clean accepted runtime-shell branch after the first landed computed executive slice" in workstream_text
    assert "Current candidate seam: none yet; `R3A` is the next lawful opening move" in workstream_text
...
def test_reference_runtime_program_lock_is_recorded() -> None:
    ...
    assert "Current campaign: reference live continuity slice opening from an accepted computed-shell baseline" in workstream_text
    assert "R2` stop-gate re-audit answer:" in workstream_text
    assert "yes, the computed reference slice is now stable enough to open the first live continuity law" in workstream_text
    assert "Open `R3A` from this clean accepted head:" in workstream_text
...
def test_erika_visualizations_are_framed_as_support_surfaces() -> None:
    ...
    assert "**Accepted factual baseline:** `codex/r2c-runtime-shell-integration` at `50c37b0`" in markdown_text
    assert "first integrated computed executive slice is now accepted inside the runtime shell" in markdown_text
    assert "Open `R3A` and keep it strictly to live continuity law on top of the landed computed shell" in markdown_text
    ...
    assert "Accepted Baseline 50c37b0" in html_text
    assert "first integrated computed executive slice are real" not in html_text
    assert "first integrated computed executive slice now lives inside that shell" in html_text
    assert "Open live continuity law on top of the accepted computed shell" in html_text
